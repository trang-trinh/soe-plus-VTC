  ...
  var response = axios
    ...
    .then((response) => {
      ...
      if (
        ...
        (data[1].filter((x) => x.is_link == "/hrm/template/smart_report")
          .length == 0 ||
          !root_path.includes("/hrm/template/smart_report")) &&
        (data[1].filter((x) => x.is_link == "/hrm/profile").length == 0 ||
          !root_path.includes("/hrm/profile/report")) &&
