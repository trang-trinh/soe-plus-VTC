    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        data.forEach((element, i) => {
          element.STT =
            (opition.value.PageNo - 1) * opition.value.PageSize + i + 1;
          element.Ngay = moment(new Date(element.log_date)).format(
            "DD/MM/YYYY HH:mm:ss",
          );
        });
...
<template>
  <div
    :class="
      'main-layout ' +
      (opition.totalRecords > 10) +
      ' flex flex-column flex-grow-1 p-2'
    "
