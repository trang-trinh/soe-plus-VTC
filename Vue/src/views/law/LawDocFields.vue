...
const itemExport = "/Portals/Mau Excel/Mẫu Excel lĩnh vực luật.xlsx"; 
const filters = ref({
...
const loadCount = () => {
	axios
		.post(
			baseUrlCheck + "api/law_doc_fields/GetDataProc",
			{ 
				str: encr(JSON.stringify({
						proc: "law_doc_fields_count",
						par: [
		...
		axios
			...
			.then((response) => {
				...
				data.forEach((element, i) => {
					...
					element.open_date = moment(
						new Date(element.open_date)
					).format("DD/MM/YYYY");
...
const onPage = (event) => {
	if (event.page == 0) {
		...
		options.value.id =
			datalists.value[datalists.value.length - 1].law_field_id;
		options.value.IsNext = true;
		if (checkSort.value) {
			options.value.id = null;
		}
		...
		//Trang trước
		options.value.id = datalists.value[0].law_field_id;
		options.value.IsNext = false;
	}
...
const openBasic = (str) => {
	...
	if (store.state.user.is_super) {
		...
		field.value.organization_id = store.getters.user.organization_id;
	}
...
const saveField = (isFormValid) => {
	...
	if (field.value.field_name.includes(",")) {
		field.value.field_name = field.value.field_name
			.replace(/,/g, " -")
			.trim();
	}
	field.value.field_name = field.value.field_name.trim();
	swal.fire({
		width: 110,
		didOpen: () => {
			swal.showLoading();
	...
	axios({
		...
		url:
			baseUrlCheck +
			`/api/law_doc_fields/${
				issaveField.value ? "Update_Field" : "Add_Field"
			}`,
		data: field.value,
		headers: {
...
const delField = (Field) => {
	swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios
				.delete(baseUrlCheck + "/api/law_doc_fields/Delete_Field", {
					headers: { Authorization: `Bearer ${store.getters.token}` },
					data: Field != null ? [Field.law_field_id] : 1,
				})
				.then((response) => {
					swal.close();
					if (response.data.err != "1") {
	...
	axios
		.post(
			baseURL + "/api/Excel/ExportExcel",
			{
				...
				proc: "law_doc_fields_listexport",
...
const Upload = () => {
	...
	axios
		.post(
			baseUrlCheck + "/api/law_doc_fields/ImportExcel",
			formData,
			config
		)
		.then((response) => {
			...
			if (response.data.err == "0") {
				...
				swal.fire({
					title: "Error!",
					text: "Xảy ra lỗi khi import lĩnh vực văn bản luật!",
					icon: "error",
					confirmButtonText: "OK",
				});
			}
...
const onSort = (event) => {
	...
	if (event.sortField != "field_name") {
		options.value.sort +=
			",field_name " + (event.sortOrder == 1 ? " ASC" : " DESC");
	}
...
const loadDataSQL = () => {
	...
	axios
		.post(baseUrlCheck + "/api/SQL/Filter_Law_Doc_Fields", data, config)
		.then((response) => {
...
const onCheckBox = (value) => {
	let data = {
		IntID: value.law_field_id,
		TextID: value.law_field_id + "",
		IntTrangthai: 1,
		BitTrangthai: value.status,
	};
	axios
		.put(
			baseUrlCheck + "/api/law_doc_fields/Update_StatusFields",
			data,
			config
		)
		.then((response) => {
						...
						<div class="grid formgrid m-0">
							<div class="field col-12 md:col-12 p-0 mb-1">
								...
								<div>
									...
									<Toolbar
										class="border-none surface-0 outline-none pb-0 px-0"
									>
										<template #start>
											<Button
		...
		<form @submit.prevent="saveField(!v$.$invalid)">
			<div class="grid formgrid m-2">
				<div class="field col-12 md:col-12 flex">
					...
					>
						Tên lĩnh vực <span class="redsao pl-1"> (*)</span>
					</label>
					<InputText
						...
						:class="{
							'p-invalid': v$.field_name.$invalid && submitted,
						}"
					/>
				</div>
				<small class="col-12 p-error"
					v-if="(v$.field_name.required.$invalid && submitted) || v$.field_name.required.$pending.$response"
				>
					<div class="field col-12 md:col-12 flex">
				...
				<small class="col-12 p-error"
					v-if="(v$.field_name.maxLength.$invalid && submitted) || v$.field_name.maxLength.$pending.$response"
				>
					<div class="field col-12 md:col-12 flex">
						<label class="col-3 text-left"></label>
						<span class="col-9 p-0">
							{{
								v$.field_name.maxLength.$message.replace(
									"The maximum length allowed is",
									"Tên lĩnh vực không được vượt quá"
								)
							}}
							ký tự
						</span>
				...
				<div class="col-12 md:col-12 flex">
					...
					<div class="field col-6 md:col-6 p-0">
						...
						<InputSwitch v-model="field.status" />
					</div>
				</div>
			</div>
		</form>
