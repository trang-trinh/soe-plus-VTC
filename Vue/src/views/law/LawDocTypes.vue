...
const itemExport = "/Portals/Mau Excel/Mẫu Excel loại văn bản luật.xlsx"; 
const filters = ref({
...
const rules = {
	law_type_name: {
		...
		$errors: [
			{
				$property: "law_type_name",
				$validator: "required",
				$message: "Tên loại văn bản luật không được để trống!",
			},
		],
	},
	law_type_code: {
		...
		$errors: [
			{
				$property: "law_type_code",
				$validator: "required",
				$message: "Mã loại văn bản luật không được để trống!",
			},
		],
	},
};
...
const loadCount = () => {
	axios
		.post(
			baseUrlCheck + "api/law_doc_types/GetDataProc",
			{ 
...
const onPage = (event) => {
	if (event.page == 0) {
		...
		options.value.id =
			datalists.value[datalists.value.length - 1].law_type_id;
		options.value.IsNext = true;
	} else if (event.page < options.value.PageNo) {
		//Trang trước
		options.value.id = datalists.value[0].law_type_id;
		options.value.IsNext = false;
	}
...
const openBasic = (str) => {
	...
	if (store.state.user.is_super) {
		...
		caType.value.organization_id = store.getters.user.organization_id;
	}
...
const saveField = (isFormValid) => {
	...
	caType.value.law_type_name = caType.value.law_type_name.trim();
	swal.fire({
		width: 110,
		didOpen: () => {
			swal.showLoading();
	...
	axios({
		...
		url:
			baseUrlCheck +
			`/api/law_doc_types/${
				issaveField.value ? "Update_Type" : "Add_Type"
			}`,
		data: caType.value,
		headers: {
		...
		.then((response) => {
			if (response.data.err != "1") {
				swal.close();
				toast.success("Cập nhật loại văn bản luật thành công!");
				loadData(true);
				closeDialog();
			} else {
				...
				swal.fire({
					title: "Thông báo",
					text: "Xảy ra lỗi khi cập nhật văn",
					icon: "error",
					confirmButtonText: "OK",
				});
			}
...
const editField = (dataPlace) => {
	...
	headerDialog.value = "Sửa loại văn bản luật";
...
const delField = (Field) => {
	swal.fire({
		title: "Thông báo",
		text: "Bạn có muốn xoá loại văn bản luật này không!",
		icon: "warning",
		...
		if (result.isConfirmed) {
			...
			axios
				.delete(baseUrlCheck + "/api/law_doc_types/Delete_Type", {
					headers: { Authorization: `Bearer ${store.getters.token}` },
					data: Field != null ? [Field.law_type_id] : 1,
				})
				.then((response) => {
					swal.close();
					if (response.data.err != "1") {
				...
				.catch((error) => {
					swal.close();
					if (error.status === 401) {
						swal.fire({						text: "Mã token đã hết hạn hoặc không hợp lệ, vui lòng đăng nhập lại!",	
			confirmButtonText: "OK",
						});
					}
				});
...
const exportData = (method) => {
	...
	axios
		.post(
			...
			{
				excelname: "DANH SÁCH LOẠI VĂN BẢN LUẬT",
				proc: "law_doc_types_listexport",
...
const Upload = () => {
	...
	axios
		.post(baseUrlCheck + "/api/law_doc_types/ImportExcel", formData, config)
		.then((response) => {
			swal.close();
			if (response.data.err == "0") {
				...
				swal.fire({
					title: "Error!",
					text: "Xảy ra lỗi khi import loại văn bản luật!",
					icon: "error",
					confirmButtonText: "OK",
				});
			}
...
const onSort = (event) => {
	...
	if (event.sortField != "law_type_name") {
		options.value.sort +=
			",law_type_name " + (event.sortOrder == 1 ? " ASC" : " DESC");
	}
...
const loadDataSQL = () => {
	let data = {
		sqlS:
			filterTrangthai.value != null
				? filterTrangthai.value.toString()
				: null,
		sqlF:
			filterPhanloai.value != null
				? filterPhanloai.value.toString()
				: null,
		id: options.value.id,
	...
	axios
		.post(baseUrlCheck + "/api/SQL/Filter_Law_Doc_Types", data, config)
		.then((response) => {
...
const onFilter = (event) => {
	...
	for (const [key, value] of Object.entries(event.filters)) {
		if (key != "global") {
			let obj = {
				key: key != "law_type_name" ? "law_type_name" : key,
...
const onCheckBox = (value) => {
	let data = {
		IntID: value.law_type_id,
		TextID: value.law_type_id + "",
		IntTrangthai: 1,
		BitTrangthai: value.status,
	};
	axios
		.put(
			baseUrlCheck + "/api/law_doc_types/Update_StatusTypes",
			data,
			config
		)
		.then((response) => {
...
<template>
	<div class="surface-100">
		...
		<div class="d-lang-table ml-3">
			<DataTable
				...
				:globalFilterFields="['law_type_name']"
			>
				<Column
	...
	>
		<h3>
			<label>
				<a :href="basedomainURL + itemExport" download>Nhấn vào đây</a>
		...
		<form @submit.prevent="saveField(!v$.$invalid)">
			<div class="grid formgrid m-2">
				<div class="field col-12 md:col-12 flex">
					<label
						class="col-3 text-left p-0 flex"
						style="align-items: center"
					>
						Loại văn bản luật <span class="redsao pl-1"> (*)</span>
					</label>
					<InputText
						v-model="caType.law_type_name"
						spellcheck="false"
						class="col-9 p-0 m-0 ip36 px-2"
						:class="{ 'p-invalid': v$.law_type_name.$invalid && submitted }"
					/>
				</div>
				<small class="col-12 p-error"
					v-if="(v$.law_type_name.required.$invalid && submitted) || v$.law_type_name.required.$pending.$response"
				>
					<div class="field col-12 md:col-12 flex">
						<label class="col-3 text-left"></label>
						<span class="col-9 p-0">
              {{
								v$.law_type_name.required.$message
									.replace("Value", "Tên loại văn bản luật")
				...
				<small class="col-12 p-error"
					v-if="(v$.law_type_name.maxLength.$invalid && submitted) || v$.law_type_name.maxLength.$pending.$response"
				>
					<div class="field col-12 md:col-12 flex">
						<label class="col-3 text-left"></label>
						<span class="col-9 p-0"
							>{{
								v$.law_type_name.maxLength.$message.replace(
									"The maximum length allowed is",
									"Tên loại văn bản luật không được vượt quá"
								)
							}}
							ký tự
						</span>
				...
				<div class="field col-12 md:col-12 flex">
					<label
						...
						style="align-items: center"
					>
						Mã loại <span class="redsao pl-1"> (*)</span>
					</label>
					<InputText
						v-model="caType.law_type_code"
						class="col-9 p-0 ip36 px-2"
						:class="{
							'p-invalid': v$.law_type_code.$invalid && submitted,
						}"
					/>
				</div>
				<small
					...
					v-if="
						(v$.law_type_code.required.$invalid && submitted) || v$.law_type_code.required.$pending.$response
					"
				>
					<div class="field col-12 md:col-12 flex">
						<label class="col-3 text-left"></label>
						<span class="col-9 p-0"
							>{{
								v$.law_type_code.required.$message
									.replace("Value", "Mã loại văn bản luật")
									.replace("is required", "không được để trống")
							}}
						</span>
					</div>
				...
				<small
					class="col-12 p-error"
					v-if="
						(v$.law_type_code.maxLength.$invalid && submitted) || v$.law_type_code.maxLength.$pending.$response
					"
				>
					<div class="field col-12 md:col-12 flex">
						<label class="col-3 text-left"></label>
						<span class="col-9 p-0"
							>{{
								v$.law_type_code.maxLength.$message.replace(
									"The maximum length allowed is",
									"Mã loại văn bản luật không được vượt quá"
								)
							}}
							ký tự
						</span>
				...
				<div class="col-12 md:col-12 flex">
					...
					<div class="field col-6 md:col-6 p-0">
						...
						<InputSwitch v-model="caType.status" />
					</div>
				</div>
			</div>
		</form>
