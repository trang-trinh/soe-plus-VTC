...
const itemExport = "/Portals/Mau Excel/Mẫu Excel ngôn ngữ.xlsx"; 
const filters = ref({
    ...
    axios
      ...
      .then((response) => {
        ...
        data.forEach((element, i) => {
            ...
            element.open_date = moment(new Date(element.open_date)).format("DD/MM/YYYY");
            element.end_date = moment(new Date(element.end_date)).format("DD/MM/YYYY");
        });
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].law_language_id;
    options.value.IsNext = true;
    if (checkSort.value) {
      options.value.id = null;
    }
...
const Upload = () => {
  ...
  axios
    .post(baseUrlCheck +
      "/api/law_doc_language/ImportExcel", formData, config)
    .then((response) => {
      swal.close();
      if (response.data.err == "0") {
        ...
        loadData(true);
				...
				swal.fire({
					title: "Error!",
					text: "Xảy ra lỗi khi import ngôn ngữ văn bản luật!",
					icon: "error",
					confirmButtonText: "OK",
				});
			}
...
const onSort = (event) => {
  ...
  if (event.sortField != "language_name") {
    options.value.sort += ",language_name " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseUrlCheck +
      "/api/SQL/Filter_Law_Doc_Language", data, config)
    .then((response) => {
...
const editLanguage = (dataLan) => {
  ...
  headerDialog.value = "Sửa ngôn ngữ";
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseUrlCheck +
            "/api/law_doc_language/Delete_Language", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
...
const onCheckBox = (value) => {
  ...
  axios
    .put(baseUrlCheck +
      "/api/law_doc_language/Update_StatusLanguage", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
...
<template>
    ...
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-globe"></i> Danh sách ngôn ngữ ({{
          ...
          <template #body="Language">
            <div v-if="
              ...
              (store.state.user.is_admin == true &&
                store.state.user.organization_id == Language.data.organization_id)
              "
            >
              <Button
  ...
  >
      ...
      <div class="grid formgrid m-2">
          <div class="field col-12 md:col-12 flex">
            <label class="col-3 text-left p-0 flex" style="align-items: center;"> 
              Tên ngôn ngữ <span class="redsao pl-1"> (*)</span>
            </label>
            <InputText
              v-model="language.language_name"
              spellcheck="false"
              class="col-9 p-0 m-0 ip36 px-2"
              :class="{ 'p-invalid': v$.language_name.$invalid && submitted }"
            />
          </div>
          <small class="col-12 p-error"
              v-if="(v$.language_name.required.$invalid && submitted) || v$.language_name.required.$pending.$response"
            >
              <div class="field col-12 md:col-12 flex">
                <label class="col-3 text-left"></label>
                <span class="col-9 p-0">
                  {{
                    v$.language_name.required.$message
          ...
          <small class="col-12 p-error"
            v-if="(v$.language_name.maxLength.$invalid && submitted) || v$.language_name.maxLength.$pending.$response"
          >
            <div class="field col-12 md:col-12 flex">
              <label class="col-3 text-left"></label>
              <span class="col-9 p-0">
                {{
                  v$.language_name.maxLength.$message.replace(
                    "The maximum length allowed is",
                    "Tên ngôn ngữ không được vượt quá"
                  )
                }}
                ký tự
              </span>
          ...
          <div class="col-12 md:col-12 flex">
            <div class="field col-6 md:col-6 p-0">
              ...
              <InputNumber v-model="language.is_order" class="col-6 p-0 ip36" />
            </div>
            <div class="field col-6 md:col-6 p-0">
              ...
              <InputSwitch v-model="language.status" />
            </div>
          </div>
      </div>
    </form>
    <template #footer>
      ...
      <Button label="Lưu" icon="pi pi-check" @click="saveLanguage(!v$.$invalid)" />
    </template>
  </Dialog>
</template>
<style scoped>
