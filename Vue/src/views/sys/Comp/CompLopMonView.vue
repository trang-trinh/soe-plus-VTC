...
const loadLopMonhoc = () => {
  axios
    ...
    .then((response) => {
      ...
      let sm = [...new Set(monhoclops.value.filter(x=>x.Monhoc_ID).map((x) => x.Monhoc_ID))].length;
      let smgv = [...new Set(monhoclops.value.map((x) => x.user_id))].length;
...
const addGVMon = (data) => {
  mlop.value = { Monhocs: [data.Monhoc_ID], Bosach_ID: data.Bosach_ID };
  saveMon();
};
const saveMon = () => {
  opAddMon.value.hide();
  ...
  let mons = [];
  if (mlop.value.Monhocs) {
    mlop.value.Monhocs.forEach((m) => {
      mons.push({
        LopUser_ID: -1,
...
<template>
  <div class="main-layout true flex-grow-1 p-2'" style="min-height: 450px !important">
    <DataTable
      ...
      groupRowsBy="Monhoc_Ten"
    >
      <template #groupheader="slotProps">
        <i class="pi pi-book mr-2"></i> {{ slotProps.data.Monhoc_Ten }}
        <Button
          v-if="props.lophoc.IsPermision"
          type="button"
          v-tooltip="'Thêm giáo viên giảng dạy'"
      ...
      <template #header>
        <Toolbar class="w-full custoolbar">
          <template #start>
            <Button v-if="props.lophoc.IsPermision" label="Cập nhật" icon="pi pi-save" @click="saveTableMon()" />
          </template>
          <template #end>
            <Button
