...
const options = ref({
  isNext: true,
  loading: false,
  profile_id: store.getters.user.profile_id,
...
const searchUser = () => {
  ...
  tempusers.value = filterusers.value.filter(
    (x) =>
      ...
      (x["profile_nick_name"] || "").toLowerCase().indexOf(s) !== -1
  );
};
                    ...
                    <Avatar
                      v-bind:label="
                        slotProps.data.avatar
                          ? ''
                          : slotProps.data.profile_nick_name.substring(0, 1)
                      "
