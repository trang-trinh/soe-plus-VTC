...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].title_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].title_id;
    options.value.IsNext = false;
  }
...
const saveData = (isFormValid) => {
  ...
  if (title.value.title_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên chức danh không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (title.value.countryside_fake)
    title.value.countryside = title.value.countryside_fake;
  formData.append("hrm_ca_title", JSON.stringify(title.value));
  swal.fire({
    width: 110,
    didOpen: () => {
      swal.showLoading();
  ...
  if (!isSaveTem.value) {
    axios
      .post(baseURL + "/api/hrm_ca_title/add_hrm_ca_title", formData, config)
      .then((response) => {
    ...
    axios
      .put(baseURL + "/api/hrm_ca_title/update_hrm_ca_title", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa chức danh thành công!");
...
const editTem = (dataTem) => {
  ...
  if (store.state.user.is_admin) {
    title.value.organization_id = 0;
  } else {
    title.value.organization_id = 1;
  }
};
//Xóa bản ghi
const delTem = (Tem) => {
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/hrm_ca_title/delete_hrm_ca_title", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Tem != null ? [Tem.title_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá chức danh thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.title_id,
      TextID: value.title_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(baseURL + "/api/hrm_ca_title/update_s_hrm_ca_title", data, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái chức danh thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
    ...
    axios
      .put(baseURL + "/api/hrm_ca_title/Update_DefaultStamp", data1, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái chức danh thành công!");
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa chức danh mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá chức danh này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      <template #header>
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm chức danh')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      <div class="grid formgrid m-2">
        <div class="field col-12 md:col-12">
          <label class="col-3 text-left p-0"
            >Tên chức danh <span class="redsao">(*)</span></label
          >
          <InputText
            ...
            :class="{
              'p-invalid': v$.title_name.$invalid && submitted,
            }"
          />
        </div>
        <div style="display: flex" class="field col-12 md:col-12">
          ...
          >
            <span class="col-12 p-0">{{
              v$.title_name.required.$message
                .replace("Value", "Tên chức danh")
        ...
        <div class="col-12 field md:col-12 flex">
          <div class="field col-6 md:col-6 p-0 align-items-center flex">
            ...
            <InputNumber v-model="title.is_order" class="col-6 ip36 p-0" />
          </div>
          <div class="field col-6 md:col-6 p-0 align-items-center flex">
            <div class="col-6 text-center p-0">Trạng thái</div>
            <InputSwitch v-model="title.status" />
          </div>
        </div>
      </div>
    </form>
