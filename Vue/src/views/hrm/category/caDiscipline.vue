  ...
  if (!isSaveTem.value) {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa hình thức kỉ luật thành công!");
...
const editTem = (dataTem) => {
  ...
  headerDialog.value = "Sửa hình thức kỉ luật";
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá hình thức kỉ luật thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
  ...
  if (check) {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái hình thức kỉ luật thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa hình thức kỉ luật mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá hình thức kỉ luật này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách hình thức kỉ luật ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm hình thức kỉ luật')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
