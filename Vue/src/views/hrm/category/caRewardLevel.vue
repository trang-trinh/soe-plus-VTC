...
const rules = {
  reward_level_name: {
    ...
    $errors: [
      {
        $property: "reward_level_name",
        $validator: "required",
        $message: "Tên cấp khen thưởng không được để trống!",
      },
    ],
  },
      ...
      baseURL + "/api/hrm_ca_SQL/getData",
        {
          str: encr(
            JSON.stringify({
        proc: "hrm_ca_reward_level_count",
        par: [
...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id =
      datalists.value[datalists.value.length - 1].reward_level_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].reward_level_id;
    options.value.IsNext = false;
  }
...
const v$ = useVuelidate(rules, reward_level);
...
const saveData = (isFormValid) => {
  ...
  if (reward_level.value.reward_level_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên cấp khen thưởng không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (!isSaveTem.value) {
    axios
      .post(
        baseURL + "/api/hrm_ca_reward_level/add_hrm_ca_reward_level",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Thêm cấp khen thưởng thành công!");
          loadData(true);
    ...
    axios
      .put(
        baseURL + "/api/hrm_ca_reward_level/update_hrm_ca_reward_level",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa cấp khen thưởng thành công!");
...
const editTem = (dataTem) => {
  submitted.value = false;
  reward_level.value = dataTem;
  if (reward_level.value.countryside)
    reward_level.value.countryside_fake = reward_level.value.countryside;
  if (reward_level.value.is_default) {
    checkIsmain.value = false;
  } else {
    checkIsmain.value = true;
  }
  headerDialog.value = "Sửa cấp khen thưởng";
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(
            baseURL + "/api/hrm_ca_reward_level/delete_hrm_ca_reward_level",
            {
              headers: { Authorization: `Bearer ${store.getters.token}` },
              data: Tem != null ? [Tem.reward_level_id] : 1,
            }
          )
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá cấp khen thưởng thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/hrm_ca_SQL/Filter_hrm_ca_reward_level", data, config)
    .then((response) => {
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.reward_level_id,
      TextID: value.reward_level_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(
        baseURL + "/api/hrm_ca_reward_level/update_s_hrm_ca_reward_level",
        data,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái cấp khen thưởng thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa cấp khen thưởng mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá cấp khen thưởng này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          selectedStamps.value.forEach((item) => {
            listId.push(item.reward_level_id);
          });
          axios
            .delete(
              baseURL + "/api/hrm_ca_reward_level/delete_hrm_ca_reward_level",
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách cấp khen thưởng ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm cấp khen thưởng')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      <div class="grid formgrid m-2">
        ...
        <div class="field col-12 md:col-12">
          <label class="col-3 text-left p-0"
            >Cấp khen thưởng <span class="redsao">(*)</span></label
          >
          <InputText
            v-model="reward_level.reward_level_name"
            spellcheck="false"
            class="col-9 ip36 px-2"
            :class="{
              'p-invalid': v$.reward_level_name.$invalid && submitted,
            }"
          />
        </div>
        <div style="display: flex" class="field col-12 md:col-12">
          ...
          <small
            v-if="
              (v$.reward_level_name.$invalid && submitted) ||
              v$.reward_level_name.$pending.$response
            "
            class="col-9 p-error"
          >
            <span class="col-12 p-0">{{
              v$.reward_level_name.required.$message
                .replace("Value", "Tên cấp khen thưởng")
        ...
        <div class="col-12 field md:col-12 flex">
          ...
          <div class="field col-4 md:col-4 p-0 align-items-center flex">
            ...
            <InputSwitch v-model="reward_level.status" />
          </div>
          <div
          ...
          >
            ...
            <InputSwitch v-model="reward_level.is_system" />
          </div>
        </div>
