    ...
    $errors: [
      {
        ...
        $validator: "required",
        $message: "Tên danh hiệu khen thưởng không được để trống!",
      },
    ],
  },
...
const saveData = (isFormValid) => {
  ...
  if (reward_title.value.reward_title_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên danh hiệu khen thưởng không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (!isSaveTem.value) {
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Thêm danh hiệu khen thưởng thành công!");
          loadData(true);
    ...
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa danh hiệu khen thưởng thành công!");
...
const editTem = (dataTem) => {
  ...
  headerDialog.value = "Sửa danh hiệu khen thưởng";
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá danh hiệu khen thưởng thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
  ...
  if (check) {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái danh hiệu khen thưởng thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa danh hiệu khen thưởng mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá danh hiệu khen thưởng này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách danh hiệu khen thưởng ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm danh hiệu khen thưởng')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      <Column
        ...
        header="Tên danh hiệu khen thưởng"
        :sortable="true"
          ...
          >
            <span class="col-12 p-0">{{
              v$.reward_title_name.required.$message
                .replace("Value", "Tên danh hiệu khen thưởng")
