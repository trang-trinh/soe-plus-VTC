...
const rules = {
  personel_groups_name: {
    ...
    $errors: [
      {
        $property: "personel_groups_name",
        $validator: "required",
        $message: "Tên loại nhân sự không được để trống!",
      },
    ],
  },
      ...
      baseURL + "/api/hrm_ca_SQL/getData",
        {
          str: encr(
            JSON.stringify({
        proc: "hrm_ca_personel_groups_count",
        par: [
      ...
      baseURL + "/api/hrm_ca_SQL/getData",
        {
          str: encr(
            JSON.stringify({
          proc: "hrm_ca_personel_groups_list",
          par: [
...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id =
      datalists.value[datalists.value.length - 1].personel_groups_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].personel_groups_id;
    options.value.IsNext = false;
  }
...
const v$ = useVuelidate(rules, personel_groups);
...
const saveData = (isFormValid) => {
  ...
  if (personel_groups.value.personel_groups_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên loại nhân sự không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (personel_groups.value.countryside_fake)
    personel_groups.value.countryside = personel_groups.value.countryside_fake;
  formData.append("hrm_ca_personel_groups", JSON.stringify(personel_groups.value));
  swal.fire({
    width: 110,
    didOpen: () => {
      swal.showLoading();
  ...
  if (!isSaveTem.value) {
    axios
      .post(
        baseURL + "/api/hrm_ca_personel_groups/add_hrm_ca_personel_groups",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Thêm loại nhân sự thành công!");
          loadData(true);
    ...
    axios
      .put(
        baseURL + "/api/hrm_ca_personel_groups/update_hrm_ca_personel_groups",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa loại nhân sự thành công!");
...
const editTem = (dataTem) => {
  submitted.value = false;
  personel_groups.value = dataTem;
  if (personel_groups.value.countryside)
    personel_groups.value.countryside_fake = personel_groups.value.countryside;
  if (personel_groups.value.is_default) {
    checkIsmain.value = false;
  } else {
    checkIsmain.value = true;
  }
  headerDialog.value = "Sửa loại nhân sự";
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(
            baseURL + "/api/hrm_ca_personel_groups/delete_hrm_ca_personel_groups",
            {
              headers: { Authorization: `Bearer ${store.getters.token}` },
              data: Tem != null ? [Tem.personel_groups_id] : 1,
            }
          )
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá loại nhân sự thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/hrm_ca_SQL/Filter_hrm_ca_personel_groups", data, config)
    .then((response) => {
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.personel_groups_id,
      TextID: value.personel_groups_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(
        baseURL + "/api/hrm_ca_personel_groups/update_s_hrm_ca_personel_groups",
        data,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái loại nhân sự thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
    ...
    axios
      .put(
        baseURL + "/api/hrm_ca_personel_groups/Update_DefaultStamp",
        data1,
        config
      )
      .then((response) => {
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa loại nhân sự mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá loại nhân sự này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          selectedStamps.value.forEach((item) => {
            listId.push(item.personel_groups_id);
          });
          axios
            .delete(
              baseURL + "/api/hrm_ca_personel_groups/delete_hrm_ca_personel_groups",
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách loại nhân sự ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm loại nhân sự')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      <Column
        field="personel_groups_name"
        header="Tên loại nhân sự"
        :sortable="true"
      ...
      <div class="grid formgrid m-2">
          ...
          >
            <span class="col-12 p-0">{{
              v$.personel_groups_name.required.$message
                .replace("Value", "Tên loại nhân sự")
        ...
        <div class="col-12 field md:col-12 flex">
          ...
          <div class="field col-4 md:col-4 p-0 align-items-center flex">
            ...
            <InputSwitch v-model="personel_groups.status" />
          </div>
          <div
          ...
          >
            ...
            <InputSwitch v-model="personel_groups.is_system" />
          </div>
        </div>
