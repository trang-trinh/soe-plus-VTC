...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].vacancy_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].vacancy_id;
    options.value.IsNext = false;
  }
...
const saveData = (isFormValid) => {
  ...
  if (vacancy.value.countryside_fake)
    vacancy.value.countryside = vacancy.value.countryside_fake;
  formData.append("hrm_ca_vacancy", JSON.stringify(vacancy.value));
  swal.fire({
    width: 110,
    didOpen: () => {
      swal.showLoading();
  ...
  if (!isSaveTem.value) {
    axios
      .post(baseURL + "/api/hrm_ca_vacancy/add_hrm_ca_vacancy", formData, config)
      .then((response) => {
    ...
    axios
      .put(baseURL + "/api/hrm_ca_vacancy/update_hrm_ca_vacancy", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const editTem = (dataTem) => {
  submitted.value = false;
  vacancy.value = dataTem;
  if (vacancy.value.countryside)
    vacancy.value.countryside_fake = vacancy.value.countryside;
  if (vacancy.value.is_default) {
    checkIsmain.value = false;
  } else {
  ...
  if (store.state.user.is_admin) {
    vacancy.value.organization_id = 0;
  } else {
    vacancy.value.organization_id = 1;
  }
};
//Xóa bản ghi
const delTem = (Tem) => {
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/hrm_ca_vacancy/delete_hrm_ca_vacancy", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Tem != null ? [Tem.vacancy_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/hrm_ca_SQL/Filter_hrm_ca_vacancy", data, config)
    .then((response) => {
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.vacancy_id,
      TextID: value.vacancy_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(baseURL + "/api/hrm_ca_vacancy/update_s_hrm_ca_vacancy", data, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
    ...
    axios
      .put(baseURL + "/api/hrm_ca_vacancy/Update_DefaultStamp", data1, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái vị trí tuyển dụng thành công!");
      ...
      <div class="grid formgrid m-2">
        <div class="field col-12 md:col-12">
          ...
          <InputText
            ...
            :class="{
              'p-invalid': v$.vacancy_name.$invalid && submitted,
            }"
          />
        </div>
        <div style="display: flex" class="field col-12 md:col-12">
          ...
          <small
            v-if="
              (v$.vacancy_name.$invalid && submitted) ||
              v$.vacancy_name.$pending.$response
            "
            class="col-9 p-error"
          >
