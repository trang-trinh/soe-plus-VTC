...
const rules = {
  experience_name: {
    ...
    $errors: [
      {
        $property: "experience_name",
        $validator: "required",
        $message: "Tên kinh nghiệm không được để trống!",
      },
    ],
  },
};
...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].experience_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].experience_id;
    options.value.IsNext = false;
  }
...
const saveData = (isFormValid) => {
  ...
  if (experience.value.experience_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên kinh nghiệm không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (experience.value.countryside_fake)
    experience.value.countryside = experience.value.countryside_fake;
  formData.append("hrm_ca_experience", JSON.stringify(experience.value));
  swal.fire({
    width: 110,
    didOpen: () => {
      swal.showLoading();
  ...
  if (!isSaveTem.value) {
    axios
      .post(baseURL + "/api/hrm_ca_experience/add_hrm_ca_experience", formData, config)
      .then((response) => {
    ...
    axios
      .put(baseURL + "/api/hrm_ca_experience/update_hrm_ca_experience", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const editTem = (dataTem) => {
  submitted.value = false;
  experience.value = dataTem;
  if (experience.value.countryside)
    experience.value.countryside_fake = experience.value.countryside;
  if (experience.value.is_default) {
    checkIsmain.value = false;
  } else {
    checkIsmain.value = true;
  }
  headerDialog.value = "Sửa kinh nghiệm";
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/hrm_ca_experience/delete_hrm_ca_experience", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Tem != null ? [Tem.experience_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/hrm_ca_SQL/Filter_hrm_ca_experience", data, config)
    .then((response) => {
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.experience_id,
      TextID: value.experience_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(baseURL + "/api/hrm_ca_experience/update_s_hrm_ca_experience", data, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái kinh nghiệm thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá kinh nghiệm này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          selectedStamps.value.forEach((item) => {
            listId.push(item.experience_id);
          });
          axios
            .delete(baseURL + "/api/hrm_ca_experience/delete_hrm_ca_experience", {
              headers: { Authorization: `Bearer ${store.getters.token}` },
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-building"></i> Danh sách kinh nghiệm ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm kinh nghiệm')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      <div class="grid formgrid m-2">
        <div class="field col-12 md:col-12">
          <label class="col-3 text-left p-0"
            >Tên kinh nghiệm <span class="redsao">(*)</span></label
          >
          <InputText
            ...
            :class="{
              'p-invalid': v$.experience_name.$invalid && submitted,
            }"
          />
        </div>
        <div style="display: flex" class="field col-12 md:col-12">
          ...
          <small
            v-if="
              (v$.experience_name.$invalid && submitted) ||
              v$.experience_name.$pending.$response
            "
            class="col-9 p-error"
          >
            <span class="col-12 p-0">{{
              v$.experience_name.required.$message
                .replace("Value", "Tên kinh nghiệm")
        ...
        <div class="col-12 field md:col-12 flex">
          ...
          >
            ...
            <InputSwitch v-model="experience.is_system" />
          </div>
        </div>
