...
const rules = {
  receipt_name: {
    ...
    $errors: [
      {
        $property: "receipt_name",
        $validator: "required",
        $message: "Tên giấy tờ không được để trống!",
      },
    ],
  },
...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id =
      datalists.value[datalists.value.length - 1].receipt_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].receipt_id;
    options.value.IsNext = false;
  }
...
const v$ = useVuelidate(rules, receipt);
...
const saveData = (isFormValid) => {
  ...
  if (receipt.value.receipt_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên giấy tờ không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (receipt.value.countryside_fake)
    receipt.value.countryside = receipt.value.countryside_fake;
  formData.append("hrm_ca_receipt", JSON.stringify(receipt.value));
  swal.fire({
    width: 110,
    didOpen: () => {
      swal.showLoading();
  ...
  if (!isSaveTem.value) {
    axios
      .post(
        baseURL + "/api/hrm_ca_receipt/add_hrm_ca_receipt",
        formData,
        config
      )
      .then((response) => {
    ...
    axios
      .put(
        baseURL + "/api/hrm_ca_receipt/update_hrm_ca_receipt",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa giấy tờ thành công!");
...
const editTem = (dataTem) => {
  submitted.value = false;
  receipt.value = dataTem;
  if (receipt.value.countryside)
    receipt.value.countryside_fake = receipt.value.countryside;
  if (receipt.value.is_default) {
    checkIsmain.value = false;
  } else {
    checkIsmain.value = true;
  }
  headerDialog.value = "Sửa giấy tờ";
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(
            baseURL + "/api/hrm_ca_receipt/delete_hrm_ca_receipt",
            {
              headers: { Authorization: `Bearer ${store.getters.token}` },
              data: Tem != null ? [Tem.receipt_id] : 1,
            }
          )
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá giấy tờ thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/hrm_ca_SQL/Filter_hrm_ca_receipt", data, config)
    .then((response) => {
...
const onCheckBox = (value, check, checkIsmain) => {
  if (check) {
    let data = {
      IntID: value.receipt_id,
      TextID: value.receipt_id + "",
      IntTrangthai: 1,
      BitTrangthai: value.status,
    };
    axios
      .put(
        baseURL + "/api/hrm_ca_receipt/update_s_hrm_ca_receipt",
        data,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái giấy tờ thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá giấy tờ này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách giấy tờ ({{
    ...
    <form>
      <div class="grid formgrid m-2">
        ...
        <div class="field col-12 md:col-12">
          ...
          >
          <InputText
            v-model="receipt.receipt_name"
            spellcheck="false"
            class="col-9 ip36 px-2"
            :class="{
              'p-invalid': v$.receipt_name.$invalid && submitted,
            }"
          />
        </div>
        <div style="display: flex" class="field col-12 md:col-12">
          ...
          <small
            v-if="
              (v$.receipt_name.$invalid && submitted) ||
              v$.receipt_name.$pending.$response
            "
            class="col-9 p-error"
          >
            <span class="col-12 p-0">{{
              v$.receipt_name.required.$message
                .replace("Value", "Tên giấy tờ")
        ...
        <div class="col-12 field md:col-12 flex">
          ...
          >
            ...
            <InputSwitch v-model="receipt.is_system" />
          </div>
        </div>
