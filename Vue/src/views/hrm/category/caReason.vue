    ...
    $errors: [
      {
        ...
        $validator: "required",
        $message: "Tên lý do không được để trống!",
      },
    ],
  },
};
...
const saveData = (isFormValid) => {
  ...
  if (discipline.value.discipline_name.length > 250) {
    swal.fire({
      title: "Error!",
      text: "Tên lý do không được vượt quá 250 ký tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá lý do thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
  ...
  if (check) {
    ...
    axios
      ...
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái lý do thành công!");
          loadData(true);
          closeDialog();
        } else {
          swal.fire({
...
const deleteList = () => {
  ...
  selectedStamps.value.forEach((item) => {
    if (item.is_default) {
      toast.error("Không được xóa lý do mặc định!");
      checkD = true;
      return;
    }
  });
  if (!checkD) {
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xoá lý do này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
...
const listReason = ref([
  { name: "Bổ nhiệm", code: 1 },
  { name: "Miễn nhiệm", code: 2 },
  { name: "Điều chuyển", code: 3 },
  { name: "Chấm dứt HĐ lao động", code: 4 },
]);
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-credit-card"></i> Danh sách lý do ({{
