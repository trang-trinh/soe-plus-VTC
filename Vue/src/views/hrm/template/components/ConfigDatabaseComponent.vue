    ...
    setup(props, ctx) {
            ...
            if (axResponse.status == 200) {
                if (axResponse.data.error) {
                    ...
                    if (mdProc.value) {
                        ...
                        let dfs = props.report.proc_name ? props.report.proc_name.split(" ") : [];
                        if (dfs.length > 0)
                            dts.forEach((dt, i) => {
                                sql +=dfs[i + 1];
                            });
        ...
        const goSQL = async (sql) => {
            ...
            if (axResponse.status == 200) {
                if (axResponse.data.error) {
                    ...
                    mdTable.value = dtTables.value.filter(x => x.stt == (props.group.tid || props.group.key || 0))[0];
            ...
            try {
                ...
                if (axResponse.status == 200) {
                    if (axResponse.data.error) {
                        ...
                        if (props.report.report_config && props.report.report_config.trim() != "") {
        ...
        const saveDatabase = () => {
            ...
            arrChons.forEach(tb => {
                ...
                if (!cc) {
                    cc = { column_id: mdTable.value.stt, column_title: mdTable.value.table_name }
                }
                tbc.cols = [cc];
                tbchons.push(tbc);
            });
            props.callbackFun(tbchons, { name: mdProc.value ? mdProc.value.proc_name : "", parameters: dtPars.value, sql: txtSQL.value, issql: (txtSQL.value||"").toLowerCase().includes('select ') });
        }
        onMounted(() => {
            if (props.report.proc_name && props.report.proc_name.toLowerCase().includes("select")) {
                mdTable.value = { stt: 0, tid: 0, cols: [], issql: true }
                txtSQL.value = props.report.proc_name;
                goSQL(txtSQL.value);
            } else
                if (dtProcs.value.length == 0) {
                    listProc();
                }
        });
        ...
        return {
            ...
            goSQL, goTableProc,
            dtTables,
            mdTable, filterconfigDatabases, searchCols,
            mdProc, txtSQL
        }
    },
})
...
<template>
    <DataTable v-model:selection="selectedTabel" v-model:filters="filterconfigDatabases"
        :globalFilterFields="['column_id', 'column_title']" :loading="showLoadding" v-if="mdTable && mdTable.cols"
        :value="mdTable.cols" showGridlines class="p-datatable-sm h-full" scrollable scrollHeight="calc(100vh - 340px)"
        tableStyle="table-layout: fixed;">
        <template #header>
            <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                <i class="pi pi-database"></i>
                <span class="text-xl text-900 font-bold">Bảng</span>
                <Button class="m-0" size="small" :label="tb.stt.toString()" @click="goTableProc(tb)"
                    :severity="mdTable.stt == tb.stt ? 'info' : 'secondary'" rounded outlined v-for="tb in dtTables" />
                <div class="flex-1"></div>
                <span class="p-input-icon-left">
                    <i class="pi pi-search" />
                    <InputText @keyup.enter="searchCols()" v-model="filterconfigDatabases['global'].value"
