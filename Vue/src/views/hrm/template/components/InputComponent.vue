...
export default {
  ...
  setup(props) {
    ...
    const changeDiv = (ev, f) => {
      ...
      if (f == false) props.ipmodel[props.okey] = vl;
...
<template>
  <div
    v-if="props.div"
    @keyup="changeDiv($event, true)"
    @blur="changeDiv($event, false)"
    contenteditable="true"
    spellcheck="false"
    :class="textalign + ' w-full'"
  >
    {{ bindValue() }}
  </div>
  <div v-else class="w-full h-full ip-36">
    <InputText
      spellcheck="false"
      type="number"
      v-if="props.col.inputtype == 'Number'"
      v-model="props.ipmodel[props.okey]"
      class="w-full h-full ip36"
    />
    <InputNumber
      spellcheck="false"
      :useGrouping="true"
      v-if="props.col.inputtype == 'Currency'"
      v-model="props.ipmodel[props.okey]"
      class="w-full h-full ip36"
    />
    <Calendar
    ...
    <InputMask
      ...
      v-if="props.col.inputtype == 'DateTime'"
      v-model="props.ipmodel[props.okey]"
      class="w-full h-full ip36"
    />
    <Dropdown
      spellcheck="false"
      @change="changeDrForm($event, props.ipmodel)"
      :editable="true"
      v-if="
        (!props.col.inputtype || props.col.inputtype == 'Text') &&
        props.col.auto
      "
      v-model="props.ipmodel[props.okey]"
      optionLabel="label"
      :options="itemtypeInputs"
