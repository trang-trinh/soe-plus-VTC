...
import {
    uid, encr, getKeyByValue, decodeHTMLEntities, download, isObject, removeAccents,
    change_unsigned, getTagSelection, capitalizeFirstLetter, getRandomInt, getRandomArray, utilformatDate
} from "../../../../util/function.js";
import { getDataTemp } from "../../../../util/data.js";
import { draggableEle, wrap, parseHTML, parseHTMLText, colName, colNumber } from "../../../../util/htmleditor.js";
export default ({
    ...
    setup(props) {
        ...
        const isReportListView = ref(props.pars && Object.keys(props.pars).length == 0);
  ...
  headers: { Authorization: `Bearer ${store.getters.token}` },
        ...
        const itemButExports = isUrlReport.value || readonly.value ? [
            {
                label: 'In',
                icon: 'pi pi-print',
                command: () => {
                ...
                items: [
                                ...
                                command: () => {
                                    ...
                                    if (tagn == "tr") {
                                        ...
                                        tclone.querySelectorAll("span").forEach(el => {
                                            ...
                                            el.id = uid();
                                            el.className = el.id;
                                            spans.value.push({ element: "", id: el.id, version: 1, historys: [] });
                    ...
                    },
                    {
                        ...
                        command: () => {
                            const sel = iframeWindow.getSelection();
                            const range = sel.getRangeAt(0);
                            draggableEle(range.startContainer.querySelector("img"), iframeDoc);
                        }
                    },
                ]
            },
        ...
        let users = [
            { id: 'cong', name: 'Công', fname: "Hoàng Đức Công", severity: "success", img: "https://apiv2.soe.vn//Portals/Users/44311433_1860466920669366_6024507152440229888_21013107585291.jpg" },
            { id: 'chung', name: 'Chung', fname: "Nguyễn Thành Chung", severity: "info", img: "https://sfile.soe.vn//Portals/CBA146D4A77C43259CC8231885703B78/NhanSu/Thumb/%E1%BA%A3nh%20s%E1%BA%AFp%20up%20av21110210285375.jpg" },
        ]
        ...
        function addBlurSpan(e) {
            ...
            if (span) {
                ...
                if (!html || el.innerHTML.trim() != parseHTML(html)) {
                    ...
                    let vesion = {
                        version: hversion.value, user: user.value, element: el.outerHTML,
                        date: new Date()
                    }
                    ...
                    if (span.historys.length == 0) {
                        //Add gốc
                        span.historys[0] = {
                            version: hversion.value - 1, user: user.value, element: elementSpan.value.element,
                            date: new Date()
                        };
                    }
                    span.historys[hversion.value] = vesion;
                    reUsers.value.forEach(u => {
                        u.badge = spans.value.filter(x => x.historys.filter(h => h.user.id == u.id) > 0);
                    });
                }
        ...
        const eventDoc = () => {
            isdoc.value = dochtml.querySelector("title") != null && dochtml.querySelector("title").innerHTML != 'landscape';
            if (!isUrlReport.value && !readonly.value)
                dochtml.querySelectorAll("table").forEach(element => {
                    element.addEventListener("click", function (e) {
                        if (element.innerHTML.match(/{{[^}]*}}/igm)) {
                            tableElement = element;
                            let fdatas = e.target.className.replaceAll("for-data", "").split("-");
            ...
            let groups = [];
            dochtml.querySelectorAll(isxls.value ? 'td' : 'p>span').forEach((element) => {
                let hasedid = false;
                if (isxls.value) {
                    if (element.innerText.includes("data-")) {
                        let fdatas = element.innerText.split("|")[1].replaceAll("]", "").replaceAll("[", "");
                        ...
                        element.innerText = element.innerText.split("|[")[0];
                        let type = cname.includes("0;") ? 0 : cname.includes("1;") ? 1 : 2;
                        let etr = element;
                        if (type == 0) {
                ...
                if (element.className && element.className.match(/os-span-edit-os/)) {
                    ...
                    element.className = element.className.replace("os-span-edit-os", " os-span-edit-os");
                } else if (element.classList.length == 1 && !isxls.value) {
                    hasedid = true;
                    if (!element.id)
                        element.id = element.className.trim();
                }
                ...
                spans.value.push({ element: clone.outerHTML, id: osid, version: 1, historys: [] });
                if (!element.id) {
                    element.id = osid;
                    if (!hasedid)
                        element.classList.add(osid);
                }
                if (!isUrlReport.value && !readonly.value)
        ...
        const changeInputExcel = (xls) => {
            ...
            if (xls) {
                ...
                n1 = objExcel.value[textEdit.value.match(/{{[^{]+}}/g)[0]].match(/\d+/)[0];
                textEdit.value = textEditExcel.value.replaceAll(n2, n1).replace(/\w+\d+/g, function (vl) {
        ...
        const clickConfigData = (td) => {
            if (td.innerHTML.match(/{{[^}]*}}/igm)) {
                tableElement = td;
                if (td.innerText.includes("|")) {
                    let fdatas = td.innerText.split("|")[1].replaceAll("]", "").replaceAll("[", "");
                ...
                displayTable.value = true;
            } else if (td.innerHTML.match(/\[[^}]*\]/igm)) {
                tableElement = td;
                if (td.innerText.includes("|")) {
                    let fdatas = td.innerText.split("|")[1].replaceAll("]", "").replaceAll("[", "");
                    mdData.value = fdatas.replace("data-", "");
        ...
        function displayNumber(key, so) {
            if (parseInt(so) > 10000)
                return parseInt(so).toLocaleString();
            return so;
        }
        const addExcelStyle = () => {
            if (isxls.value) {
                ...
                dochtml.querySelectorAll("tbody>tr").forEach((tr, i) => {
                    ...
                    tr.querySelectorAll("td").forEach((td, j) => {
                        ...
                        if (td.getAttribute("colspan") > 1 && tr.nextElementSibling) {
                            ...
                            if (css.getPropertyValue("background-color") == "rgb(255, 192, 0)") {
                                for (let k = 0; k < td.getAttribute("colspan"); k++) {
                                    if (tr.nextElementSibling.children[stl]) {
                                        tr.nextElementSibling.children[stl].setAttribute("i", col + j + k - sttcol);
                                        tr.nextElementSibling.children[stl].setAttribute("tname", td.innerText);
                                    }
                                    stl += 1;
                                }
        ...
        const saveCK = () => {
            if (isComment.value) {
                ...
                el.classList.add("os-span-edit-os");
                el.innerHTML = spanEditor.value.replace(/<p>(.+)<\/p>/igm, '$1');
                elementSpan.value.version++;
                let vesion = {
                    version: elementSpan.value.version, user: user.value, element: el.outerHTML,
                    date: new Date()
                }
        ...
        const goKey = (item) => {
            if (elementSpan.value != null) {
                ...
                datakeys.value = move(datakeys.value, datakeys.value.indexOf(item), datakeys.value.length - 1);
            }
        }
        ...
        function highlight(cssText) {
            ...
            if (startContainer === endContainer) {
                ...
                if (startContainer.tagName) {
                    span = [...startContainer.childNodes].find(x => x.innerText == sel.toString());
                    if (span != null) {
        ...
        const openHTML = (html) => {
            html = (html || elementSpan.value.element).replaceAll("os-span-edit-os", "");
        ...
        function PrintDiv(divid, title, style) {
            ...
            contents = contents.replace(/<tr class="tablecell"[^</]*>(.*)<\/tr>/igm, "");
            contents = contents.replace(/<td class="tablecell"[^</]*>(\d+)<\/td>/igm, "");
            contents = contents.replace(/<col class="tablecell"[^</]*>/igm, "");
            ...
            frameDoc.document.write(`<html><head><title>${title}</title><style type="text/css">${style}</style>`);
        ...
        const printDiv = () => {
            ...
            if (!ifr) {
                ...
                divContents = divContents.replace(/<tr class="tablecell"[^</]*>(.*)<\/tr>/igm, "");
                divContents = divContents.replace(/<td class="tablecell"[^</]*>(\d+)<\/td>/igm, "");
                divContents = divContents.replace(/<col class="tablecell"[^</]*>/igm, "");
                var a = window.open('', '', 'height=500, width=500');
                a.document.write('<html><head><title>Lịch</title><style type="text/css">');
                ...
                PrintDiv("dochtml", filename, style);
        }
        const rmHTML = (html) => {
            ...
            if (readonly.value) {
                ...
                if (!oneRow.value) {
                    ...
                    dtDataReports.value.forEach(dr => {
                        Object.keys(dr).filter(x => x.includes("_")).forEach(el => {
                            objt[el] = dr[el];
                        });
                        if (dr.datatemp) {
                            dr.ok = true;
                ...
                let idx = objDataTemp.value[0].cols.findIndex(x => x.value == "[List nhân sự]");
                if (idx != -1) {
                    objDataTemp.value[0].cols[idx].key = objDataTemp.value[1].rows.map(x => x["[Ông/bà]"] + " " + x["[Tên nhân sự]"]).join(", ");
                }
                objDataTemp.value.forEach(r => {
                    let o = {};
                    r.cols.forEach(c => {
                        if (c.inputtype == "Date" && c.key instanceof Date && !isNaN(c.key)) {
        ...
        const isTypeInput = (vl, k) => {
            if (!isNaN(vl) && vl != "" && !vl.toString().startsWith("0") && k != "năm") {
                return 1;//Số
            } else if (vl && vl.toString().includes("/") && vl.toString().split("/").length == 3) {
                return 2;//Date
            }
            return 0;//Text
        }
        const arrayKeys = (r, r1) => {
            ...
            keys = keys.filter(x => !html.includes(`{{${x}}}=`) && !html.includes(`{{${x}}})=`));
            return keys;
        }
        ...
        const loadJsonTemplate = (text) => {
            ...
            reUsers.value.forEach(u => {
                u.badge = spans.value.filter(x => x.historys.filter(h => h.user.id == u.id).length > 0).length;
            });
        ...
        const myDocUploader = async (event) => {
            ...
            apimethod = 'PostFile';
            try {
                const response = await fetch(apiURL + "api/Files/" + apimethod, {
                    method: 'POST',
                    body: formData
                });
                    ...
                    if (trs[i].children.length >= cols.length) {
                        ...
                        cols.forEach((c, j) => {
                            if (trs[i].children[j + 1]){
                                ...
                                }else obj[c.value] = trs[i].children[j + 1].getAttribute("x:num") || trs[i].children[j + 1].innerText.trim();
                            }
                            if (obj[c.value] == "#REF!") {
                                obj[c.value] = null;
                            }
                ...
                dtDataReports.value.forEach(r => {
                    let rr = rows.find(x => x["HỌ VÀ TÊN"].trim().toUpperCase() == r["Họ tên"].split('<br/>')[0].trim().toUpperCase()
                        || (r["profile_code"] && (x["Mã NS"] == r["profile_code"] || x["Mã Nhân sự"] == r["profile_code"]))
                    );
                    if (rr) {
                        r.datatemp = rr;
                    }
        ...
        const initDataTempAuto = async (tf) => {
            ...
            if (!isUrlReport.value) {
                let dts = await goProc(true, `soe_user_info`, [{ par: "user_id", va: store.getters.user.user_id} ], false, true);
                dtUser = dts[0];
            }
            let stt = 0;
            ...
            dochtml.querySelectorAll('[style*="background-color:#ffff00"]').forEach((el, i) => {
                if (!el.closest("td") || el.closest("td").style.backgroundColor != 'rgb(255, 255, 0)') {
                    if (!(el.closest("p") && /\[.?\s*\[/g.test(el.closest("p").innerText))) {
                        if (obj.cols.findIndex(x => x.value == removeTags(el.innerText)) == -1) {
                            obj.cols.push({ "tid": stt, "stt": i + 1, "key": readonly.value ? '' : removeTags(el.innerText), "value": removeTags(el.innerText) });
                        }
                    }
                    else {
            ...
            tagps.forEach((tp, i) => {
                stt++;
                let obj = { "stt": i + 1, "key": stt, "value": "data-" + stt, "table": true, cols: [] };
                tp.querySelectorAll('[style*="background-color:#ffff00"]').forEach((el, j) => {
                    if (!el.closest("td") || el.closest("td").style.backgroundColor != 'rgb(255, 255, 0)') {
                        if (/\[.+\]/g.test(el.innerText) && obj.cols.findIndex(x => x.value == removeTags(el.innerText)) == -1) {
                            let k = removeTags(el.innerText);
                            k = k.replace(/\[.*\[/, "[").replace(/\].*\]/, "]").trim();
                            obj.cols.push({ "tid": stt, "stt": j + 1, "key": readonly.value ? '' : k, "value": k });
                        } else {
            ...
            dochtml.querySelectorAll('table').forEach((etb, i) => {
                if (etb.querySelector("td").style.backgroundColor == 'rgb(255, 255, 0)') {
                    stt++;
                    ...
                    etb.querySelectorAll('tr>td').forEach((td, j) => {
                        if (/\[.+\]/g.test(td.innerText))
                            obj.cols.push({ "tid": stt, "stt": j + 1, "key": readonly.value ? '' : removeTags(td.innerText), "value": removeTags(td.innerText) });
                    });
                    objDataTemp.value.push(obj);
                }
            });
            expandedRows.value = objDataTemp.value.filter(x => x.cols);
            if (tf != true && props.report.report_config && props.report.report_config.trim() != "") {
                try {
        ...
        const initTempAI = async () => {
            let stt = 0;
            spans.value.filter(x => (/\.*…+\.*…*\.*…*/igm).test(x.element) || (/\.{3,}/igm).test(x.element)).forEach(el => {
            ...
            spans.value = [];
            eventDoc();
            spans.value.filter(x => (/\.{3,}/g).test(x.element)).forEach(el => {
        ...
        .highlighted,.os-span-edit-os,.x,[class^=for-data],span.modified{background-color:#ff0!important}.history .os-span-edit-os,.os-span-edit-os.off{background-color:unset!important}tr,/*tr td{height:fit-content!important*/}.p-panel-content{overflow-x:auto}.text1line{overflow:hidden}.ul-form{margin:0;padding:0}.ul-form li{display:inline-block}.ul-form li .cols-2{margin-bottom:5px}[class^=for-data]{position:relative}[class^=for-data]:after{content:"\e94a";font-family:primeicons;position:absolute;top:-25px;right:0;cursor:pointer}.vc-color-wrap{width:24px!important;margin:0!important;height:20px!important}.history .os-span-edit-os{cursor:pointer}.history-row.cong{background-color:#f0f8ff}.history-row.hai{background-color:#faebd7}.history-row.duong{background-color:#eee}.history-row{vertical-align:middle;justify-content:center;align-items:center}.text1line{text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:1;line-clamp:1;-webkit-box-orient:vertical}td{vertical-align:middle!important;align-items:center}.vue-diff-viewer .vue-diff-row .hljs{word-break:break-word!important;padding:10px;font-family:arial}.code-diff-view .diff-table .blob-code .blob-code-inner{font-size:15px!important;font-family:arial!important}.code-diff-view .diff-table .blob-code{padding:10px}.doc-page{margin: auto;width:29.7cm;padding:10mm;display:block;page-break-before:always;background-color:#fff;position:relative}.p-dialog-content tr:hover td{background-color:#ff0}.div-excel{padding:0!important}.div-excel tr:hover td{background-color:#ff0!important;color:#000!important}.div-excel tr:hover td:hover{background-color:#f0f8ff!important;color:#000!important}.div-excel.line td{border:1px solid #999}.tablecell{text-align:center;background-color:#eee;border-bottom:1px solid #999}td.thcell{padding:5px;border-right:1px solid #999}.thcell,td.tablecell{background-color:#eee;position:sticky;z-index:999;top:0;left:0}#app-sidebar,#gtx-trans,#mobile-header{display:none!important}.true .p-contextmenu-root-list{max-height:400px;overflow-y:auto;padding:10px!important}[contenteditable]{outline:transparent solid 0}.os-delete-os{display:none}
            ...
            if(dochtml){
            tempHTMLGoc = dochtml.innerHTML;
            isdoc.value = dochtml.querySelector("table").offsetWidth <= dochtml.offsetWidth;
            if (!isdoc.value) {
        ...
        const openCogDatabase = (row, f, o) => {
            if (f) {
                if (row.isfor) {
                    ...
                    let rr = dtExcels.value.find(x => x.rowmnumber == row.rowmnumber && x.isfor);
                    if (rr) {
        ...
        const saveConfig = () => {
            props.callbackFun({ "report_config": JSON.stringify({ data: isxls.value ? dtExcels.value : objDataTemp.value, proc: proc }) });
            toast.success("Đã lưu cấu hình thành công!");
        }
        const nextDBrow = (f) => {
            let idx = dtTempCols.value.findIndex(x => x["value"] == dbrow.value["value"]);
        ...
        const downloadTemplateFile = async (f) => {
            ...
            if (dochtml.querySelector("title"))
                dochtml.querySelector("title").innerHTML = isdoc.value ? 'portrait' : 'landscape';
            let html = dochtml.innerHTML;
            ...
            try {
                const axResponse = await axios
                    .post(
                        apiURL + "api/Files/" + (isxls.value ? 'ConvertFileXLS' : 'ConvertFile'),
                        html,
                       config
                    );

                if (axResponse.status == 200) {
                    window.open(apiURL + "api/Files/downloadFile" + (isxls.value ? 'XLS' : '') + "?name=" + filename + (isxls.value ? '.xlsx' : ".docx"));
                    let sps = [];
                    spans.value.filter(x => x.historys.length > 0).forEach(sp => {
                        sps.push({ id: sp.id, version: sp.version, historys: sp.historys });
                    });
                    if (sps.length > 0)
        ...
        const downloadFile = async (f) => {
            ...
            let data = datausers.find(x => x["Họ và Tên"] == chonuser.fname) || datausers[0];
            //Table
            rerenderImg();
                    ...
                    if (dtclass) {
                        (data[dtclass] || data[ok.name] || []).forEach((r, i) => {
                            ...
                            Object.keys(r).forEach(k => {
                                ...
                                if (!dhtml.includes(`{{${k.trim()}}}=`) && !dhtml.includes(`{{${k.trim()}}})=`))
            ...
            if (isxls.value) {
                dochtml.querySelectorAll("td.os-span-edit-os").forEach(element => {
                    element.innerText = element.innerText + "{{" + element.className + "}}";
                });
            }
            dochtml.querySelectorAll("os-delete-os").forEach(element => {
                element.remove();
            ...
            html = html.replace(/(funXLS\[[^\[]+\])/g, function (su) {
                let fu = su.replace("funXLS", "").replaceAll(".", "").replaceAll("[", "").replaceAll("]", "");
                fu = decodeHTMLEntities(fu).replace(/(<([^>]+)>)/gi, "").replaceAll("”", "\"");
                try {
                    let vl;
                    if (fu.includes("=")) {
                        let ofu = (fu.split("=")[0]).replaceAll("{{", "").replaceAll("}}", "");
                        ...
                        if (!vl) {
                            ...
                            if (fu.includes("{{")) {
                                fu.match(/{{[^}]*}}/g).forEach(kk => {
                                    fu = fu.replaceAll(kk, objFun[kk.replaceAll("{{", "").replaceAll("}}", "")] || 0);
                                });
                                //console.log(fu);
                                vl = eval(fu);
                                if (vl && !isNaN(vl))
            ...
            html = html.replace(/<div style="float:right;?">(.+)<\/div>/igm, "$1");
            html = html.replaceAll("{{}}", "");
            html = html.replaceAll("os-span-edit-os", "");
            html = html.replaceAll("os-delete-os", "");
            html = html.replaceAll('style="-aw-import:ignore"', "");
            if (isxls.value) {
                html = html.replace("<style>", `<style>tr, td {height: auto!important;}`);
            }
            dochtml.innerHTML = html;
            //Sum-data
                    ...
                    div.querySelectorAll("td").forEach((td, i) => {
                        if (td.innerText.trim() == "0") {
                            ...
                            while (tr && !tr.className.includes("sum-data-")) {
                                let vl = tr.children[i].innerText.replaceAll(".", "").replaceAll(",", "");
                                if (vl && !isNaN(vl))
                                    sumvl += parseInt(vl);
            ...
            //
            html = html.replace(/title="[^"]+"/igm, "");
            html = html.replace(/<tr class="tablecell"[^</]*>(.*)<\/tr>/igm, "");
            html = html.replace(/<td class="tablecell"[^</]*>(\d+)<\/td>/igm, "");
            html = html.replace(/<col class="tablecell"[^</]*>/igm, "");
            showLoadding.value = true;
            try {
                const axResponse = await axios
                    .post(
                        apiURL + "api/Files/" + (isxls.value ? 'ConvertFileXLSX' : 'ConvertFile'),
                        html,
                        config
                    );
        ...
        function randomData() {
            ...
            let tens = ["Trang", "Thu", "Dung", "Hương", "Hạnh", "Thuỷ", "Chung", "Long", "Thái", "Thắng", "Tùng", "Nguyệt"];
            let arrs = [];
            let stt = getRandomInt(25, 50);
            for (let i = 0; i <= stt; i++) {
                let obj = {
                    "STT": i + 1,
                    "Họ tên": getRandomArray(hos) + " " + getRandomArray(dems) + " " + getRandomArray(tens),
                ...
                let luong = (obj["Lương chính"] + obj["Trách nhiệm"] + obj["Ăn trưa"] + obj["Xăng xe"])
                    * (obj["Ngày công"] / 26);
                obj["Thuế TNCN"] = luong > 30000000 ? luong * 20 / 100 : luong > 15000000 ? luong * 10 / 100 : 0;
                luong = luong - obj["Thuế TNCN"] - Math.ceil(obj["Lương đóng bảo hiểm"] * 10.5 / 100);
                obj["Tạm ứng"] = getRandomInt(1, 100) * luong / 100;
                if (obj["Tạm ứng"] > luong)
                    obj["Tạm ứng"] = luong;
                arrs.push(obj);
            }
            return arrs;
        ...
        const callbackFunChild = (dt, pr) => {
            ...
            if (isxls.value) {
                let ir = dtExcels.value.findIndex(x => x.rowmnumber == dbrow.value.rowmnumber);
                if (ir != -1) {
        ...
        const initReportData = async (cg) => {
            ...
            dtDataReports.value = await goProc(objConfig.proc.issql, objConfig.proc.sql, []);
        ...
        const rfReportData = async () => {
            ...
            if (tselect)
                tselect.classList.remove("selected");
            dochtml.innerHTML = tempHTMLGoc;
    ...
    config
        ...
        const viewReport = async () => {
            if (isxls.value && dtExcels.value.length > 0) {
                ...
                dtExcels.value.filter(x => x.isfor).forEach(dtr => {
                    let tr = dochtml.querySelector("tbody>tr:nth-child(" + (dtr.rowmnumber + 1) + ")");
                    if (tr) {
                            ...
                            trclone.querySelectorAll("td").forEach(td => {
                                if (td.className.includes("tablecell")) {
                                    ...
                                    if (td.innerText == 1) {
                                        ...
                                        let ok = dtr.cols.find(x => x.colname == td.getAttribute("title"));
                                        if (ok) {
                                            td.innerText = r[ok.key] || "";
                                        }
                                    }
                                    td.setAttribute("title", td.getAttribute("title").replace(dtr.rowmnumber, dtr.rowmnumber + i));
                                }
                            });
                ...
                if (readonly.value) {
                    ...
                    downloadFile(!isUrlReport.value);
            }
        }
        ...
        async function renderTableWord(objpar) {
            ...
            datausers = dts;
            dochtml.innerHTML = tempHTMLGoc;
            dochtml.querySelectorAll('[style*="background-color:#ffff00"]').forEach((el) => {
                if (!(el.closest("p") && el.closest("p").id && el.closest("p").id.includes("data"))) {
                    let pa = el.parentElement;
                    el.outerHTML = el.outerHTML.replace(/<[^<]+background-color:#ffff00[^>]+>([^<]+)<\/[^>]+>/igm, function (s, ke) {
                        ...
                        try {
                            if (k.toString().startsWith("=")) {
                                ...
                                k = dts[0][0][k] || dts[0][0][k.replace("[", "").replace("]", "")];
                            if (k == null) k = "";
                        } catch (e) {
                            console.log(e);
                        }
                        ...
                        if (ke == ke.toLowerCase()) {
                            ...
                            vl = vl.toUpperCase();
                        } else if (ke.substring(0, 1) == ke.substring(0, 1).toUpperCase()) {
                            vl = capitalizeFirstLetter(vl);
                        }
                        return s.replaceAll(ke, vl).replaceAll('background-color:#ffff00', '');
                    });
            ...
            dochtml.querySelectorAll("table[class*=data-]").forEach(tb => {
                ...
                let doc = (/\[.+\]/g).test(tb.querySelector("tr").querySelector("td:last-child").innerText);
                let otb = objDataTemp.value.find(x => x.value == tb.className.replace("table ", ""));
                if (otb) {
                    ...
                    if (!doc) {
                        ...
                        tb.querySelectorAll("tr").forEach(tr => {
                            ...
                            data.forEach((r) => {
                                ...
                                let ok = otb.cols.find(x => x.value == tdclone.innerText.trim());
                                if (ok) {
                                    td.style.backgroundColor = "";
                                    tdclone.innerHTML = td.innerHTML.replace(tdclone.innerText.trim(), r[ok.key] || "");
                                }
            ...
            dochtml.querySelectorAll("p[id*=data-]").forEach(tb => {
                ...
                data.rows.forEach(r => {
                    ...
                    let phtml = tb.innerHTML.replace(/\[[^\[\]]+\]/igm, function (su) {
                        return r[su.trim()] || "";
                    });
                    phtml = phtml.replaceAll("[", "").replaceAll("]", "").replaceAll("background-color:#ffff00", "");
                    p.innerHTML = phtml;
        ...
        const bindValueExcel = (tdtemp, td, fun, isfun) => {
            ...
            let vl = fun.replace(/(\[[^\]]+\])/igm, function (su) {
        ...
        const renderDataExcel = () => {
            ...
            dochtml.querySelectorAll("td").forEach(td => {
                if (/\[(.+)\]/igm.test(td.innerText)) {
                    var css = window.getComputedStyle(td);
                    if (css.getPropertyValue("background-color") == "rgb(255, 255, 0)") {

                        bindValueExcel(tdtemp, td, td.innerText, false);
                    } else if (css.getPropertyValue("background-color") == "rgb(255, 192, 0)") {
                        td.innerText = dtUser[td.innerText] || (dtProfile && dtProfile[trimTagValue(td.innerText)]) || td.innerText;
                ...
                if (td.getAttribute("x:fmla")) {//Nếu có công thức
        ...
        const initdbXLS = () => {
            ...
            var ths = [...dochtml.querySelectorAll("td")].filter(x => window.getComputedStyle(x).getPropertyValue("background-color") == "rgb(255, 192, 0)");
            dochtml.querySelectorAll("td").forEach((td, i) => {
                if (/\[(.+)\]/igm.test(td.innerText)) {
                    ...
                    if (css.getPropertyValue("background-color") == "rgb(255, 255, 0)" ||
                        ...
                        let mts = td.innerText.match(/\[[^\]]+\]/igm);
                        let iauto = css.getPropertyValue("background-color") == "rgb(255, 192, 0)";
                        if (iauto) {
                    ...
                    if (css.getPropertyValue("background-color") == "rgb(255, 255, 0)") {
                        ...
                        let th = ths.find(x => x.getAttribute("title").replace(/\d/g, '') == xn && !x.getAttribute("colspan"));
                        if (th) {
                            let tn = th.innerText;
            ...
            dtExcels.value.filter(x => !x.isfor && !x.auto).forEach(ex => {
                obj.cols.push({
                    key: '',
                    value: ex.key,
            ...
            objDataTemp.value.push(obj);
            dtExcels.value.filter(x => x.isfor && !x.auto).forEach((ex, i) => {
                obj = {
                    key: i + 1,
                    cols: [],
                    inputtype: ex.inputtype,
        ...
        const addRowXls = (td, el, iauto, f) => {
            ...
            if (obj.isfor) {
                ...
                obj.colname = `${td.children[1].getAttribute("title")}${td.children[td.children.length - 1].getAttribute("title")}`;
                obj.cols = [];
                ...
                let i = dtExcels.value.findIndex(x => x.colname == obj.colname && x.key == obj.key);
                // if (ir != -1) {
        ...
        //Compute
        const compuDatakeys = computed(() => mapdatakeys.filter(x => Array.isArray(datausers[0][x.name]) || isObject((datausers[0][x.name]))));
        const compSpans = computed(() => spans.value.filter(x => (!filterUser.value.id && x.historys.length > 0) || x.historys.filter(u => u.user.id == filterUser.value.id).length > 0));
        const compXLS = computed(() => {
            let ars = Object.keys(objExcel.value).filter(x => objExcel.value[x].includes(txtIPXLS.value.toUpperCase()) || objExcel.value[x].includes(txtIPXLS.value) || x.includes(txtIPXLS.value) || x.toLowerCase().includes(txtIPXLS.value));
            ars = ars.sort((a, b) => {
                return objExcel.value[a].substring(0, 1) - objExcel.value[b].substring(0, 1);
            });
            return ars;
        });
        ...
        const initProfile = async (r) => {
            let dts = await goProc(true, `profile_info '${store.getters.user.user_id}', '${r.profile_id}'`, [], false, true);
            if (dts.length > 0) {
                dtProfile = dts[0];
            }
        }
                ...
                if (tbs) {
                    objDataTemp.value.forEach((ot, i) => {
                        ...
                        ot.cols.forEach((co, i) => {
                            dt.datatemp[co.value] = t ? t[co.value] : (objvalue[co.value.replace(/\[(.+)\]/g, "$1")]
                                || tb[co.value.replace(/\[(.+)\]/g, "$1")] || null);
                        });
                    });
                }
            });
        ...
        const renderTypeInput = (co) => {
            let vl = co.key;
            if(co.value.toLowerCase().includes("tiền")
        ...
        const editForm = async (r, rcopy) => {
            ...
            isdataSidebar.value = true;
            dts = await goProc(true, `profile_info '${store.getters.user.user_id}', '${r.profile_id}'`, [], true, true);
            if (dts.length > 0) {
                dtProfile = dts[0][0]
            }
            let objvalue = {};
            ...
            objDataTemp.value.forEach((ot, i) => {
                ...
                ot.cols.forEach(co => {
                    co.key = tb ? tb[co.value] : objvalue[co.value.replace(/\[(.+)\]/g, "$1")];
                    if (!co.inputtype) {
...
<template>
    <div :class="'flex mb-' + (isUrlReport ? 0 : 1)">
        <div class="tool flex-1 flex" v-if="isHasHTML && !isUrlReport && !readonly">
            <Button v-tooltip="'Chữ thường'" @click="execStyle('N')" label="Normal"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Chữ đậm'" @click="execStyle('B')" label="B"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Chữ nghiêng'" @click="execStyle('I')" label="I"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Chữ gạch chân'" @click="execStyle('U')" label="U"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Viết hoa'" @click="execStyle('AA')" label="AA"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Viết hoa chữ đầu'" @click="execStyle('Aa')" label="Aa"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Viết thường'" @click="execStyle('aa')" label="aa"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Chọn màu chữ'" class="p-button-secondary p-button-outlined ml-1">
                <colorpickers style="width:24px" v-model:gradientColor="gradientColor" pickerType="chrome"
                    @pureColorChange="execStyle" v-model:pureColor="pureColor" />
            </Button>
            <Button v-if="!isViewReport" @click="clearSelect" icon="pi pi-refresh"
            ...
            <ToggleButton v-if="!isViewReport && isedit" v-model="isdoc" onLabel="" offLabel="" onIcon="pi pi-arrows-v"
                offIcon="pi pi-arrows-h" class="ml-1" />
            ...
            <Button v-if="!isViewReport" v-tooltip="'Dữ liệu'" @click="showDataSidebar" icon="pi pi-cog"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button @click="toogledivZoom(false)" icon="pi pi-search-minus"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button @click="divZoom = 1" :label="parseInt(divZoom * 100) + '%'"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button @click="toogledivZoom(true)" icon="pi pi-search-plus"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-tooltip="'Full'" @click="isfull = !isfull"
                :icon="'pi pi-window-' + (isfull ? 'minimize' : 'maximize')"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-if="isxls" v-tooltip="'Hiển thị line Excel'" @click="toolCellLine" icon="pi pi-check-square"
                class="p-button-secondary p-button-outlined ml-1" />
            <ToggleButton v-model="isViewReport" onIcon="pi pi-pencil" offLabel="" onLabel="" offIcon="pi pi-eye"
                class="ml-1 mr-2" />
        </div>
        <div class="tool flex w-full p-2" v-if="isUrlReport || readonly"
            :style="'height:' + (isUrlReport || readonly ? '40pt' : '')">
            <Button v-if="!readonly" v-tooltip="'Quay lại trang trước'" @click="goBack()" icon="pi pi-angle-left"
                class="p-button-secondary p-button-outlined mr-1" />
            <div class="flex-1">
                <h3 v-if="!readonly" class="p-2 m-0"><i class="mr-1 pi pi-print"></i>{{ report.report_name }}</h3>
            </div>
            <ToggleButton v-if="isedit" v-model="isdoc" onLabel="" offLabel="" onIcon="pi pi-arrows-v"
                offIcon="pi pi-arrows-h" class="ml-1" />
            <Button @click="toogledivZoom(false)" icon="pi pi-search-minus"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button @click="divZoom = 1" :label="parseInt(divZoom * 100) + '%'"
            ...
            <Button @click="toogledivZoom(true)" icon="pi pi-search-plus"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-if="isxls" v-tooltip="'Full'" @click="isfull = !isfull"
                :icon="'pi pi-window-' + (isfull ? 'minimize' : 'maximize')"
                class="p-button-secondary p-button-outlined ml-1" />
            <Button v-if="isxls" v-tooltip="'Hiển thị line Excel'" @click="toolCellLine" icon="pi pi-check-square"
                class="p-button-secondary p-button-outlined ml-1" />
            <SplitButton class="ml-1" @click="viewReport()" icon="pi pi-download" label="" :model="itemButExports" />
        </div>
        <SplitButton v-if="!isUrlReport && !readonly" @click="viewReport()" icon="pi pi-eye" label="Xem"
            :model="itemButExports" />
        <ProgressSpinner v-if="showLoadding" style="width:50px;height:50px" strokeWidth="8" fill="var(--surface-ground)"
    ...
    <div class="flex" style="background-color: #eee;">
        <iframe id="docIframe" src="about:blank" frameborder="0"
            :style="'background:#ccc;width: 100%;height:calc(100vh - ' + (isUrlReport ? 60 : 130) + 'px)'"></iframe>
        <div id="divleft-frame" :class="isxls ? 'div-excel' : ''"
            style="overflow-y: auto;max-height: calc(100% - 40px);padding: 20px;background-color: #ccc;">
            <div class="doc-page card shadow-1"
                :style="'zoom:' + divZoom + ';padding:' + (isxls ? '0' : '') + ';width:' + (isxls ? 'fit-content;min-width:21cm' : (isdoc ? '21cm' : '29.7cm'))">
                <div :contenteditable="!isUrlReport && !readonly" spellcheck="false" id="dochtml"
                    @contextmenu="onRightClick">
                </div>
            </div>
        </div>
        <!--|| !readonly || dtDataReports.length > 0-->
        <div class="flex-1 bg-white" v-if="(!isUrlReport || isReportListView) && isHasHTML && !isfull && isedit"
            :style="'min-width:' + ((readonly || isReportListView) ? 500 : 640) + 'px;'">
            <div v-if="isViewReport">
                <DataTable v-model:filters="rpfilters" @rowSelect="onRowSelectReport" selectionMode="single"
                    :metaKeySelection="false" scrollable :scrollHeight="'calc(100vh - ' + (isUrlReport ? 120 : 190) + 'px)'"
                    class="p-datatable-sm" v-model:expandedRowGroups="expandedRowGroups" expandableRowGroups
                    rowGroupMode="subheader" groupRowsBy="group" showGridlines :value="dtDataReports">
                    ...
                    <template #header v-if="!onedata">
                        <div class="flex justify-content-end">
                            <span class="p-input-icon-left">
                                ...
                                <InputText v-model="rpfilters['global'].value" placeholder="Tìm kiếm" />
                            </span>
                            <Button class="ml-1" :loading="showLoadding" @click="rfReportData()" icon="pi pi-refresh"
                                severity="secondary" outlined />
                        </div>
                    </template>
                    <Column :style="'width:' + ((col.includes('Ảnh') || col == 'ok') ? '50pt' : 'auto')"
                        v-for="col of dtColumns" :key="col" :field="col"
                        :header="(col != 'ok' && !col.includes('_')) ? col : ''">
                        <template #body="dt">
                            <div v-if="col.includes('Ảnh')">
                                <Avatar v-if="(dt.data[col] || '').includes('.')" :image="dt.data[col]" size="large"
                                    shape="circle" />
                                <Avatar v-else :label="dt.data[col]" size="large" :style="{
                            ...
                            <div v-if="!col.includes('_') && !col.includes('Ảnh')">
                                <div v-if="col == 'ok'" style="text-align: center;font-size: 200%;">
                    ...
                    <Column class="text-center" v-if="readonly && isedit" style="width: 50px;">
                        <template #header>
                            <Button v-if="cForm && !onedata" size="small" @click="copyRow()"
                                :icon="'pi pi-' + (isCopy ? 'check' : 'copy')" v-tooltip.left="isCopy ? 'Dán' : 'Copy'" text
                                outlined />
                            <Button v-if="!onedata" size="small" @click="editForm()" icon="pi pi-pencil"
                                severity="secondary" rounded outlined text class="mr-1" />
                        </template>
                        <template #body="slotProps">
                            <Button size="small" @click="editForm(slotProps.data)" icon="pi pi-pencil" severity="secondary"
                                rounded outlined class="mr-1" />
                        </template>
                    </Column>
                </DataTable>
            </div>
            <div v-else style="height: calc(100vh - 130px);overflow-y: auto;">
                <div class="p-0">
                    ...
                    <div class="flex m-2">
                        <div class="flex-1 mr-2">
                            <span class="p-input-icon-left w-full">
                                <i class="pi pi-search" />
                                <InputText class="w-full" v-model="rpfilters['global'].value" placeholder="Tìm kiếm" />
                            </span>
                        </div>
                        <Button @click="saveConfig()" icon="pi pi-save" outlined class="mr-1" />
                        <!-- <Button @click="addCogDatabase()" outlined class="mr-1" icon="pi pi-cog" severity="secondary" /> -->
                        <Button @click="refershConfig()" icon="pi pi-refresh" severity="secondary" outlined />
                    </div>
                    <DataTable v-model:expandedRows="expandedRows" :rowClass="rowClass"
                        v-if="!isxls && objDataTemp.length > 0" scrollable scrollHeight="calc(100vh - 200px)"
                        class="p-datatable-sm" v-model:filters="rpfilters" showGridlines :value="objDataTemp">
                        <Column expander style="width: 50px" />
                        <Column class="text-center" style="width: 50px" header="STT">
                            <template #body="slotProps">
                                {{ slotProps.index + 1 }}
                            </template>
                        </Column>
                        <Column field="value" header="Template"></Column>
                        <Column field="key" header="Data">
                            <template #body="slotProps">
                                <b
                                    :style='"background-color:" + (slotProps.data.isconfig && !slotProps.data.cols ? "#ffff00" : "")'>{{
                                        slotProps.data.key
                                    }}</b>
                            </template>
                        </Column>
                        <Column style="width: 100px;text-align: center;">
                            <template #body="slotProps">
                                <Button @click="openCogDatabase(slotProps.data)" icon="pi pi-cog" severity="secondary"
                                    outlined class="mr-1" style="width: 36px;height:36px;" />
                                <Button @click="delRowXLS(slotProps.data)" icon="pi pi-trash" severity="danger" outlined
                                    style="width: 36px;height:36px;" />
                            </template>
                        </Column>
                        <template #expansion="row">
                            <div class="pt-3 pb-3" v-if="row.data.cols">
                                <h3 class="p-0 m-0 mb-2">Cột</h3>
                                <DataTable class="p-datatable-sm" showGridlines :value="row.data.cols">
                                    <Column headerStyle="text-align: center;" field="value" header="Template">
                                    </Column>
                                    <Column field="key" header="Data"></Column>
                                    <Column>
                                        <template #body="slotProps">
                                            <Checkbox v-tooltip="'Lấy tự động'" v-model="slotProps.data.auto"
                                                :binary="true" />
                                        </template>
                                    </Column>
                                    <Column field="inputtype" header="Nhập">
                                        <template #body="slotProps">
                                            <Dropdown @change="changeInputType(slotProps.data)" v-if="!slotProps.data.auto"
                                                v-model="slotProps.data.inputtype" optionLabel="lable" optionValue="value"
                                                dropdownIcon="" :options="isTypeInputs" class="w-full no-icon">
                                                <template #option="slotProps">
                                                    <div><i :class="slotProps.option.icon"></i> {{ slotProps.option.lable }}
                                                    </div>
                                                </template>
                                            </Dropdown>
                                        </template>
                                    </Column>
                                    <Column style="width: 100px;text-align: center;" headerClass="text-center"
                                        class="text-center">
                                        <template #body="slotProps">
                                            <Button @click="openCogDatabase(slotProps.data, true, true)" icon="pi pi-cog"
                                                severity="secondary" outlined style="width: 36px;height:36px" />
                                            <Button @click="delRowXLS(row, slotProps.data)" icon="pi pi-trash"
                    ...
                    <DataTable :rowClass="rowClass" v-model:expandedRows="expandedRows" v-if="isxls && dtExcels.length > 0"
                        scrollable scrollHeight="calc(100vh - 200px)" class="p-datatable-sm" showGridlines
                        :value="dtExcels">
                        <Column expander />
                        <Column style="width: 100px;text-align: center;" headerClass="text-center" class="text-center"
                            field="colname" header="Dòng/Cột">
                        </Column>
                        <Column field="key" header="Trường"></Column>
                        <Column>
                            <template #body="slotProps">
                                <Checkbox v-tooltip="'Lấy tự động'" v-model="slotProps.data.auto" :binary="true" />
                            </template>
                        </Column>
                        <Column field="inputtype" header="Nhập">
                            <template #body="slotProps">
                        ...
                        <Column style="width: 140px;text-align: center;" headerClass="text-center" class="text-center">
                            <template #body="slotProps">
                                <Button @click="openCogDatabase(slotProps.data, true, true)" icon="pi pi-cog"
                                    severity="secondary" outlined />
                                <Button class="ml-1" @click="delRowXLS(slotProps.data)" icon="pi pi-trash" severity="danger"
                                    outlined />
                            </template>
                        </Column>
                        <template #expansion="slotProps">
                            <div class="p-3" v-if="slotProps.data.cols.length > 0">
                                ...
                                <DataTable scrollable scrollHeight="300px" class="p-datatable-sm" showGridlines
                                    :value="slotProps.data.cols">
                                    <Column headerStyle="width: 50px;text-align: center;" field="colname" header="Cột">
                                    </Column>
                                    ...
                                    <Column style="width: 140px;text-align: center;" headerClass="text-center"
                                        class="text-center">
                                        <template #body="slotProps">
                                            <Button @click="openCogDatabase(slotProps.data, true, true)" icon="pi pi-cog"
                                                severity="secondary" outlined />
                                            <Button class="ml-1" @click="delRowXLS(slotProps.data)" icon="pi pi-trash"
                                                severity="danger" outlined />
                                        </template>
                                    </Column>
                                </DataTable>
    ...
    <Dialog v-if="!readonly" :header="isComment ? 'Bình luận' : 'Chỉnh sửa nâng cao'" v-model:visible="displayCK"
        :breakpoints="{ '960px': '75vw', '640px': '90vw' }" :style="{ width: '50vw' }" :maximizable="true" :modal="true">
        <Editor v-model="spanEditor" editorStyle="height: 320px" />
        <template #footer>
            <Button label="Không" icon="pi pi-times" @click="displayCK = false" class="p-button-text" />
            <Button label="Lưu lại" icon="pi pi-save" @click="saveCK()" autofocus />
        </template>
    </Dialog>
    <Dialog v-if="!readonly" header="Thông tin chỉnh sửa" v-model:visible="displayElement"
        :breakpoints="{ '960px': '75vw', '640px': '90vw' }" :style="{ width: '50vw' }" :maximizable="true" :modal="true">
        <div class="flex">
            <Dropdown v-model="drcurrentText" class="w-full flex-1" optionLabel="version" :options="elementSpan.historys"
                :filter="true" placeholder="So sánh với gốc">
                <template #value="slotProps">
                    <div v-if="slotProps.value.element">
                        <Tag class="mr-2" :value="'Version ' + slotProps.value.version"></Tag>
                        <Tag class="mr-2" severity="success" :value="slotProps.value.user.fname"></Tag>
                    </div>
                    <span v-else>
                ...
                <template #option="slotProps">
                    <div @click="setCurrentText(slotProps.option)">
                        <p>
                            <Tag class="mr-2" :value="'Version ' + slotProps.option.version"></Tag>
                            <Tag class="mr-2" severity="success" :value="slotProps.option.user.fname"></Tag>
                        </p>
                        <p class="mt-2">
            ...
            <Button v-if="drcurrentText.element" class="ml-2" icon="pi pi-refresh" @click="setCurrentText()"
                label="So sánh gốc" />
        </div>
        <TabView>
            <TabPanel header="Xem chỉnh sửa 1">
                <div class="p-2" style="background-color: #ccc;">
                    <Card>
                        <template #content>
                            <code-diff :old-string="removeTags(currentText)" :new-string="removeTags(htmlElement)"
                                maxHeight="calc(100vh - 250px)" outputFormat="side-by-side" />
                        </template>
                    </Card>
                </div>
            </TabPanel>
            <TabPanel header="Xem chỉnh sửa 2">
                <div class="p-2" style="background-color: #ccc;">
                    <Card>
                        <template #content>
                            <Diff mode="split" theme="light" language="plaintext" :prev="removeTags(currentText)"
                                :current="removeTags(htmlElement)" :virtual-scroll="{ height: 300 }" />
                        </template>
                    </Card>
                </div>
            </TabPanel>
        ...
        <template #footer>
            <Button label="Không" icon="pi pi-times" @click="displayElement = false" class="p-button-text" />
            <Button label="Lưu lại" icon="pi pi-save" @click="displayElement = false" autofocus />
        </template>
    </Dialog>
    <Dialog v-if="!readonly" header="Thiết lập bảng" v-model:visible="displayTable" :style="{ width: '360px' }"
        :modal="true">
        <p class="mb-2 mt-2"><b>Tiêu đề bảng</b></p>
        <Dropdown v-model="mdDataHeader" :editable="true" class="w-full" optionValue="id" optionLabel="name"
            :options="compuDatakeys" :filter="true" placeholder="Trường dữ liệu bảng">
            <template #option="slotProps">
                <div>{{ slotProps.option.name }}</div>
            </template>
        </Dropdown>
        <p class="mb-2 mt-2"><b>Dữ liệu bảng</b></p>
        <Dropdown v-model="mdData" :editable="true" class="w-full" optionValue="id" optionLabel="name"
            :options="compuDatakeys" :filter="true" placeholder="Trường dữ liệu bảng">
            <template #option="slotProps">
                <div>{{ slotProps.option.name }}</div>
            </template>
        ...
        <template #footer>
            <Button label="Không" icon="pi pi-times" @click="displayTable = false" class="p-button-text" />
            <Button label="Lưu lại" icon="pi pi-save" @click="saveConfigTable()" autofocus />
        </template>
    </Dialog>
    <Dialog v-if="!readonly" header="Chỉnh sửa nội dung" v-model:visible="displayEditData" :style="{ width: '640px' }"
        :modal="true">
        <p v-if="textEditExcel != ''"><b>Excel</b></p>
        <p v-if="textEditExcel != ''">
            <span class="p-input-icon-left w-full">
                <i class="pi pi-fx" style="color:green;font-weight: bold;top: 18px;font-size: 150%;">fx</i>
                <Textarea @blur="changeInputExcel(true)" style="color: red;font-size: 120%;" class="w-full"
                    v-model="textEditExcel" id="textEdit" autoResize rows="1" />
            </span>
        </p>
        <Textarea style="font-size: 120%;" @blur="changeInputExcel(false)" class="w-full" v-model="textEdit" id="textEdit"
            autoResize rows="1" />
        <div class="flex" style="margin-top: 10px;padding: 5px;align-items: center;background-color: #eee;color: #fff;">
            <span class="p-input-icon-right p-inputtext-sm flex-1" style="font-size: 120%;">
                <i class="pi pi-search" />
                <InputText class="w-full" v-model="txtIPXLS" placeholder="Tìm kiếm giá trị cột" />
            </span>
        </div>
        <div style="max-height: 320px;overflow-y: auto;">
            <div class="grid m-2">
                <div style="border: .5px solid #ccc;" class="col-6" v-for="k in compXLS">
                    <b><span style="padding: 5px;font-weight: bold;color: blue;padding:5px">{{ objExcel[k] }}</span></b>
                    <b>{{ k }}</b>
                </div>
            </div>
        </div>
        <template #footer>
            <Divider />
            <Button label="Không" icon="pi pi-times" @click="displayEditData = false" class="p-button-text" />
            <Button label="Lưu lại" icon="pi pi-save" @click="saveEditData()" autofocus />
        </template>
    </Dialog>
    <Dialog v-if="!readonly" :header="'Thông tin cấu hình nguồn dữ liệu ' + dbrow.value" v-model:visible="isDisplayDatabase"
        :breakpoints="{ '960px': '75vw', '640px': '90vw' }" :style="{ width: '1024px' }" :maximizable="true" :modal="true">
        <ConfigDatabaseComponent :group="dbrow" ref="configDBChild" :xls="isxls" :one="IsOne"
            :callbackFun="callbackFunChild" :report="report" />
        <template #footer>
            <div class="flex">
                <Button v-if="(dtTempCols || objDataTemp).length > 0" icon="pi pi-angle-left" class="mr-1"
                    @click="nextDBrow(false)" severity="secondary" text />
                <Dropdown v-if="(dtTempCols || objDataTemp).length > 0" v-model="dbrow" optionLabel="value"
                    :options="dtTempCols || objDataTemp" :filter="true" placeholder="Chọn key">
                </Dropdown>
                <Button v-if="(dtTempCols || objDataTemp).length > 0" icon="pi pi-angle-right" class="ml-1"
                    @click="nextDBrow(true)" severity="secondary" text />
                <div class="flex-1"></div>
                <Button label="Không" icon="pi pi-times" @click="isDisplayDatabase = false" class="p-button-text" />
                <Button :disabled="showLoadding" :loading="showLoadding" label="Lưu lại" icon="pi pi-save"
                    @click="saveDatabase()" />
            </div>
        </template>
    </Dialog>
    <ContextMenu :class="isDesingData" v-if="isDesingData && !readonly" ref="menu" :model="datakeys">
        <template #item="{ item }">
            <Button @click="goKey(item)" :label="item" class="mb-1 w-full" />
        </template>
    </ContextMenu>
    <ContextMenu v-if="!isDesingData && !isUrlReport && !readonly" ref="menu" :model="actionMenus"></ContextMenu>
    <Sidebar v-model:visible="isdataSidebar" position="right" :class="'w-full' + (isfullSidebar ? '' : ' md:w-8 lg:w-8')">
        <template #header>
            <div class="flex">
                <Button @click="isfullSidebar = !isfullSidebar"
                    :icon="'pi pi-window-' + (!isfullSidebar ? 'maximize' : 'minimize')" text outlined
                    class="p-button-outlined p-button-secondary" />
                <div class="text-center flex-1">
                    <SelectButton v-if="!readonly" v-model="opTypeDB" :options="optionTypeDBs" optionLabel="name" />
                    <h2 class="p-0 m-0" v-else>Cập nhật thông tin
        ...
        <div class="flex w-full p-2 text-end">
            <div class="flex-1"></div>
            <Button @click="refersMapData" icon="pi pi-refresh" class="p-button-outlined p-button-secondary mr-1" />
            <Button v-if="opTypeDB.value == 1" @click="addTable()" icon="pi pi-plus"
                class="p-button-outlined p-button-secondary mr-1" />
            <FileUpload chooseLabel="Import Excel" class="mr-1" mode="basic" :auto="true" :customUpload="true"
                @uploader="importJsonData" name="doc[]" accept=".xls,.xlsx" />
            <Button v-if="opTypeDB.value == 2" severity="secondary" outlined class="mr-1" type="button" label="Tải"
                icon="pi pi-download" @click="downloadJsonData" />
            <Button type="button" label="Lưu" icon="pi pi-save" @click="saveDatamap(false)" />
        </div>
        <div class="p-1">
            <Accordion v-if="opTypeDB.value == 2" :multiple="false" :activeIndex="0">
                <AccordionTab v-for="(r, index) in datamaps">
                    ...
                    <ul class="ul-form">
                        <li :class="'m-1 grids' + (Array.isArray(r[r1]) ? ' w-full' : '')" v-for="r1 in Object.keys(r)"
                            :style="'width:' + (r[r1] && r[r1].toString().includes('<img') ? '100%' : 'auto')">
                            <Panel toggleable v-if="Array.isArray(r[r1])" v-for="r in datamaps">
                                <template #header>
                                    <b>{{ r1 }}({{ r[r1].length }})</b>
                                </template>
                                <template #icons>
                                    <button @click="addRow(r, r1)" class="p-panel-header-icon p-link mr-2">
                                    ...
                                    <tbody>
                                        <tr v-for="(td, idx) in r[r1]">
                                            <td v-for="th in arrayKeys(r, r1)">
                                                <InputNumber spellcheck="false" :useGrouping="isFormat(th, td[th])"
                                                    v-if="isTypeInput(td[th], th) == 1" v-model="td[th]" class="w-full" />
                                                <InputMask spellcheck="false" mask="99/99/9999"
                                                    v-if="isTypeInput(td[th], th) == 2" v-model="td[th]" class="w-full" />
                                                <Textarea v-if="isTypeInput(td[th], th) == 0" spellcheck="false" autoResize
                                                    v-model="td[th]" rows="1" class="w-full"></Textarea>
                                            </td>
                                            <td class="text-center">
                                                <button @click="delRow(r, r1, idx)" class="p-panel-header-icon p-link">
                            ...
                            <div v-else>
                                <div class="cols-2"><b>{{ r1 }}</b></div>
                                <div class="cols-10">
                                    <InputNumber spellcheck="false" :useGrouping="isFormat(r1, r[r1])"
                                        v-if="isTypeInput(r[r1], r1) == 1" v-model="r[r1]" class="w-full" />
                                    <InputMask spellcheck="false" mask="99/99/9999" v-if="isTypeInput(r[r1], r1) == 2"
                                        v-model="r[r1]" class="w-full" />
                                    <Textarea v-if="isTypeInput(r[r1], r1) == 0" spellcheck="false" autoResize
                                        v-model="r[r1]" rows="1" class="w-full"></Textarea>
                                </div>
                            </div>
                        </li>
                    </ul>
            ...
            <div v-if="opTypeDB.value == 1">
                <DataTable class="p-datatable-sm" showGridlines :value="dtTables">
                    <Column class="text-center" headerStyle="width: 50px;text-align: center;" field="index" header="STT">
                    </Column>
                    <Column field="name" header="Tiêu đề"></Column>
                    <Column style="width: 140px;text-align: center;" headerClass="text-center" class="text-center">
                        <template #body="slotProps">
                            <Button @click="openCogDatabase(slotProps.data, true, false)" icon="pi pi-cog"
                                severity="secondary" outlined />
                            <Button class="ml-1" @click="removeTable(slotProps.data)" icon="pi pi-trash" severity="danger"
                                outlined />
                        </template>
                    </Column>
                </DataTable>
            </div>
            <div v-else class="p-2" style="height: calc(100vh - 170px);overflow-y: auto;">
                <Panel v-if="oneRow" toggleable :collapsed="false" v-for="dt in objDataTemp">
                    <template #header>
                        <h3 class="p-0 m-0"><i class="pi pi-user"></i> Thông tin lấy dữ liệu</h3>
                    </template>
                    <DataTable class="p-datatable-sm" showGridlines :value="dt.rows" v-if="dt.table">
                        <template #groupheader="slotProps">
                            <b>{{ slotProps.data.group || "Dữ liệu" }}</b>
                        </template>
                        <template #header>
                            <div class="flex justify-content-end">
                                <UserComponent ref="rfUserComp" :removes="dt.users" :hrm="true" :full="true"
                                    :keyuser="dt.key.toString()" :callbackFun="getCompUsers" :one="false" :basic="true">
                                </UserComponent>
                                <Button class="ml-1" @click="addRowWord(dt)" icon="pi pi-plus-circle" severity="secondary"
                                    outlined />
                            </div>
                        </template>
                        <Column field="stt" header="STT" style="width:60px;text-align: center;">
                            <template #body="slotProps">
                                <InputNumber spellcheck="false" v-model="slotProps.data.stt" class="w-full" />
                            </template>
                        </Column>
                        <Column v-for="c in dt.cols" style="min-width: 100px;white-space: nowrap;">
                            <template #header>
                                {{ c.value.replace(/\[(.+)\]/g, "$1") }}
                            </template>
                            <template #body="slotProps">
                                <Textarea style="height: auto;padding: 5px;" spellcheck="false" autoResize
                                    v-model='slotProps.data[c.value]' class="w-full"></Textarea>
                            </template>
                        </Column>
                        <Column header="">
                            <template #body="slotProps">
                                <button @click="delRow(dt, 'rows', slotProps.index)" class="p-panel-header-icon p-link">
                                    <span class="pi pi-trash"></span>
                                </button>
                            </template>
                        </Column>
                    ...
                    <div class="grid" v-else>
                        <div :class="'col-4' + (r.auto ? ' disabled' : '')" v-for="r in dt.cols">
                            <div class="mb-2"><label><b>{{ r.value.replace(/\[(.+)\]/g, "$1") }}</b></label></div>
                            <InputComponent :okey="'key'" :col="r" :ipmodel="r" />
                        </div>
                    </div>
                </Panel>
                <div v-else>
                    <DataTable v-model:filters="rpfilters" scrollable :scrollHeight="'calc(100vh - 250px)'"
                        class="p-datatable-sm fix-head" v-model:expandedRowGroups="expandedRowGroups" expandableRowGroups
                        rowGroupMode="subheader" groupRowsBy="group" showGridlines :value="dtDataReports">
                        <template #groupheader="slotProps">
                            <b>{{ slotProps.data.group || "Dữ liệu" }}</b>
                        </template>
                        ...
                        <Column frozen
                            :style="'min-width:' + (col.includes('Họ tên') ? '150pt' : 'unset') + ';width:' + ((col.includes('Ảnh') || col == 'ok') ? '50pt' : 'auto')"
                            v-for="col of dtColumns" :key="col" :field="col"
                            :header="(col != 'ok' && !col.includes('_')) ? col : ''">
                            <template #body="dt">
                                <div v-if="col.includes('Ảnh')">
                        ...
                        <Column v-for="c in objDataTemp[0].cols" style="white-space: nowrap;">
                            <template #header>
                                {{ c.value.toString().replace(/\[(.+)\]/g, "$1") }}
                            </template>
                            <template #body="slotProps">
                                <InputComponent :div="true" :okey="c.value" v-if="slotProps.data.datatemp" :col="c"
