...
const options = ref({
  ...
  pageno: 0,
  pagesize: 50,
  pagenoDM: 0,totalRecordsExport:50,
  pagesizeDM: 10,
  loading: true,
  totalRecords: null,
  totalRecordsDM: null,
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_doc_report_list_send", dataTeo, config)
    .then((response) => {
      ...
      if (data.length > 0) {
        data.forEach((element, i) => {
          element.is_order =
          (dataTeo.PageNo) * options.value.pagesize + i + 1;
        });
...
const RemoveMul=(index,value)=>{
   ...
   if(index==1)
options.value.ca_groups_list= options.value.ca_groups_list.filter(x=>x.code!=value);
    
     if(index==3)
options.value.ca_fields_list= options.value.ca_fields_list.filter(x=>x.code!=value);
     if(index==4)
options.value.ca_dispatch_book_list= options.value.ca_dispatch_book_list.filter(x=>x.code!=value);
     if(index==5)
options.value.ca_user_recever_list= options.value.ca_user_recever_list.filter(x=>x.code!=value);
...
const columns = ref([
  ...
  { field: "related_unit", header: "Đơn vị nhận khác" },
        ...
        <thead>
          ...
          <div style="width:100%; align-item:center; font-weight:600">Tổng số: `+  datalistsExport.value.length+` </div>
            ...
            <td    style="width:33.33%;padding: 0 0 0.5rem 0 ;text-align:center; " >
               ...
               <div class="title1" style="width:100%;font-weight:600;height:100%;    padding-top:0">VĂN BẢN ĐI</div> 
  ...
  for (let index = 0; index < datalistsExport.value.length; index++) {
    ...
    dispatch_book_code +
      ...
      ` </div>
            ...
            <td   >
              <div >
               <div style="text-align:center;padding:0px">  `+doc_date
      +
      ` </div>
...
else
{
  ...
  .then((response) => {
    let data = JSON.parse(response.data.data)[0];
    data.forEach(element => {
      if(!element.compendium)
      element.compendium='';
      if(!element.receive_place)
      element.receive_place='';
      if(!element.ldt)
      element.ldt='';
      if(!element.signer)
      element.signer='';
      if(!element.user_receive)
      element.user_receive='';
      if(!element.dispatch_book_num)
      element.dispatch_book_num='';
      if(!element.doc_code)
        element.doc_code='';
    });
    if (data.length > 0) {
...
const exportData = (method) => {
  ...
  axios
    .post(
      ...
      {
        excelname: "BÁO CÁO SỔ CÔNG VĂN ĐI",
        proc: "doc_report_list_send_export",
        par: [
          { par: "pageno", va:  (options.value.pagenoExport-1  ) },
          { par: "pagesize", va: options.value.totalRecordsExport },
          { par: "user_id", va: store.state.user.user_id },
    ...
    <div class="d-lang-table">
      ...
      >
        <template #header>
          <div>
            <h3 class="module-title my-2 ml-1">
              <font-awesome-icon icon="fa-solid fa-file-arrow-up" /> Báo cáo
              khối cơ quan đi ({{
                  ...
                  <div class="grid formgrid m-2">
                    ...
                    <div class="field col-12 md:col-12 flex">
                      ...
                      <div class="col-9 p-0 align-items-center flex">
                        <MultiSelect
                          v-model="options.ca_groups_list"
                          :options="listFilterDM.ca_groups_list"
                          optionLabel="name"
                          ...
                          <template #value="slotProps">
                            <div class="flex">
                              <div
                                ...
                                :key="option.code"
                              >
                                      <Chip :label="option.name" removable  @remove="RemoveMul(2,option.code)"  />
                              </div>
                            </div>
                            <template
                              v-if="
                    ...
                    <div class="field col-12 md:col-12 flex">
                      ...
                      <div class="col-9 p-0 align-items-center flex">
                        <MultiSelect
                          v-model="options.ca_fields_list"
                          :options="listFilterDM.ca_fields_list"
                          optionLabel="name"
                    ...
                    <div class="field col-12 md:col-12 flex">
                      ...
                      <div class="col-9 p-0 align-items-center flex">
                        <MultiSelect
                          v-model="options.ca_dispatch_book_list"
                          :options="listFilterDM.ca_dispatch_book_list"
                          optionLabel="name"
                     ...
                     <div class="field col-12 md:col-12 flex">
                      ...
                      <div class="col-9 p-0 align-items-center flex">
                        <MultiSelect
                          v-model="options.ca_user_recever_list"
                          :options="listFilterDM.ca_user_recever_list"
                          optionLabel="name"
        ...
        <Column
          ...
          bodyStyle="text-align:center;max-width:120px;word-break:break-all"
          :sortable="true"
        >
        ...
        <Column
          ...
          :sortable="true"
          filterField="receive_date" dataType="date"
        >
          <template #filter="{ filterModel }">
            <Calendar
              v-model="filterModel.value"
        ...
        <Column
          ...
          bodyStyle="max-width:100px;word-break:break-all"
          bodyClass="limit-line-1"
          :sortable="true"
        >
          <template #filter="{ filterModel }">
            <InputText
...
{{data.data.compendium}}
        ...
        <template
									...
									>
                  <div>
              {{ data.data.ldt }} <span v-if="data.data.saodv"> - </span
              >{{ data.data.saodv }}
            </div>
									</template>
        </Column>
        ...
        <Column
          ...
          class="align-items-center justify-content-center text-center"
          headerStyle="text-align:center;max-width:70px"
          bodyStyle="text-align:center;max-width:70px; word-break:break-word"
        >
          <template #body="data">
            <div v-if="data.data.file_path">
           
              <Button @click="showURLSCV(basedomainURL + data.data.file_path)" icon="pi pi-paperclip" v-tooltip.top="'File đính kèm'" class="p-button-rounded p-button-outlined" />
    ...
    <iframe name="printframe" id="printframe" style="display: none"></iframe> </div><Dialog    :style="{ width: '20vw' }" :header="headerExport" v-model:visible="showExport" >
	<div class="grid">
