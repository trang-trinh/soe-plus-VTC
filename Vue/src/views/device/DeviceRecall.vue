...
import printListRecall from "./print/printListRecall.vue";
...
const listTypeP = ref([
  { name: "Bàn giao theo yêu cầu", code: 1 },
  { name: "Bàn giao cá nhân", code: 0 },
]);
...
const showListAssets = () => {
  if (device_recall.value.product_user != null) {
    filterSQLDM.value = [];
    ...
    swal.fire({
      title: "Error!",
      text: "Chọn người sử dụng thiết bị để thu hồi!",
      icon: "error",
...
const delCard = (Card) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá phiếu thu hồi này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/device_recall/delete_device_recall", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Card != null ? [Card.device_recall_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá phiếu thu hồi thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_device_recall", data, config)
    .then((response) => {
...
const loadDataSQLDM = () => {
  ...
  let filterS = {
    filterconstraints: [{ value: "DSD", matchMode: "equals" }],
    filteroperator: "and",
    key: "status",
  };
...
const onSort = (event) => {
  if (event.sortField == null) {
    ...
    if (event.sortField != " device_recall_id") {
      options.value.sort +=
        ", device_recall_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
    }
...
//Xóa nhiều
const deleteList = () => {
  let listId = new Array(selectedHandOver.length);
  let check = false;
  ...
  selectedHandOver.value.forEach((element) => {
    if (element.status != 0) {
      swal.fire({
        ...
        text:
          element.status == 1
            ? "Không được xóa phiếu thu hồi có trạng thái: Chờ xác nhận"
            : element.status == 2
            ? "Không được xóa phiếu thu hồi có trạng thái:  Đã xác nhận"
            : "Không được xóa phiếu thu hồi có trạng thái: Trả lại",
        icon: "error",
        confirmButtonText: "OK",
      });
      check = true;
  ...
  if (check) {
    ...
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xóa danh sách thu hồi này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          selectedHandOver.value.forEach((item) => {
            listId.push(item.device_recall_id);
          });
          axios
            .delete(baseURL + "/api/device_recall/delete_device_recall", {
              headers: { Authorization: `Bearer ${store.getters.token}` },
            ...
            .then((response) => {
              ...
              if (response.data.err != "1") {
                swal.close();
                toast.success("Xoá thu hồi thành công!");
                checkDelList.value = false;
...
const v$ = useVuelidate(rules, device_recall);
const danhMuc = ref();
//METHOD

const onChangeUser1 = (value) => {
 
  device_recall.value.user_warehouse_position =
    listDropdownWarehouse.value.filter((x) => x.code == value)[0].position_name;
};
const onChangeUser2 = (value) => {
  selectedCard.value = [];
  device_recall.value.product_user_position = listDropdownUser.value.filter(
    (x) => x.code == value
  )[0].position_name;
};

const listDropdownWarehouse = ref();
...
const loadUserWarehouse = (value) => {
  listDropdownWarehouse.value = [];
  axios
    .post(
      baseURL + "/api/device_card/getData",
      {
        str: encr(
          JSON.stringify({
            proc: "device_warehouse_user_list",
            par: [
              { par: "warehouse_id", va: value },{ par: "user_id", va: store.getters.user.user_id }],
          }),
          SecretKey,
          cryoptojs
        ).toString(),
    ...
    .then((response) => {
      ...
      data.forEach((element, i) => {
        listDropdownWarehouse.value.push({
          name: element.full_name  ,
          code: element.stocker  ,
...
const openDetails = (device_recall_id) => {
  axios
    .post(
      baseURL + "/api/device_card/getData",
      {
        str: encr(
          JSON.stringify({
            proc: "device_recall_card_list",
            par: [{ par: "device_recall_id", va: device_recall_id }],
          }),
          SecretKey,
          cryoptojs
        ).toString(),
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              {
                par: "device_node_id",
                va: deviceDataDetails.value.device_recall_id,
              },
              { par: "type", va: 3 },
            ],
          }),
...
loadLogs( deviceDataDetails.value.device_recall_id,3);
...
const openDetailsHandover = (data) => {
  device_recall_id.value = data.device_recall_id;
...
const deleteFileH = (value) => {
  listFilesS.value = listFilesS.value.filter(
    (x) => x.recall_files_id != value.recall_files_id
  );
};
...
const saveHandover = (isFormValid) => {
  ...
  if (device_recall.value.recall_number.length > 50) {
    swal.fire({
      title: "Thông báo!",
  ...
  if (device_recall.value.created_place.length > 250) {
    swal.fire({
      title: "Thông báo!",
      text: "Nơi lập không được dài quá 250 kí tự!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  if (!selectedCard.value.length > 0) {
    swal.fire({
      title: "Thông báo!",
      text: "Vui lòng chọn thiết bị trước khi thu hồi!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  formData.append("recall", JSON.stringify(device_recall.value));
  formData.append("recalldetails", JSON.stringify(selectedCard.value));
  formData.append("recallfiles", JSON.stringify(listFilesS.value));
  ...
  if (!isSaveCard.value) {
    axios
      .post(baseURL + "/api/device_recall/add_device_recall", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
    ...
    axios
      .put(
        baseURL + "/api/device_recall/update_device_recall",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa phiếu thu hồi thành công!");
...
const editCard = (data) => {
  ...
  axios
      ...
      baseURL + "/api/device_card/getData",
      {
        str: encr(
          JSON.stringify({
            ...
            par: [{ par: "device_recall_id", va: data.device_recall_id }],
          }),
          SecretKey,
          cryoptojs
        ).toString(),
    ...
    .then((response) => {
      ...
      headerDialog.value = "Sửa phiếu thu hồi";
...
const openBasic = (str) => {
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [ { par: "user_id", va: store.getters.user.user_id },
              ...
              { par: "code_number", va: "TS_PhieuThuHoi" },
              { par: "status", va: true },
            ],
          }),
          SecretKey,
  ...
  displayBasic.value = true;
        ...
        swal.fire({
          title: "Error!",
          text: "Vui lòng cấu hình số hiệu cho phiếu thu hồi!",
          icon: "error",
          confirmButtonText: "OK",
        });
    ...
    .post(
      baseURL + "/api/device_card/getData",
      {
        str: encr(
          JSON.stringify({
            proc: "device_recall_get",
            par: [{ par: "device_recall_id", va: device_recall_id.value }],
          }),
          SecretKey,
          cryoptojs
        ).toString(),
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              {
                par: "device_note_id",
                va: device_recall.value.device_recall_id,
              },
              {
                par: "device_process_code",
                va: device_recall.value.recall_number,
              },
              { par: "device_process_type", va: 3 },
            ],
          }),
          SecretKey,
          cryoptojs
...
htmltable +=
 ...
 htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td align='center' style='font-size:25px;padding-top:30px;padding-bottom:25px;font-weight:bold;'>BIÊN BẢN THU HỒI TÀI SẢN</td></tr></tbody></table>";
     //   htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td><p style='text-align:center;font-size:16px;'>Hà Nội, ngày " + new Date( device_recall.value.created_date).getDate() + " tháng " + (new Date( device_recall.value.created_date).getMonth() + 1) + " năm " + new Date( device_recall.value.created_date).getFullYear() + "</p></td></tr></tbody></table>";
     //   htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td><p style='text-align:center;font-size:16px;'>Số phiếu: " + device_recall.value.recall_number + "</p></td></tr></tbody></table>";
     htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td><p style='font-size:16px;'>Căn cứ tờ trình được duyệt số: " +  device_recall.value.recall_number + " ngày " + new Date( device_recall.value.created_date).getDate() + " tháng " + (new Date( device_recall.value.created_date).getMonth() + 1) + " năm " + new Date( device_recall.value.created_date).getFullYear() + " về việc thu hồi tài sản, chúng tôi gồm:</p></td></tr></tbody></table>";
      
      
      htmltable += "<table border='0' width='1024' cellpadding='10'><tbody>";
        htmltable += "<tr><td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Người thu hồi: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.full_name + "</a></td>";
        htmltable += "<td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Chức vụ: </a><a style='width:calc(100% - 100px);float:right;'>" + device_recall.value.position_name + "</a></td></tr>";
        htmltable += "<tr><td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Người nhận: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.user_warehouse_name + "</a></td>";
              htmltable += "<tr><td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Kho nhận: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.warehouse_name + "</a></td>";
      htmltable += "<td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Chức vụ: </a><a style='width:calc(100% - 100px);float:right;'>" + device_recall.value.user_warehouse_position + "</a></td></tr>";
        htmltable += "<tr><td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Người sử dụng: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.product_user_name + "</a></td>";
        htmltable += "<td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Phòng ban: </a><a style='width:calc(100% - 100px);float:right;'>" + device_recall.value.product_user_department + "</a></td></tr>";
        htmltable += "<tr><td colspan='2' align='left' style='font-size:16px;'><a style='font-weight:bold;'>Nội dung: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.content_recall + "</a></td></tr>";
        htmltable += "<tr><td colspan='2' align='left' style='font-size:16px;'><a style='font-weight:bold;'>Lý do thu hồi: </a><a style='width:calc(100% - 120px);float:right;'>" + device_recall.value.reason + "</a></td></tr>";
        htmltable += "<tr><td colspan='2' align='left' style='font-size:16px;'><a style='font-weight:bold;'>Danh sách tài sản thu hồi: </a></th></tr></tbody></table>";
        htmltable += "<table width='1024' cellpadding='10' style='padding:0px 15px 15px;border-spacing:0;'><thead>";
        htmltable += "<tr><th width='40' align='center' style='font-size:16px;border:1px solid #000;'>STT</th>";
        htmltable += "<th width='90' align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Số hiệu</th>";
        htmltable += "<th width='90'  align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Mã barcode</th>";
        htmltable += "<th align='center' style ='word-wrap:break-word;font-size:16px;border:1px solid #000;border-left:none;'>Tên tài sản</th>";
        //htmltable += "<th width='70'  align='center' style='font-size:16px;border:1px solid #000;'>Năm SX</th>";
        htmltable += "<th width='90'  align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Giá trị hiện tại</th>";
        //htmltable += "<th width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Giá trị</th>";
        htmltable += "<th width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Tình trạng</th>";
        // htmltable += "<th width='120' align='center' style='word-wrap:break-word; font-size:16px;border:1px solid #000;border-left:none;'>Tình trạng</th>";
        // htmltable += "<th width='120' align='center' style='word-wrap:break-word; font-size:16px;border:1px solid #000;border-left:none;'>Trạng thái</th>";
        ...
        listAssetsH.value.forEach(function (ts) {
            stt += 1;
            if(ts.current_price)
            Tong+=ts.current_price;
            htmltable += "<tr><td width='40' align='center' style='font-size:16px;border:1px solid #000;border-top:none;'>" + stt + "</td>";
            htmltable += "<td width='90' align='center' style='word-break:break-all;font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" + ts.device_number + "</td>";
            htmltable += "<td width='90'  align='center' style='word-break:break-all;font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" + ts.barcode_id + "</td>";
            htmltable += "<td align='left' style ='word-wrap:break-word;font-size:16px;border:1px solid #000;border-left:none;border-top:none;' >" + ts.device_name + "</td>";
            //htmltable += "<td width='70'  align='center' style='font-size:16px;border:1px solid #000;'>" + ts.NamSanxuat + "</td>";
            // htmltable += "<td width='90'  align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" + (ts.Thoigiansudung > 0 ? (ts.Thoigiansudung + ' tháng') : 0) + "</td>";
            //htmltable += "<td width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" + ts.Giatri + "</td>";
            htmltable += "<td width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" + ts.current_price.toLocaleString() + "</td>";
            htmltable += "<td width='120' align='left' style='word-wrap:break-word;font-size:16px;border:1px solid #000;border-left:none;border-top:none;' >" + ts.condition + "</td>";
            // htmltable += "<td width='120' align='left' style='word-wrap:break-word;font-size:16px;border:1px solid #000;border-left:none;border-top:none;' >" + (ts.Trangthai == 0 ? 'Hỏng' : ts.Trangthai == 1 ? 'Chuyển kho sửa chữa' : ts.Trangthai == 2 ? 'Chuyển kho' : '') + "</td></tr>";
        });
        htmltable += "</tbody></table>";
        htmltable += "<table border='0' width='1024' cellpadding='0' style='padding:0px 15px;'><tbody>";
        htmltable += "<tr><td style='font-size:16px;font-weight:bold;text-align:right;'>Tổng:  " + Tong.toLocaleString() + " VND </td></tr></tbody></table>";
        if (device_recall.value.print_note == null || device_recall.value.print_note == '') {
            htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td><p style='font-size:16px;text-align:justify;'>Biên bản này có hiệu lực từ ngày ký và được lập thành 02 bản, mỗi bên giữ 01 bản có giá trị như nhau.</p></td></tr></tbody></table>";
        }
        else {
            htmltable += "<table border='0' width='1024' cellpadding='10'><tbody><tr><td><p style='font-size:16px;text-align:justify;'>" + device_recall.value.print_note + "</p></td></tr></tbody></table>";
        }
        htmltable += "<table border='0' width='1024' cellpadding='0' style='padding:0px 15px;'><tbody>";
        htmltable += "<tr><td colspan='4' align='left' style='font-size:16px;font-weight:bold;'>Xác nhận quy trình xử lý:</td></tr>";
      var isStaRow = true;
...
const exportData = (method) => {
  ...
  axios
    .post(
      ...
      {
        excelname: "PHIẾU THU HỒI",
        proc: "device_recall_list_export",
...
onMounted(() => {  if (!checkURL(window.location.pathname, store.getters.listModule)) {
  ...
  return {
    ...
    danhMuc,
        ...
        <template>
  <div class="d-device_recall_idntainer">
    <div class="d-lang-table">
      ...
      >
        <template #header>
          <div>
            <h3 class="module-title my-2 ml-1">
              <i class="pi pi-history"></i> Danh sách phiếu thu hồi ({{
          ...
          <Toolbar class="d-toolbar p-0 py-3 surface-50">
            ...
            <template #end>
              ...
              />
              <Button
                @click="openBasic('Thêm phiếu thu hồi')"
                label="Thêm mới"
                icon="pi pi-plus"
                class="mr-2"
              />
        ...
        <Column
          ...
          field="recall_created_date"
          header="Ngày lập"
        >
          <template #body="data">
            <div>
              {{
                moment(new Date(data.data.recall_created_date)).format(
                  "DD/MM/YYYY"
                )
              }}
            </div>
          ...
          <template #body="data">
            <div>
              <div class="flex w-full align-items-center pr-2">
                <Avatar
                  v-bind:label="
                    data.data.product_avatar
                      ? ''
                      : data.data.product_name.substring(
                          data.data.product_name.lastIndexOf(' ') + 1,
                          data.data.product_name.lastIndexOf(' ') + 2
                        )
                  "
                  :image="basedomainURL + data.data.product_avatar"
                  size="small"
                  :style="
                    data.data.product_avatar
                      ? 'background-color: #2196f3'
                      : 'background:' +
                        bgColor[data.data.product_name.length % 7]
                  "
                  shape="circle"
                  @error="
                    $event.target.src =
                  ...
                  "
                />
                <div class="px-2">{{ data.data.product_name }}</div>
              </div>
            </div>
          </template>
        </Column>
  ...
  >
    <div class="w-full format-center">
      <h3>Danh sách thiết bị thu hồi kèm theo</h3>
    </div>
    <div
  ...
  >
    <form>
      <div class="grid formgrid m-2">
        ...
        <div class="col-12 field flex p-0">
          <div class="col-6 flex p-0 align-items-center">
            ...
            <div style="width: calc(100% - 10rem)">
              <InputText
                v-model="device_recall.recall_number"
                class="w-full"
                :class="{
                  'p-invalid': v$.recall_number.$invalid && submitted,
                }"
              />
            </div>
          </div>
          <div class="col-6 flex p-0 text-center align-items-center">
            ...
            <div class="col-8 p-0 flex text-left">
              <Calendar
                ...
                class="w-full"
                id="basic_use_date"
                v-model="device_recall.recall_created_date"
                autocomplete="on"
                :min-date="
                  device_recall.recall_created_date
                    ? device_recall.recall_created_date
                    : new Date('1/1/1970')
                "
                :showIcon="true"
        ...
        <div
          v-if="
            (v$.recall_number.$invalid && submitted) ||
            v$.recall_number.$pending.$response
          "
        ...
        >
          ...
          <small style="width: calc(100% - 10rem)">
            <span style="color: red" class="w-full">{{
              v$.recall_number.required.$message
        ...
        <div class="col-12 flex p-0">
          <div class="col-12 field p-0 flex text-left align-items-center">
            ...
            <div style="width: calc(100% - 10rem)">
              <InputText v-model="device_recall.created_place" class="w-full" />
            </div>
          </div>
        </div>
        <div class="col-12 flex p-0">
          <div class="col-12 field p-0 flex text-left align-items-center">
            ...
            <div style="width: calc(100% - 10rem)">
              <Textarea
                ...
                spellcheck="false"
                :class="{
                  'p-invalid': v$.content_recall.$invalid && submitted,
                }"
              ></Textarea>
            </div>
          </div>
        ...
        <div
          v-if="
            (v$.content_recall.$invalid && submitted) ||
            v$.content_recall.$pending.$response
          "
          class="col-12 field p-0 flex"
        >
          <div class="w-10rem"></div>
          <small style="width: calc(100% - 10rem)">
            <span style="color: red" class="w-full">{{
              v$.content_recall.required.$message
        ...
        <div class="col-12 flex p-0">
          <div class="col-12 field p-0 flex text-left align-items-center">
            <div class="w-10rem">
              Lý do: <span class="redsao pl-1"> (*)</span>
            </div>
            <div style="width: calc(100% - 10rem)">
              <Textarea
                autoResize
                ...
                spellcheck="false"
                :class="{
                  'p-invalid': v$.reason.$invalid && submitted,
                }"
              ></Textarea>
            </div>
          </div>
        ...
        <div
          v-if="
            (v$.reason.$invalid && submitted) || v$.reason.$pending.$response
          "
          class="col-12 field p-0 flex"
        >
          <div class="w-10rem"></div>
        ...
        <div class="col-12 flex p-0 align-items-center">
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              <div class="w-10rem">
                Người thu hồi: <span class="redsao pl-1"> (*)</span>
              </div>
              <div
          ...
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              ...
              <div
                ...
                class="p-2 font-bold"
              >
                {{ store.getters.user.position_name }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-12 field p-0 text-lg font-bold">
          <Toolbar class="p-0 surface-0 border-none">
            <template #start>
              <div class="text-lg font-bold">Nơi nhận </div>
            </template>
            <template #end> </template>
          </Toolbar>
        </div>
 <div class="col-12 flex p-0">
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              <div class="w-10rem">
                Kho nhận<span class="redsao pl-1"> (*)</span>
              </div>
              <Dropdown
                v-model="device_recall.warehouse_id"
                panelClass="d-design-dropdown"
                ...
                optionValue="code"
                style="width: calc(100% - 10rem)"
                @change="loadUserWarehouse(device_recall.warehouse_id)"
                placeholder="Chọn kho nhận"
                :class="{
                  'p-invalid': !device_recall.warehouse_id && submitted,
                }"
              >
            ...
            <div
              v-if="!device_recall.warehouse_id && submitted"
              class="col-12 field p-0 flex"
            >
              <div class="w-10rem"></div>
              <small style="width: calc(100% - 10rem)">
        ...
        <div class="col-12 flex p-0" v-if="device_recall.warehouse_id">
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              <div class="w-10rem">
                Người nhận<span class="redsao pl-1"> (*)</span>
              </div>
              <Dropdown
                v-model="device_recall.stocker"
                panelClass="d-design-dropdown"
                class="sel-placeholder"
                :options="listDropdownWarehouse"
                :filter="true"
                optionLabel="name"
                optionValue="code"
                style="width: calc(100% - 10rem)"
                @change="onChangeUser1(device_recall.stocker)"
                placeholder="Chọn người nhận"
                :class="{
                  'p-invalid': !device_recall.stocker && submitted,
                }"
              >
                <template #option="slotProps">
                  <div class="country-item flex align-items-center">
            ...
            <div
              v-if="!device_recall.stocker && submitted"
              class="col-12 field p-0 flex"
            >
              <div class="w-10rem"></div>
              <small style="width: calc(100% - 10rem)">
          ...
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              <div class="w-10rem pl-5">Chức vụ:</div>
              <InputText
                v-model="device_recall.user_warehouse_position"
                disabled
                style="width: calc(100% - 10rem)"
              />
            </div>
        ...
        <div class="col-12 field p-0">
          ...
          <div class="col-12 flex p-0">
            <div class="col-6 p-0 text-left align-items-center">
              <div class="col-12 field p-0 flex text-left align-items-center">
                <div class="w-10rem">
                  Người sử dụng: <span class="redsao pl-1"> (*)</span>
                </div>
                <Dropdown
                  v-model="device_recall.product_user"
                  panelClass="d-design-dropdown"
                  class="sel-placeholder"
                  :options="listDropdownUser"
                  :filter="true"
                  ...
                  optionValue="code"
                  :class="{
                    'p-invalid': !device_recall.product_user && submitted,
                  }"
                  style="width: calc(100% - 10rem)"
                  @change="onChangeUser2(device_recall.product_user)"
            ...
            <div class="col-6 p-0 text-left align-items-center">
              <div class="col-12 field p-0 flex text-left align-items-center">
                <div class="w-10rem pl-5">Chức vụ:</div>
                <InputText
                  v-model="device_recall.product_user_position"
                  disabled
                  style="width: calc(100% - 10rem)"
                />
              </div>
          ...
          <div
            v-if="!device_recall.product_user && submitted"
            class="col-12 field p-0 flex"
          >
            <div class="w-10rem"></div>
            <small style="width: calc(100% - 10rem)">
        ...
        <div class="col-12 p-0">
          ...
          >
            ...
            <div class="w-full field flex align-items-center">
              <div class="w-10rem p-0 text-center">Tình trạng</div>
          ...
          <template #item="slotProps">
            <div class="product-item">
              ...
              <div class="product-list-detail">
                ...
                <div class="flex">
                  <div class="w-full" v-tooltip.top="'Giá trị hiện tại'">
                    <i class="pi pi-money-bill product-category-icon"></i>
                    {{ slotProps.item.current_price?slotProps.item.current_price.toLocaleString():'0' }} VND 
                  </div>
  ...
  <Dialog
    ...
    v-model:visible="displayDetailsHandover" :modal="true"
    :maximizable="true"
    :style="{ width: '60vw' }"
  >
    <form>
      <div v-if="displayDetailsHandover">
  ...
  >
    <div v-if="displayDeviceRepair">
      <configAprroved
        ...
        :device_process_code="deviceRepairS.recall_number"
        :device_note_id="deviceRepairS.device_recall_id"
        :checkApp="1"
        :device_process_id="null"
        :device_process_type="3"
        :listAssetsH="null"
      ></configAprroved>
    </div>
  </Dialog>
...
.d-avatar-device_recall {
  position: relative;
  width: 100%;
  height: 350px;
}
.d-avatar-device_recall img {
  position: absolute;
  top: 50%;
  left: 50%;
