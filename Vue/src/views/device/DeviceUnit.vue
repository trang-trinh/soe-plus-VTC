...
const v$ = useVuelidate(rules, device_unit);
...
const saveWarehouse = (isFormValid) => {
  ...
  if (!issaveWarehouse.value) {
    axios
      .post(
        baseURL + "/api/device_unit/add_device_unit",
        device_unit.value,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          ...
          swal.fire({
            title: "Thông báo!",
            text:
              ms.includes("device_unit_name") == true
                ? "Tên đơn vị tính không quá 500 ký tự!"
                : ms,
            icon: "error",
            confirmButtonText: "OK",
          });
    ...
    axios
      .put(
        baseURL + "/api/device_unit/update_device_unit",
        device_unit.value,
        config
      )
      .then((response) => {
...
const editWarehouse = (dataPlace) => {
  submitted.value = false;
  device_unit.value = dataPlace;
  device_unit.value.organization_id =
  ...
  headerDialog.value = "Sửa đơn vị tính";
...
const delWarehouse = (Warehouse) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá đơn vị tính này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/device_unit/delete_device_unit", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Warehouse != null ? [Warehouse.device_unit_id] : 1,
          })
          .then((response) => {
            swal.close();
...
const onFilter = (event) => {
  ...
  for (const [key, value] of Object.entries(event.filters)) {
    if (key != "global") {
      let obj = {
        key: key == "device_unit_name" ? "device_unit_name" : key,
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_device_unit", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  options.value.loading = true;
  let data = {
    IntID: value.device_unit_id,
    TextID: value.device_unit_id + "",
    IntTrangthai: 1,
  ...
  if (
    ...
    axios
      .put(baseURL + "/api/device_unit/update_s_device_unit", data, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa đơn vị tính thành công!");
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        selectedWarehouses.value.forEach((item) => {
          listId.push(item.device_unit_id);
        });
        axios
          .delete(baseURL + "/api/device_unit/delete_device_unit", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-pound"></i> Danh sách đơn vị tính ({{
    ...
    <form>
      <div class="grid formgrid m-2">
        <div class="col-12 md:col-12 p-0">
          <div class="field col-12 md:col-12 flex">
            <div class="col-12 p-0 flex align-items-center">
              <label class="col-4 text-left p-0"
                >Tên đơn vị tính <span class="redsao">(*)</span></label
              >
              <InputText
                v-model="device_unit.device_unit_name"
                spellcheck="false"
                class="col-8 ip36 px-2"
                :class="{
                  'p-invalid': v$.device_unit_name.$invalid && submitted,
                }"
              />
            </div>
          </div>
          <div
            v-if="
              (v$.device_unit_name.$invalid && submitted) ||
              v$.device_unit_name.$pending.$response
            "
            style="display: flex"
          ...
          >
            ...
            <small class="col-10 p-error p-0">
              <span class="col-12 p-0">{{
                v$.device_unit_name.required.$message
                  .replace("Value", "Tên đơn vị tính")
