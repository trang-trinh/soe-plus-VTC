...
import printListRepair from "./print/printListRepair.vue";
...
const delCard = (Card) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá sửa chữa này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/device_repair/delete_device_repair", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Card != null ? [Card.device_repair_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá thẻ sửa chữa thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_device_repair", data, config)
    .then((response) => {
...
const loadDataSQLDM = () => {
  datalistsDM.value = [];
  let arw = null;
  if (device_repair.value.department_id_fake) {
    Object.keys(device_repair.value.department_id_fake).forEach((key) => {
      arw = key;
      return;
    });
  }
  if (
    device_repair.value.warehouse_id != null ||
    device_repair.value.department_id_fake != null
  ) {
    if (device_repair.value.warehouse_id != null) {
      filterSQLDM.value = [];
      let filterS = {
        filterconstraints: [
          {
            value: device_repair.value.warehouse_id,
            matchMode: "equals",
          },
        ],
        filteroperator: "or",
      ...
      let filterS1 = {
        filterconstraints: [{ value: "DSD", matchMode: "notEquals" }],
        filteroperator: "and",
        key: "status",
      };
      filterSQLDM.value.push(filterS1);
    }
    if (device_repair.value.department_id_fake != null) {
      if (!store.getters.user.is_admin) {
        ...
        filterSQLDM.value.push(filterS11);
      } else {
        let filterS = {
  ...
  filterSQLDM.value.push(filterS);
  let filterS2 = {
    filterconstraints: [{ value: "CXN", matchMode: "notEquals" }],
    filteroperator: "and",
    key: "status",
  };
  filterSQLDM.value.push(filterS2);
  let filterS3 = {
    filterconstraints: [{ value: "HKS", matchMode: "notEquals" }],
    filteroperator: "and",
    key: "status",
  };
  ...
  axios
    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        ...
        datalistsDM.value = [data, selectedCard.value];
...
const onSort = (event) => {
  if (event.sortField == null) {
    ...
    if (event.sortField != " device_repair_id") {
      options.value.sort +=
        ", device_repair_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
    }
...
const deleteList = () => {
  ...
  selectedHandOver.value.forEach((element) => {
    if (element.status != 0) {
      swal.fire({
        ...
        text:
          element.status == 1
            ? "Không được xóa phiếu sửa chữa có trạng thái: Chờ xác nhận"
            : element.status == 2
            ? "Không được xóa phiếu sửa chữa có trạng thái:  Đã xác nhận"
            : "Không được xóa phiếu sửa chữa có trạng thái: Trả lại",
        icon: "error",
        confirmButtonText: "OK",
      });
      check = true;
  ...
  if (check) {
    ...
    swal
      .fire({
        title: "Thông báo",
        text: "Bạn có muốn xóa danh sách sửa chữa này không!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          selectedHandOver.value.forEach((item) => {
            listId.push(item.device_repair_id);
          });
          axios
            .delete(baseURL + "/api/device_repair/delete_device_repair", {
              headers: { Authorization: `Bearer ${store.getters.token}` },
            ...
            .then((response) => {
              ...
              if (response.data.err != "1") {
                swal.close();
                toast.success("Xoá sửa chữa thành công!");
                checkDelList.value = false;
...
const listTypeRepair = ref([
  ...
  { name: "Hoàn thành sửa chữa", code: 1 },
]);
const files = ref([]);
...
const v$ = useVuelidate(rules, device_repair);
const danhMuc = ref();
//METHOD
...
const onShowProcedure = () => {
  ...
  axios
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              {
                par: "device_node_id",
                va: deviceDataDetails.value.device_repair_id,
              },
              { par: "type", va: 1 },
            ],
          }),
    ...
    .then((response) => {
      ...
      loadLogs(deviceDataDetails.value.device_repair_id, 1);
...
const deleteFileH = (value) => {
  listFilesS.value = listFilesS.value.filter(
    (x) => x.repair_files_id != value.repair_files_id
  );
};
...
const saveHandover = (isFormValid) => {
  ...
  if (device_repair.value.department_id_fake)
    Object.keys(device_repair.value.department_id_fake).forEach((key) => {
      device_repair.value.department_id = Number(key);
    });
  if (device_repair.value.repair_number.length > 50) {
    swal.fire({
      title: "Thông báo!",
  ...
  formData.append("repair", JSON.stringify(device_repair.value));
  formData.append("repairdetails", JSON.stringify(selectedCard.value));
  formData.append("repairfiles", JSON.stringify(listFilesS.value));
  ...
  if (!isSaveCard.value) {
    axios
      .post(baseURL + "/api/device_repair/add_device_repair", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Thêm  thẻ sửa chữa thành công!");
    ...
    axios
      .put(
        baseURL + "/api/device_repair/update_device_repair",
        formData,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa thẻ sửa chữa thành công!");
...
const editCard = (data) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      device_repair.value.department_id_fake = {};
      if (store.getters.user.is_admin == true)
        device_repair.value.department_id_fake[
          device_repair.value.department_id
        ] = true;
      else {
        device_repair.value.department_id_fake = null;
      }
      ...
      headerDialog.value = "Sửa thẻ sửa chữa";
      if (listDepartment.value.length > 0 && store.getters.user.is_admin) {
        device_repair.value.department_id_fake = {};
        device_repair.value.department_id_fake[
          device_repair.value.department_id
        ] = true;
...
const loadDeviceNumber = (dataVL) => {
  axios
    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        device_repair.value.repair_number =
...
const openBasic = (str) => {
  ...
  axios
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              ...
              { par: "code_number", va: "TS_PhieuSuaChua" },
              { par: "status", va: true },
            ],
          }),
          SecretKey,
    ...
    device_repair.value.department_id_fake = null;
    device_repair.value.department_id = store.getters.user.department_id;
  }
  files.value = [];
  submitted.value = false;
  ...
  displayBasic.value = true;
        ...
        swal.fire({
          title: "Error!",
          text: "Vui lòng cấu hình số hiệu cho phiếu sửa chữa!",
          icon: "error",
          confirmButtonText: "OK",
        });
        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              {
                par: "device_note_id",
                va: device_repair.value.device_repair_id,
              },
              {
                par: "device_process_code",
                va: device_repair.value.repair_number,
              },
              { par: "device_process_type", va: 1 },
            ],
          }),
          SecretKey,
          cryoptojs
...
function renderhtml(id, htmltable) {
  ...
  htmltable +=
    ...
    new Date(device_repair.value.created_date).getDate() +
    ", tháng " +
    (new Date(device_repair.value.created_date).getMonth() + 1) +
    ", năm " +
    new Date(device_repair.value.created_date).getFullYear() +
  ...
  htmltable +=
    ...
    "</a></td></tr>";
  htmltable +=
    "<tr><td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Chức vụ: </a><a style='width:calc(100% - 120px);float:right;'>" +
    device_repair.value.position_name +
    "</a></td>";
  htmltable +=
    "<td align='left' style='font-size:16px;'><a style='font-weight:bold;'>Phòng ban: </a><a style='width:calc(100% - 100px);float:right;'>" +
    device_repair.value.department_name +
    "</a></td></tr>";
  htmltable +=
    "<tr><td colspan='2' align='left' style='font-size:16px;'><a style='font-weight:bold;'>Danh sách tài sản sửa chữa: </a></th></tr></tbody></table>";
  htmltable +=
    "<table width='1024' cellpadding='10' style='padding:0px 15px 15px;border-spacing:0;'><thead>";
  htmltable +=
    "<tr><th width='30' align='center' style='font-size:16px;border:1px solid #000;'>STT</th>";
  htmltable +=
    "<th width='90' align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Số hiệu</th>";
  //htmltable += "<th align='center' style='font-size:16px;border:1px solid #000;'>Mã Barcode</th>";
  htmltable +=
    "<th style='font-size:16px;border:1px solid #000;border-left:none;'>Tên tài sản</th>";
  htmltable +=
    "<th width='90' align='center' style='font-size:16px;border:1px solid #000;border-left:none;padding:10px 5px;'>Giá sửa chữa</th>";
  htmltable +=
    "<th width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;padding:10px 5px;'>Tình trạng</th>";
  htmltable +=
    "<th width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;padding:10px 5px;'>Phương án sửa</th>";
  htmltable +=
    "<th width='100' align='center' style='font-size:16px;border:1px solid #000;border-left:none;padding:10px 5px;'>Trạng thái sửa</th>";
  htmltable +=
    "<th width='80' align='center' style='font-size:16px;border:1px solid #000;border-left:none;'>Ngày sửa</th></tr></thead>";
  htmltable += "<tbody>";
  var stt = 0;
  var tong = 0;
  listAssetsH.value.forEach(function (ts) {
    stt += 1;
    if (ts.repair_price) tong += ts.repair_price;
    htmltable +=
      "<tr><td width='30' align='center' style='font-size:16px;border:1px solid #000;border-top:none;'>" +
      stt +
      "</td>";
    htmltable +=
      "<td width='90' align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" +
      ts.device_number +
      "</td>";
    //htmltable += "<td width='90' align='center' style='font-size:16px;border:1px solid #000;'>" + (ts.MaBarcode != null ? ts.MaBarcode : '') + "</td>";
    htmltable +=
      "<td align='left' style='font-size:16px;border:1px solid #000;word-wrap:break-word;border-left:none;border-top:none;'>" +
      ts.device_name +
      "</td>";
    htmltable +=
      "<td width='90' align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;padding:10px 5px;'>" +
      (ts.repair_price != null
        ? ts.repair_price.toLocaleString() + " VND"
        : " 0 ") +
      "</td>";
    htmltable +=
      "<td width='100' align='left' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;padding:10px 5px;'>" +
      (ts.condition != null ? ts.condition : "") +
      "</td>";
    htmltable +=
      "<td width='100' align='left' style='font-size:16px;border:1px solid #000;word-wrap:break-word;border-left:none;border-top:none;padding:10px 5px;'>" +
      (ts.repair_plan != null ? ts.repair_plan : "") +
      "</td>";
    htmltable +=
      "<td width='100' align='left' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;padding:10px 5px;'>" +
      (ts.repair_condition == 1
        ? "Hoàn thành"
        : ts.repair_condition == 2
        ? "Chuyển kho sửa chữa"
        : ts.repair_condition == 3
        ? "Hỏng không sửa chữa được"
        : "") +
      "</td>";
    htmltable +=
      "<td width='80' align='center' style='font-size:16px;border:1px solid #000;border-left:none;border-top:none;'>" +
      (ts.repair_date != null
        ? moment(ts.repair_date).format("DD/MM/YYYY")
        : "") +
      "</td></tr>";
  });
  ...
  htmltable +=
    "<tr><td style='font-size:16px;font-weight:bold;text-align:right;'><a style='padding-right:20px'>Tổng giá trị sửa chữa: </a>" +
    tong.toLocaleString() +
    " VND</td></tr></tbody></table>";
  htmltable +=
    "<table border='0' width='1024' cellpadding='0' style='padding:0px 15px;'><tbody>";
  htmltable +=
    "<tr><td colspan='4' align='left' style='font-size:16px;font-weight:bold;'>Xác nhận quy trình xử lý:</td></tr>";
...
const filterCard = (check) => {
  ...
  if (filterPhongban.value != null) {
    var strf = null;
    Object.keys(filterPhongban.value).forEach((label) => {
    ...
    let filterS = {
      filterconstraints: [{ value: strf, matchMode: "contains" }],
      filteroperator: "and",
      key: "department_id",
    };
    filterSQL.value.push(filterS);
...
const onChangeWarehouse = () => {
  if (device_repair.value.warehouse_id != null) {
    device_repair.value.department_id_fake = null;
    device_repair.value.department_id = null;
    selectedCard.value = [];
  }
};
...
const onChangeDepartment1 = () => {
  if (device_repair.value.department_id != null) {
    selectedCard.value = [];
    device_repair.value.warehouse_id = null;
  }
};
...
const exportData = (method) => {
  ...
  axios
    .post(
      ...
      {
        excelname: "DANH SÁCH THẺ SỬA CHỮA",
        proc: "device_repair_list_export",
        par: [  { par: "user_id", va: store.state.user.user_id },
          { par: "search", va: options.value.search },
          { par: "status", va: filterTrangthai.value },
          { par: "repair_type", va: filterType.value  },
          { par: "department_id", va: filterPhongban.value },
          { par: "proposer", va: filterCardUserSend.value },
...
onMounted(() => {  if (!checkURL(window.location.pathname, store.getters.listModule)) {
  ...
  return {
    ...
    danhMuc,
        ...
        <template>
  <div class="d-device_repair_idntainer">
      ...
      >
        <template #header>
          <div>
            <h3 class="module-title my-2 ml-1">
              <i class="pi pi-ticket"></i> Danh sách thẻ sửa chữa ({{
          ...
          <Toolbar class="d-toolbar p-0 py-3 surface-50">
                  ...
                  <div class="grid formgrid m-2">
                    ...
                    <div class="field col-12 md:col-12 flex">
                      ...
                      <Dropdown
                        ...
                        class="col-8 p-0"
                        :style="
                          filterType != null ? 'border:2px solid #2196f3' : ''
                        "
                      />
                    </div>
            ...
            <template #end>
              ...
              />
              <Button
                @click="openBasic('Thêm phiếu sửa chữa')"
                label="Thêm mới"
                icon="pi pi-plus"
                class="mr-2"
              />
        ...
        >
          <template #body="data">
            <div>
              {{
                data.data.repair_type == 1 ? "Sửa chữa" : "Bảo trì - Bảo dưỡng"
              }}
            </div>
          </template>
        </Column>
        ...
        >
          <template #body="data">
            <div>
              {{
                moment(new Date(data.data.repair_created_date)).format(
                  "DD/MM/YYYY"
                )
              }}
            </div>
            ...
            >
              ...
              <Button
                ...
                class="
                  ...
                  mx-1
                "
                v-tooltip.left="'Chuyển xác nhận'"
  ...
  >
    <form>
      <div class="grid formgrid m-2">
          ...
          <div class="col-6 flex p-0 align-items-center">
            ...
            <div style="width: calc(100% - 10rem)">
              <InputText
                ...
                class="w-full"
                :class="{
                  'p-invalid': v$.repair_number.$invalid && submitted,
                }"
              />
            </div>
          </div>
          <div class="col-6 flex p-0 text-center align-items-center">
            ...
            <div class="col-8 p-0 flex text-left">
              <Calendar
                ...
                autocomplete="on"
                :min-date="
                  device_repair.repair_created_date
                    ? device_repair.repair_created_date
                    : new Date('1/1/1970')
                "
                :showIcon="true"
        ...
        <div
          v-if="
            (v$.repair_number.$invalid && submitted) ||
            v$.repair_number.$pending.$response
          "
        ...
        >
          ...
          <small style="width: calc(100% - 10rem)">
            <span style="color: red" class="w-full">{{
              v$.repair_number.required.$message
        ...
        <div class="col-12 flex p-0">
          <div class="col-6 p-0 text-left align-items-center">
            <div class="col-12 field p-0 flex text-left align-items-center">
              ...
              <div style="width: calc(100% - 10rem)">
                <TreeSelect
                  v-model="device_repair.department_id_fake"
                  :options="listDepartment"
                  :showClear="true"
                  :max-height="200"
                  placeholder="---------- Đơn vị ----------"
                ...
                <Dropdown
                  v-model="device_repair.department_id"
                  v-else
          ...
          <div
            class="col-6 p-0 text-left align-items-center"
            v-if="listWarehouse.length > 0"
          >
            <div class="col-12 field p-0 flex text-left align-items-center">
              <div class="col-4 p-0 pl-5 text-left">Kho</div>
              <Dropdown
                v-model="device_repair.warehouse_id"
        ...
        <div class="col-12 p-0">
          ...
          >
            ...
            <div class="w-full field flex align-items-center">
              <div class="w-10rem p-0">Tình trạng</div>
              ...
              <div class="product-list-detail">
                ...
                <div class="flex">
                  ...
                  <div
                    v-else
                    class="w-full"
                    v-tooltip.top="'Phòng ban quản lý'"
                  >
                    <i class="pi pi-home product-category-icon"></i>
  ...
  >
    <div v-if="displayDeviceRepair">
      <configAprroved
        ...
        :device_process_code="deviceRepairS.repair_number"
        :device_note_id="deviceRepairS.device_repair_id"
        :checkApp="1"
        :device_process_id="null"
        :device_process_type="1"
        :listAssetsH="null"
      ></configAprroved>
    </div>
  </Dialog>
...
.d-avatar-device_repair {
  position: relative;
  width: 100%;
  height: 350px;
}
.d-avatar-device_repair img {
  position: absolute;
  top: 50%;
  left: 50%;
