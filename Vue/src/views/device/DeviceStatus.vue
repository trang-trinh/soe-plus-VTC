...
const rules = {
  device_status_name: {
    ...
    $errors: [
      {
        $property: "device_status_name",
        $validator: "required",
        $message: "Tên trạng thái thiết bị không được để trống!",
      },
    ],
  },
  device_status_code: {
    ...
    $errors: [
      {
        $property: "device_status_code",
        $validator: "required",
        $message: "Mã trạng thái thiết bị không được để trống!",
      },
    ],
  },
};
...
const v$ = useVuelidate(rules, device_status);
...
const saveWarehouse = (isFormValid) => {
  ...
  if (!issaveWarehouse.value) {
    axios
      .post(
        baseURL + "/api/device_status/add_device_status",
        device_status.value,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Thêm trạng thái thiết bị thành công!");
          ...
          swal.fire({
            title: "Thông báo!",
            text:
              ms.includes("device_status_name") == true
                ? "Tên trạng thái thiết bị không quá 500 ký tự!"
                : ms,
            icon: "error",
    ...
    axios
      .put(
        baseURL + "/api/device_status/update_device_status",
        device_status.value,
        config
      )
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
          toast.success("Sửa trạng thái thiết bị thành công!");
...
const editWarehouse = (dataPlace) => {
  submitted.value = false;
  device_status.value = dataPlace;
  device_status.value.organization_id =
  ...
  headerDialog.value = "Sửa trạng thái thiết bị";
...
const delWarehouse = (Warehouse) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá trạng thái thiết bị này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/device_status/delete_device_status", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Warehouse != null ? [Warehouse.device_status_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err == "0") {
              swal.close();
              toast.success("Xoá trạng thái thiết bị thành công!");
              if (
...
const onFilter = (event) => {
  ...
  for (const [key, value] of Object.entries(event.filters)) {
    if (key != "global") {
      let obj = {
        key: key == "device_status_name" ? "device_status_name" : key,
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_device_status", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  options.value.loading = true;
  let data = {
    IntID: value.device_status_id,
    TextID: value.device_status_id + "",
    IntTrangthai: 1,
    BitTrangthai: value.status,
  };

  axios
    .put(baseURL + "/api/device_status/update_s_device_status", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Sửa trạng thái thiết bị thành công!");
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        selectedWarehouses.value.forEach((item) => {
          listId.push(item.device_status_id);
        });
        axios
          .delete(baseURL + "/api/device_status/delete_device_status", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-qrcode"></i> Danh sách trạng thái thiết bị ({{
        ...
        <Toolbar class="w-full custoolbar">
          ...
          <template #end>
            ...
            />
            <Button
              @click="openBasic('Thêm mới trạng thái')"
              label="Thêm mới"
              icon="pi pi-plus"
              class="mr-2"
            />
      ...
      >
        <template #body="data">
          <div
            v-if="
               store.getters.user.is_admin ||  store.state.user.user_id == data.data.created_by ||
              (store.state.user.role_id == 'admin' &&
                store.state.user.organization_id == data.data.organization_id)
            "
          >
    ...
    <form>
      <div class="grid formgrid m-2">
        <div class="col-12 md:col-12 p-0">
          <div class="field col-12 md:col-12 flex">
            <div class="col-12 p-0 flex align-items-center">
              <label class="col-4 text-left p-0"
                >Mã trạng thái <span class="redsao">(*)</span></label
              >
              <InputText
                v-model="device_status.device_status_code"
                spellcheck="false"
                class="col-8 ip36 px-2"
                :class="{
                  'p-invalid': v$.device_status_code.$invalid && submitted,
                }"
              />
            </div>
          </div>
          <div
            v-if="
              (v$.device_status_code.$invalid && submitted) ||
              v$.device_status_code.$pending.$response
            "
            style="display: flex"
          ...
          >
            ...
            <small class="col-10 p-error p-0">
              <span class="col-12 p-0">{{
                v$.device_status_code.required.$message
                  .replace("Value", "Mã trạng thái thiết bị")
        ...
        <div class="col-12 md:col-12 p-0">
          <div class="field col-12 md:col-12 flex">
            <div class="col-12 p-0 flex align-items-center">
              <label class="col-4 text-left p-0"
                >Tên trạng thái <span class="redsao">(*)</span></label
              >
              <InputText
                v-model="device_status.device_status_name"
                spellcheck="false"
                class="col-8 ip36 px-2"
                :class="{
                  'p-invalid': v$.device_status_name.$invalid && submitted,
                }"
              />
            </div>
          </div>
          <div
            v-if="
              (v$.device_status_name.$invalid && submitted) ||
              v$.device_status_name.$pending.$response
            "
            style="display: flex"
            class="field col-12 md:col-12"
          >
            ...
            <small class="col-10 p-error p-0">
              <span class="col-12 p-0">{{
                v$.device_status_name.required.$message
                  .replace("Value", "Tên trạng thái thiết bị")
                  .replace("is required", "không được để trống")
              }}</span>
            </small>
          </div>
        ...
        <div class="field col-12 md:col-12 flex align-items-center">
          ...
          <InputNumber
            v-model="device_status.is_order"
            spellcheck="false"
