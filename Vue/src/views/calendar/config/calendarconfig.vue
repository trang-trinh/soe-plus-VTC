    ...
    $errors: [
      {
        $property: "is_group",
        $validator: "required",
        $message: "Loại quy trình không được để trống!",
      },
    ],
  },
  procedureform_name: {
...
const groups = ref([
  { value: 0, title: "Quy trình lịch họp tuần" },
  { value: 1, title: "Quy trình lịch công tác" },
  { value: 2, title: "Quy trình lịch trực ban" },
]);
const types = ref([
...
const saveModelProcedure = (isFormValid) => {
  ...
  axios
    .put(
      baseURL + "/api/calendar_config/update_procedureform",
      formData,
      config
    )
    .then((response) => {
...
const saveModelSign = (isFormValid) => {
  ...
  axios
    .put(baseURL + "/api/calendar_config/update_signform", formData, config)
    .then((response) => {
      if (response.data.err === "1") {
        swal.close();
        if (options.value.loading) options.value.loading = false;
  ...
  axios
    .post(
      baseURL + "/api/calendar/get_datas",
      {
        str: encr(
          JSON.stringify({
            proc: "calendar_procedureform_get",
            par: [
...
const updateStatusProcedure = (item) => {
  ...
  axios
    .put(
      baseURL + "/api/calendar_config/update_status_procedureform",
      data,
      config
    )
    .then((response) => {
...
const updateStatusSign = (item) => {
  ...
  axios
    .put(baseURL + "/api/calendar_config/update_status_signform", data, config)
    .then((response) => {
      if (response.data.err === "1") {
        swal.close();
        swal.fire({
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/calendar_config/delete_procedureform", {
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/calendar_config/delete_signform", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: ids,
          })
          .then((response) => {
  ...
  axios
    .post(
      baseURL + "/api/calendar/get_datas",
      {
        str: encr(
          JSON.stringify({
            proc: "calendar_procedureform_list",
            par: [
        ...
        <div class="d-lang-table">
          ...
          >
            <template #groupheader="slotProps">
              <div>
                <i class="pi pi-list mr-2"></i>{{ slotProps.data.group_name }}
              </div>
            </template>
            <Column
      ...
      <div class="grid formgrid m-2">
        <div class="col-12 md:col-12">
          <div class="form-group">
            <label>Loại quy trình <span class="redsao">(*)</span></label>
            <Dropdown
              :options="groups"
              :filter="true"
              :showClear="false"
              :editable="false"
              v-model="modelprocedure.is_group"
              optionLabel="title"
              optionValue="value"
              placeholder="Chọn loại quy trình"
              class="ip36"
              :class="{
                'p-invalid': vp$.is_group.$invalid && submitted,
              }"
            >
            ...
            <div
              v-if="
                (vp$.is_group.$invalid && submitted) ||
                vp$.is_group.$pending.$response
              "
            >
              <small class="p-error">
                <span class="col-12 p-0">{{
                  vp$.is_group.required.$message
                    .replace("Value", "Loại quy trình")
