    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        data.forEach((item, index) => {
          let arrDays = [];
          let total_price_user = 0;
          if (item.listdate != null) {
            arrDays = item.listdate.split(",");
          }
          day_off.forEach((item) => {
            if (!arrDays.includes(item)) arrDays.push(item);
          });
          item.listdate = arrDays;
          item.is_order = index;
          item.totalRation = dataTime.value.length - item.listdate.length;
          // tính giá tiền từng suất ăn -> tổng tiền của mỗi người
          dataTime.value.forEach((t) => {
            if (!item.listdate.includes(t)) {
              // t - ngày đăng ký ăn
              ...
              total_price_user += price_date;
            }
          });
          item.total_Price_User = total_price_user;
          item.totalPrice = formatNumber(item.total_Price_User, 0, ".", ".");
        });
        ...
        data.forEach((item, index) => {
          ...
          countPrice += item.total_Price_User;
          if (
            index == data.length - 1 ||
            ...
            let obj = {
              ...
              totalRation: totalR,
              total_Price_User: countPrice,
              totalPrice: formatNumber(countPrice, 0, ".", "."),
              role_name: "",
              department_name: "",
              listdate: dataTime.value,
            };
            total_Ration.value += obj.totalRation;
            total_Users_Price.value += obj.total_Price_User;
        ...
        total_Price.value = formatNumber(total_Users_Price.value, 0, ".", ".");
      } else {
        datalists.value = [];
      }
    ...
    <div class="w-full format-center">
      <div>
        ...
        <div class="text-md">
          ...
          <span class="font-italic">(<b>X</b> : ngày đăng ký cơm)</span>
        </div>
      </div>
    </div>
    <div>
      ...
      <table class="w-full mt-2" style="overflow-y: scroll" id="table-bc">
          ...
          >
            ...
            <th
              ...
              style="
                ...
                max-width: 100px;
              "
              class="m-checkbox-table top-0 text-lg sticky left-600"
            >
              Tổng tiền
              <br />(VNĐ)
            </th>
        ...
        <tbody>
          ...
          >
            ...
            <td
              class="sticky left-600 p-2 text-lg text-right"
              :style="
                item.isTotal
                  ? 'background-color:#dbeaf7'
                  : 'background-color: #f8f9fa'
          ...
          <tr>
            ...
            <td
              class="sticky left-50 p-2 text-lg font-bold"
              colspan="3"
              style="text-align: center"
            >
              Tổng cộng
            ...
            <td class="sticky left-600 p-2 text-lg text-right font-bold">
              {{ total_Price }}
            </td>
