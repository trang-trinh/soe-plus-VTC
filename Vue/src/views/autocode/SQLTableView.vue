...
import { renderSQL, renderVUE, renderAPI } from "../../util/autocode";
...
//Thêm sửa xoá
function callback(error, event) {}
const onCopy = () => {
  if (activeTab.value == 3) {
    copyText(table.value.CodeVUE,undefined,callback);
    toast.success("Đã copy Code VUE!");
  } else if (activeTab.value == 4) {
    copyText(table.value.CodeAPI,undefined,callback);
    toast.success("Đã copy Code API!");
  } else if (activeTab.value == 5) {
    copyText(table.value.CodeSQL,undefined,callback);
    toast.success("Đã copy Code SQL!");
  }
};
const onDownload = () => {
  let primarykey = columns.value[0];
  let model = primarykey.Name.split("_")[0];
  swalLoadding();
  var zip = JSZip();
  var folder = zip.folder(table.value.TABLE_NAME);
  folder.file(table.value.TABLE_NAME + ".sql", table.value.CodeSQL);
  folder.file(model + "View.vue", table.value.CodeVUE);
  folder.file(table.value.TABLE_NAME + "Controller.cs", table.value.CodeAPI);
  zip
    .generateAsync({
      type: "blob",
    })
    .then(function (content) {
      saveAs(content, table.value.TABLE_NAME + ".zip");
      toast.success("Tải file thành công!");
      swal.close();
    });
};
const initRenderCode = () => {
  if (table) {
    table.value.CodeSQL = renderSQL(table.value, columns.value);
    table.value.CodeAPI = renderAPI(table.value, columns.value);
    table.value.CodeVUE = renderVUE(table.value, columns.value);
  }
};
const changeTab = () => {
  initRenderCode();
    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        data.forEach((c, i) => {
          ...
          if (!c.Css) {
            if (c.Name == "Trangthai" || c.Name == "STT") {
              c.Css = "col-6";
            } else {
              c.Css = "col-12";
            }
    ...
    if (data.length > 0) {
      if (data.length > 1) {
        ...
        if (!table.value.Title) {
          table.value.Title = table.value.TableName;
        }
        data[0].forEach((e) => {
...
const AllShowForm = ref(false);
const changeAllShow = () => {
  columns.value.forEach((cl) => {
    cl.Show = AllShow.value;
  });
};
const changeAllShowForm = () => {
  columns.value.forEach((cl) => {
    cl.ShowForm = AllShowForm.value;
  });
};
onMounted(() => {
  loadDatabase();
...
<template>
              ...
              >
                <template #body="slotProps">
                  <Checkbox
                    id="IsIdentity"
                    disabled="true"
                    v-model="slotProps.data.IsIdentity"
                    :binary="true"
                  />
                </template>
              </Column>
            ...
            >
              ...
              >
                ...
                <template #body="slotProps">
                  <Checkbox
                    id="ShowForm"
                    v-model="slotProps.data.ShowForm"
                    :binary="true"
                  />
                </template>
              </Column>
              <Column
                ...
                header="Search"
                class="align-items-center justify-content-center text-center"
                headerStyle="text-align:center;max-width:70px;color:#fff;background-color: #4285f4;"
                bodyStyle="text-align:center;max-width:70px"
              >
                <template #body="slotProps">
                  <Checkbox id="Search" v-model="slotProps.data.Search" :binary="true" />
                </template>
              </Column>
              <Column
                field="IsKey"
                header="Key"
                class="align-items-center justify-content-center text-center"
                headerStyle="text-align:center;max-width:50px;color:#fff;background-color: #4285f4;"
                bodyStyle="text-align:center;max-width:50px"
              >
                <template #body="slotProps">
                  <Checkbox id="IsKey" v-model="slotProps.data.IsKey" :binary="true" />
                </template>
              </Column>
              <Column
                field="IsValue"
                header="Value"
                class="align-items-center justify-content-center text-center"
                headerStyle="text-align:center;max-width:60px;color:#fff;background-color: #4285f4;"
                bodyStyle="text-align:center;max-width:60px"
              >
                <template #body="slotProps">
                  <Checkbox
                    id="IsValue"
                    v-model="slotProps.data.IsValue"
                    :binary="true"
                  />
                </template>
              </Column>
  ...
  <Dialog
    :header="'Chỉnh sửa thông tin Table ' + (table != null ? table.TABLE_NAME : '')"
  ...
  >
    <form @submit.prevent="saveTable">
      <div class="grid formgrid m-2">
        ...
        <div class="field col-12 md:col-12">
          <label class="col-2 text-left" style="vertical-align: top">Phân trang</label>
          <InputSwitch class="col-4" v-model="table.IsPag" />
          <label class="col-2 text-right" style="vertical-align: top">Form Data</label>
          <InputSwitch class="col-4" v-model="table.IsFormData" />
        </div>
      </div>
    </form>
    <template #footer>
