...
const rules = {
	request_group_name: {
		...
		$errors: [
			{
				$property: "request_group_name",
				$validator: "required",
				$message: "Tên nhóm đề xuất không được để trống!",
			},
		],
	},
};
...
const onPage = (event) => {
	...
	if (event.page == 0) {
		...
		options.value.id =
			datalists.value[datalists.value.length - 1].request_group_id;
		options.value.IsNext = true;
	} else if (event.page < options.value.PageNo) {
		//Trang trước
		options.value.id = datalists.value[0].request_group_id;
		options.value.IsNext = false;
	}
...
const saveData = (isFormValid) => {
	...
	if (request_group.value.request_group_name.length > 250) {
		swal.fire({
			title: "Cảnh báo",
			text: "Tên nhóm đề xuất không được vượt quá 500 ký tự!",
			icon: "error",
			confirmButtonText: "OK",
		});
		return;
	}
	request_group.value.request_group_name = request_group.value.request_group_name.trim();
	let formData = new FormData();
	formData.append("model", JSON.stringify(request_group.value));
	swal.fire({
		width: 110,
		didOpen: () => {
			swal.showLoading();
	...
	if (!isSaveTem.value) {
		axios
			.post(
				baseUrlCheck + "/api/request_ca_group/add_request_ca_group",
				formData,
				config
			)
			.then((response) => {
				if (response.data.err != "1") {
					swal.close();
					toast.success("Thêm nhóm đề xuất thành công!");
					loadData(true);
					closeDialog();
				} else {
					swal.fire({
		...
		axios
			.put(
				baseUrlCheck + "/api/request_ca_group/update_request_ca_group",
				formData,
				config
			)
			.then((response) => {
				if (response.data.err != "1") {
					swal.close();
					toast.success("Cập nhật nhóm đề xuất thành công!");
					closeDialog();
				} else {
					swal.fire({
						title: "Error!",
...
const editTem = (dataTem) => {
	submitted.value = false;
	request_group.value = dataTem;
	headerDialog.value = "Cập nhật nhóm đề xuất";
	...
	swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios
				.delete(
					baseUrlCheck +
						"/api/request_ca_group/delete_request_ca_group",
					{
						headers: {
							Authorization: `Bearer ${store.getters.token}`,
						},
						data: Tem != null ? [Tem.request_group_id] : -1,
					}
				)
				.then((response) => {
					swal.close();
					if (response.data.err != "1") {
						swal.close();
						toast.success("Xoá nhóm đề xuất thành công!");
						loadData(true);
					} else {
						swal.fire({
							title: "Error!",
...
const loadDataSQL = () => {
	...
	axios
		.post(
			baseUrlCheck + "/api/request_ca_group/Filter_request_ca_group",
			data,
			config
		)
		.then((response) => {
...
const onCheckBox = (value, check) => {
	if (check) {
		let data = {
			IntID: value.request_group_id,
			TextID: value.request_group_id + "",
			IntTrangthai: 1,
		...
		axios
			.put(
				baseUrlCheck +
					"/api/request_ca_group/update_status_request_ca_group",
				data,
				config
			)
			.then((response) => {
				if (response.data.err != "1") {
					swal.close();
					toast.success("Cập nhật trạng thái nhóm đề xuất thành công!");
					loadData(true);
					closeDialog();
				} else {
					swal.fire({
...
const deleteList = () => {
	...
	if (selectedDatas.value.length == 0) {
		swal.fire({
			text: "Chọn nhóm đề xuất muốn xóa!",
			icon: "warning",
			confirmButtonText: "OK",
		});
		return;
	...
	swal.fire({
		title: "Thông báo",
		text: "Bạn có muốn xoá nhóm đề xuất này không!",
		icon: "warning",
		showCancelButton: true,
		confirmButtonColor: "#3085d6",
		cancelButtonColor: "#d33",
		...
		if (result.isConfirmed) {
			...
			selectedDatas.value.forEach((item) => {
				listId.push(item.request_group_id);
			});
			axios
				.delete(
					baseUrlCheck +
		...
		>
			<template #header>
				<h3 class="module-title mt-0 ml-1 mb-2">
					<i class="pi pi-compass"></i> Danh sách nhóm đề xuất ({{ options.totalRecords }})
				</h3>
				<Toolbar class="w-full custoolbar">
					...
					<template #end>
						<Button
							@click="openBasic('Thêm nhóm đề xuất')"
							label="Thêm mới"
							icon="pi pi-plus"
							class="mr-2"
						/>
			...
			<Column
				field="request_group_name"
				header="Tên nhóm đề xuất"
			...
			<Column
				field="created_date"
				header="Ngày tạo"
				headerStyle="justify-content:center;height:50px;max-width:150px;"
				bodyStyle="justify-content:center;max-width:150px;"
				:sortable="true"
			>
				<template #body="{ data }">
					{{ moment(new Date(data.created_date)).format("DD/MM/YYYY") }}
				</template>
			</Column>
			...
			<div class="grid formgrid m-0">
				<div class="field col-12 md:col-12 algn-items-center flex p-0">
					<div class="col-3 text-left flex p-0" style="align-items:center;">
						Tên nhóm đề xuất <span class="redsao pl-1"> (*)</span>
					</div>
					<Textarea
						v-model="request_group.request_group_name"
						spellcheck="false"
				...
				<div class="field col-12 md:col-12 flex p-0">
					...
					<small
						v-if="
							(v$.request_group_name.$invalid && submitted) ||
							v$.request_group_name.$pending.$response
						"
						class="col-9 p-error"
					>
						<span class="col-12 p-0">
							{{
								v$.request_group_name.required.$message
									.replace(
										"Value",
										"Tên nhóm đề xuất"
									)
					...
					<small
						...
						v-if="
							(v$.request_group_name.maxLength.$invalid &&
								submitted) ||
							v$.request_group_name.maxLength.$pending.$response
						"
					>
							...
							<span class="col-10 p-0">
								{{
									v$.request_group_name.maxLength.$message.replace(
										...
										"Tên nhóm đề xuất không được vượt quá"
									)
								}}
								ký tự
							</span>
				...
				<div class="col-12 field md:col-12 flex p-0">
					<div class="field col-6 md:col-6 p-0 align-items-center flex">
						...
						<InputNumber
							v-model="request_group.is_order"
