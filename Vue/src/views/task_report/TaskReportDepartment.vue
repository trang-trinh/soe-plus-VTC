    ...
    .then((response) => {
      if (store.state.user.is_super == true) {
        ...
        if (data[1][0].is_task == true || store.state.user.is_admin == true) {
          listOrganization.value = data[0].filter(
            (x) =>
              x.parent_id != null &&
              x.parent_id == store.getters.user.organization_id,
          );
          listOrganizationAll.value = data[0];
        } else {
          listOrganization.value = data[0].filter(
            (x) => x.organization_id == data[1][0].department_id,
          );
          listOrganizationAll.value = data[0];
        }
      }
...
const ExportData = () => {
  ...
  htmltable += "<thead>";
  htmltable +=
    "<tr><th colspan='15' style='font-size:16px;text-align:center;'>BÁO CÁO THỐNG KÊ TỔNG HỢP CÔNG VIỆC THEO PHÒNG BAN</th></tr></thead></table>";
  ...
  if (
    listColumExportNews.value.filter((x) => x.value == "Tennhom").length > 0
  ) {
    htmltable +=
      "<th style='width: 150px;font-size:14px;text-align: center;'> Nhóm công việc</th>";
  }
  if (
  ...
  if (arrNew.length > 0) {
    arrNew.forEach(function (t, index) {
      htmltable +=
        ...
        (t.organization_name != null ? t.organization_name : "") +
        "</th></tr>";
      t.Group.forEach(function (d, index) {
        ...
        if (
          listColumExportNews.value.filter((x) => x.value == "Tennhom").length >
          0
        ) {
          htmltable +=
            "<td style='width: 150px;font-size:14px;'>" +
            (d.Tennhom != null ? d.Tennhom : "") +
            "</td>";
        }
        if (
          listColumExportNews.value.filter((x) => x.value == "Nguoigiaoviecs")
...
const changeDonvi = (event) => {
  listOrganization.value = listOrganizationAll.value.filter(
    (x) => x.parent_id == event.value,
  );
};
...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    opition.value.id =
      listTaskReportDepartment.value[
        listTaskReportDepartment.value.length - 1
      ].task_id;
    opition.value.IsNext = true;
  } else if (event.page < opition.value.PageNo) {
    //Trang trước
...
<template>
  ...
  >
            ...
            <ul style="padding: 0px; margin: 0px">
              ...
              <li class="p-menuitem">
                ...
                >
                  ...
                  <Dropdown
                    :filter="true"
                    @click="clickPhongBan($event)"
                    v-model="opition.filterPB"
                    panelClass="d-design-dropdown"
              ...
              <li class="p-menuitem">
                ...
                >
                  ...
                  <Dropdown
                    :filter="true"
                    v-model="opition.filtergroup_id"
                    panelClass="d-design-dropdown"
                    selectionLimit="1"
                    placeholder="Chọn nhóm cồng việc"
    ...
    <DataTable
      ...
      id="table-report-department"
      :value="listTaskReportDepartment"
