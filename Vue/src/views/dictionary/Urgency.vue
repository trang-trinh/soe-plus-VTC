...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_urgency/${issaveField.value ? "Update_urgency" : "Add_urgency"}`,
    data: urgency.value,
    headers: {
      ...
      if (response.data.err != "1") {
        ...
        toast.success(
          response.config.method == "put"
            ? "Cập nhật độ khẩn thành công!"
            : "Thêm mới độ khẩn thành công!",
        );
        loadData(true);
        closeDialog();
      } else {
        ...
        swal.fire({
          title: "Thông báo!",
          text:
            ms.includes("urgency_name") == true
              ? "Độ khẩn không quá 250 ký tự!"
              : ms,
          icon: "error",
          confirmButtonText: "OK",
        });
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_Urgency", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Độ khẩn.xlsx";
