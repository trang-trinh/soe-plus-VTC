...
const openBasic = (str) => {
  ...
  if (store.state.user.is_super) {
    ...
    caTag.value.organization_id = store.state.user.organization_id;
  }
...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL + `/api/ca_tags/${issaveField.value ? "Update_tag" : "Add_tag"}`,
    data: caTag.value,
    headers: {
    ...
    .then((response) => {
      if (response.data.err != "1") {
        ...
        swal.fire({
          title: "Thông báo!",
          text:
            ms.includes("tag_name") == true
              ? "Tên nhãn không quá 250 ký tự!"
              : ms,
          icon: "error",
          confirmButtonText: "OK",
        });
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_Tags", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Nhãn.xlsx";
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2">
          <i class="pi pi-send"></i> Danh sách nhãn ({{ options.totalRecords }})
        </h3>
