      ...
      if (response.data.err != "1") {
        ...
        toast.success(
          response.config.method == "put"
            ? "Cập nhật hình thức gửi thành công!"
            : "Thêm mới hình thức gửi thành công!",
        );
        loadData(true);
        closeDialog();
      } else {
        ...
        swal.fire({
          title: "Thông báo!",
          text:
            ms.includes("send_way_name") == true
              ? "Hình thức gửi không quá 250 ký tự!"
              : ms,
          icon: "error",
          confirmButtonText: "OK",
        });
...
const editField = (dataPlace) => {
  submitted.value = false;
  send_ways.value = JSON.parse(JSON.stringify(dataPlace));
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Sendway", data, config)
    .then((response) => {
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_SendWay", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Hình thức gửi.xlsx";
          ...
          <div class="col-12 p-0">
            ...
            <div class="col-12 p-0 my-3 flex">
              ...
              <div class="col-3 flex p-0">
                ...
                <InputSwitch
                  v-model="send_ways.status"
                  class="col-6 p-0 ip36"
                />
              </div>
              <div
                class="col-3 flex p-0"
                v-if="store.state.user.is_su"
              >
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Hệ thống</div>
                <InputSwitch
                  v-model="send_ways.is_system"
                  class="col-6 p-0 ip36"
                />
              </div>
            </div>
