...
const onPage = (event) => {
  ...
  if (event.page == 0) {
    ...
    options.value.id =
      datalists.value[datalists.value.length - 1].receive_place_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
...
const openBasic = (str) => {
  ...
  if (store.state.user.is_super) {
    ...
    receive_places.value.organization_id = store.state.user.organization_id;
  }
      ...
      if (response.data.err != "1") {
        ...
        toast.success(
          response.data.method == "put"
            ? "Cập nhật nơi nhận thành công!"
            : "Thêm mới nơi nhận thành công!",
        );
        loadData(true);
        closeDialog();
      } else {
        ...
        swal.fire({
          title: "Thông báo!",
          text:
            ms.includes("receive_place_name") == true
              ? "Tên nơi nhận không quá không quá 250 ký tự!"
              : ms,
          icon: "error",
          confirmButtonText: "OK",
        });
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_ReceivePlace", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  ...
  if (
    ...
    axios
      .put(
        baseURL + "/api/ca_receive_places/Update_StatusReceive_Places",
        data,
        config,
      )
      .then((response) => {
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_ReceivePlace", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Nơi nhận.xlsx";
onMounted(() => {
  loadData(true);
  return {
    datalists,
  ...
  <div class="main-layout true flex-grow-1 p-2">
    ...
    >
      ...
      <Column
        ...
        bodyStyle="text-align:center;max-width:160px;;max-height:60px"
      >
        <template #body="Field">
          <Button
          ...
          <div class="col-12 p-0">
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber
                  v-model="receive_places.is_order"
