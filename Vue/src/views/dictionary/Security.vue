...
const openBasic = (str) => {
  ...
  if (store.state.user.is_super == true) {
    ...
    security.value.organization_id = store.state.user.organization_id;
  }
      ...
      if (response.data.err != "1") {
        ...
        toast.success(
          response.config.method == "put"
            ? "Cập nhật độ mật thành công!"
            : "Thêm mới độ mật thành công!",
        );
        loadData(true);
        closeDialog();
      } else {
        ...
        swal.fire({
          title: "Thông báo!",
          text:
            ms.includes("security_name") == true
              ? "Độ mật không quá 250 ký tự!"
              : ms,
          icon: "error",
          confirmButtonText: "OK",
        });
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_Security", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Độ mật.xlsx";
...
<template>
  <div class="main-layout true flex-grow-1 p-2">
    ...
    >
      <template #header>
        <h3 class="module-title mt-0 ml-1 mb-2s">
