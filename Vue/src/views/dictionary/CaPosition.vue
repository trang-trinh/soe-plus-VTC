...
const savePosition = (isFormValid) => {
  ...
  if (!issavePosition.value) {
    axios
      .post(
        baseURL + "/api/doc_ca_position/Add_doc_ca_positions",
        position.value,
        config,
      )
      .then((response) => {
          ...
          swal.fire({
            ...
            text:
              ms.includes("position_name") == true
                ? "Tên chức vụ văn bản không quá 250 ký tự!"
                : ms,
            icon: "error",
            confirmButtonText: "OK",
          });
    ...
    axios
      .put(
        baseURL + "/api/doc_ca_position/Update_doc_ca_positions",
        position.value,
        config,
      )
      .then((response) => {
  ...
  swal
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/doc_ca_position/Delete_ca_positions", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
...
const loadDataSQL = () => {
  ...
  options.value.loading = true;
  axios
    .post(baseURL + "/api/SQL/Filter_Doc_Ca_Postion", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  ...
  if (
    ...
    axios
      .put(
        baseURL + "/api/doc_ca_position/Update_StatusPositions",
        data,
        config,
      )
      .then((response) => {
...
const Upload = () => {
  ...
  if (checkFile == null) {
    ...
    axios
      .post(baseURL + "/api/ImportExcel/Import_CaPosition", formData, config)
      .then((response) => {
        if (response.data.err != "1") {
          swal.close();
...
const item = "/Portals/Mau Excel/Mẫu Excel Chức vụ văn bản.xlsx";
...
<template>
  <div class="main-layout true flex-grow-1 p-2">
    ...
    >
      ...
      >
        <template #body="{ data }">
          <div v-if="data.position_type == 0">Tất cả</div>
          <div v-if="data.position_type == 2">Nhà nước</div>
          <div v-if="data.position_type == 1">Công ty</div>
        </template>
      </Column>
      <Column
        field="status"
