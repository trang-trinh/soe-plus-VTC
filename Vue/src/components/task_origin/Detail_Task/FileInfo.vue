...
watch(ModalDetail, (vl) => {
  if (vl == false) {
    emitter.emit("closeViewFile", false);
  }
});
...
<template>
  ...
  >
    <div class="grid formgrid m-2 h-full format-center file-hover">
      ...
      ></Image>
      <video
        v-else-if="Videos.includes(DataDetail.file_type.toLowerCase()) == true"
        style="width: 100%"
      ...
      <iframe
        v-else-if="
          Documents.includes(DataDetail.file_type.toLowerCase()) == true
        "
        allowfullscreen
        :src="
          basedomainURL +
