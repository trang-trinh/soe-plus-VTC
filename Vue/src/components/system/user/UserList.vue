        ...
        str: encr(
          JSON.stringify({
            ...
            par: [
              ...
              { par: "department_id", va: opition.value.organization_id || opition.value.department_id },
  ...
  axios
    ...
    .then((response) => {
      ...
      if (data.length > 0) {
        ...
        selectCapcha.value = {};
        selectCapcha.value[user.value.department_id || user.value.organization_id || user.value.organization_parent_id || "-1"] = true;
      }
    })
    .catch((error) => {
      if (error.status === 401) {
...
const handleSubmit = (isFormValid) => {
  ...
  if (!selectCapcha.value) {
    swal.fire({
      title: "Thông báo!",
      text: "Vui lòng chọn đơn vị hoặc phòng ban!",
      icon: "error",
      confirmButtonText: "OK",
    });
    return;
  ...
  <div class="main-layout true flex-grow-1 p-2" id="sys_users">
    ...
    >
      <template #header>
        <h3 class="module-title module-title-hidden mt-0 ml-1 mb-2">
          ...
          <Chip
            ...
            @remove="goDonvi()"
            v-if="(opition.department_id || opition.organization_parent_id) && isfilter"
      ...
      <template #grid="slotProps">
        <div class="md:col-3 p-2 card-content">
    ...
    >
      <template #header>
        <!-- <div class="flex">
          ...
          <div class="col-3 text-right" v-if="!check_role">
            <Button
