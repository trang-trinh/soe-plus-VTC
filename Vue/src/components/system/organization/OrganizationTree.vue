...
//   router.push({ name: "organization", params:  {} })
//         .then(() => {
//           router.go(0);
//         });
}
...
//Show Modal
const showModalAddDonvi = (type) => {
    files = [];
    submitted.value = false;
    selectCapcha.value = {};
    if(type ==1){
        selectCapcha.value[id_active.value] = true;
    }
    // selectCapcha.value[store.getters.user.parent_id] = true;
    donvi.value = {
        organization_name: "",
        is_order: type ==1 ?datalistsDetails.value.length+1 : donvis.value.length + 1,
        status: true,
    ...
    axios
        .post(
            baseURL + "/api/Phongban/GetDataProc",
            {
                str: encr(
                    JSON.stringify({
                        proc: "sys_organization_dictionary",
                        par: [
...
const loadDataDetails = (id, name) => {
    ...
    axios
        ...
        .then((response) => {
            ...
            if (isFirst.value) isFirst.value = false;
            let obj = renderTree(data, "organization_id", "organization_name", "phòng ban");
...
<template>
                            ...
                            <Toolbar>
                                <template #start>
                                    <span class="p-input-icon-left">
                                        ...
                                        <InputText v-model="filters['global']" type="text"
                                            spellcheck="false" placeholder="Tìm kiếm đơn vị" />
                                    </span>
                                </template>
                                <template #end>
                                    <Button v-if="store.getters.user.is_super"
                                     @click="showModalAddDonvi(0)" label="Thêm đơn vị" icon="pi pi-plus"
                                        class="mr-2" />
                                        {{ selectedKey }}
                        ...
                        <div>
                            ...
                            <div class="w-full d-lang-table">
                                <TreeTable :value="donvis" v-model:selectionKeys="selectedKey" :paginator="true"
                                    :filters="filters"
                                    :showGridlines="true" filterMode="strict" class="p-treetable-sm" :rows="20"
                                    :rowHover="true" responsiveLayout="scroll" :scrollable="true" scrollHeight="flex">
                                    ...
                                    <Column field="Logo" header="Logo"
                                        ...
                                        <template #body="md">
                                            <div @click="loadDataDetails(md.node.data.organization_id,md.node.data.organization_name )">
                                    ...
                                    <Column field="organization_name" header="Tên đơn vị" :expander="true">
                                        <template #body="md">
                                            <div @click="loadDataDetails(md.node.data.organization_id, md.node.data.organization_name)">
                                                <span :class="md.node.data.organization_id === id_active? 'row-active': ''" :style="[ md.node.data ? 'font-weight:bold' : '',md.node.data.status ? '' : 'color:red !important',
                                    ...
                                    <Column header="Chức năng" headerClass="text-center"
                                        ...
                                        <template #body="md">
                                            <Button type="button" icon="pi pi-plus-circle"
                                                ...
                                                style="margin-right: 0.5rem" v-tooltip.top="'Thêm đơn vị con'"
                                                @click="addTreeDonvi(md.node, 0)"></Button>
                    ...
                    <div class="d-lang-table-r">
                        <div class="p-3" v-if="datalistsDetails">
                            <h3 class="module-title m-0 ">
                                <i class="pi pi-book"></i> {{ department_name }} ({{ datalistsDetails.length}})
                            </h3>
                        ...
                        <Toolbar class="w-full  ">
                            <template #start>
                                <span class="p-input-icon-left">
                                    <i class="pi pi-search" />
                                    <InputText v-model="filters_pb['global']" type="text"
                                        spellcheck="false" placeholder="Tìm kiếm phòng ban" />
                                </span>
                            </template>

                            <template #end>
                                <Button @click="showModalAddDonvi(1)" label="Thêm phòng ban" icon="pi pi-plus"
                                    class="mx-2" />
                        ...
                        <TreeTable :value="datalistsDetails" v-model:selectionKeys="selectedWarehouses"
                             :filters="filters_pb" :showGridlines="true" class="p-treetable-sm"
                            :paginator="true" :rows="options.pagesizeP" :rowHover="true" responsiveLayout="scroll"
                            :scrollable="true" scrollHeight="flex" @page="onPageP($event)"
                            ...
                            <Column header="Chức năng" headerClass="text-center"
                                ...
                                <template #body="md">
                                    <Button type="button" icon="pi pi-plus-circle"
                                        ...
                                        @click="addTreeDonvi(md.node, 1)"></Button>
                                    <Button type="button" icon="pi pi-pencil" v-tooltip.top="'Chỉnh sửa'"
                                        class="p-button-rounded p-button-secondary p-button-outlined"
                                        style="margin-right: 0.5rem" @click="editDonvi(md.node.data)"></Button>
                                    <Button type="button" icon="pi pi-trash" v-tooltip.top="'Xóa'"
    ...
    <Dialog header="Cập nhật Đơn vị/ Phòng ban" v-model:visible="displayAddDonvi" :style="{ width: '860px' }"
        ...
        <form @submit.prevent="handleSubmit(!v$.$invalid)">
            <div class="grid formgrid m-2">
                ...
                <div class="field col-12 md:col-12">
                    ...
                    <Dropdown class="col-10" v-model="donvi.organization_type" :options="tdorganization_types"
                ...
                <div class="field col-12 md:col-12" v-if="donvi.organization_type == 0">
                    ...
                    <InputText spellcheck="false" class="col-4 ip36" v-model="donvi.address" />
                    <label class="col-2 text-right">Tên viết tắt</label>
                    <InputText spellcheck="false" class="col-4 ip36" v-model="donvi.short_name" />
                </div>
                <div class="field col-12 md:col-12 flex" v-if="donvi.organization_type == 0">
                    ...
                    <div class="col-4 p-0">
                        <div class="inputanh relative">
                            ...
                            " />
                            <Button v-if="isDisplayAvt || donvi.logo" style="width: 1.5rem; height: 1.5rem"
                                icon="pi pi-times" @click="delLogo"
        ...
        <template #footer>
            <Button label="Huỷ" icon="pi pi-times" @click="closedisplayAddDonvi"
