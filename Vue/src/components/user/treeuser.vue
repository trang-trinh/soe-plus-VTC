  ...
  switch (props.one) {
    ...
    case false:
      var choses = selectedNodeUser.value.filter(
        (a) =>
          props.selected.findIndex((b) => b["user_id"] === a["user_id"]) === -1
      );
      if (choses.length > 0) {
      ...
      if (notexists.length > 0) {
        for (var i = 0; i < notexists.length; i++) {
          let idx = props.selected.findIndex(
            (a) => a["user_id"] === notexists[i]["user_id"]
          );
          if (idx != -1) {
      ...
      if (data != null) {
        ...
        if (tbs[0] != null && tbs[0].length > 0) {
          ...
          rootusers.value = tbs[0].filter(
            (a) =>
              props.selected.findIndex((b) => b["user_id"] === a["user_id"]) ===
              -1
          );
        ...
        options.value.total = rootusers.length;
      }
      swal.close();
              ...
              <template #body="slotProps">
                <div class="flex">
                  ...
                  <div class="format-center justify-content-left ml-3">
                    <div>
                      <div style="text-align: left;">{{ slotProps.data.full_name }}</div>
