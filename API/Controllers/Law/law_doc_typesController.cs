...
namespace API.Controllers
{
    ...
    public class law_doc_typesController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_Type([System.Web.Mvc.Bind(Include = "law_type_name,law_type_id,created_by,created_date,created_ip,created_token_id")] law_doc_types law_type)
        {
            ...
            try
            {
                ...
                {
                    law_type.nav_type = 0;
                    law_type.created_by = uid;
                    law_type.created_date = DateTime.Now;
                    law_type.created_ip = ip;
                    law_type.created_token_id = tid;
                    db.law_doc_types.Add(law_type);
                    ...
                    if (helper.wlog)
                    {
                        ...
                        log.log_title = "Thêm loại văn bản luật " + law_type.law_type_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = law_type });
                        log.log_module = "Luật - Loại văn bản";
                        log.id_key = law_type.law_type_id.ToString();
            ...
            catch (DbEntityValidationException e)
            {
                ...
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = contents }), domainurl + "law_doc_types/Add_Type", ip, tid, "Lỗi khi thêm loại văn bản luật", 0, "law_doc_types");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_Type([System.Web.Mvc.Bind(Include = "law_type_name,law_type_id,modified_by,modified_date,modified_ip,modified_token_id")] law_doc_types law_type)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    law_type.modified_by = uid;
                    law_type.modified_date = DateTime.Now;
                    law_type.modified_ip = ip;
                    law_type.modified_token_id = tid;
                    db.Entry(law_type).State = EntityState.Modified;
                    await db.SaveChangesAsync();
                    ...
                    if (helper.wlog)
                    {
                        ...
                        cms_logs log = new cms_logs();
                        log.log_title = "Sửa loại văn bản luật " + law_type.law_type_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = law_type });
                        log.log_module = "Luật - Loại văn bản";
                        log.id_key = law_type.law_type_id.ToString();
                        log.created_date = DateTime.Now;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "law_doc_types/Update_Type", ip, tid, "Lỗi khi cập nhật loại văn bản luật", 0, "law_doc_types");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_Type([System.Web.Mvc.Bind(Include = "")][FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    var das = await db.law_doc_types.Where(a => id.Contains(a.law_type_id)).ToListAsync();
                    if (das != null)
                    {
                        List<law_doc_types> del = new List<law_doc_types>();
                        foreach (var da in das)
                        {
                            ...
                            if (helper.wlog)
                            {
                                ...
                                cms_logs log = new cms_logs();
                                log.log_title = "Xóa loại văn bản luật " + da.law_type_name;
                                log.log_module = "Luật - Loại văn bản";
                                log.id_key = da.law_type_id.ToString();
                                log.created_date = DateTime.Now;
                                log.created_by = uid;
                                log.created_token_id = tid;
                                log.created_ip = ip;
                        ...
                        db.law_doc_types.RemoveRange(del);
                    }
                    await db.SaveChangesAsync();
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "law_doc_types/Delete_Type", ip, tid, "Lỗi khi xoá loại văn bản luật", 0, "law_doc_types");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusTypes([System.Web.Mvc.Bind(Include = "IntID,BitTrangthai")] Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    var das = db.law_doc_types.Where(a => a.law_type_id == ID_Change_Status).FirstOrDefault<law_doc_types>();
                    if (das != null)
                    {
                        das.modified_by = uid;
                        ...
                        if (helper.wlog)
                        {
                            ...
                            cms_logs log = new cms_logs();
                            log.log_title = "Sửa trạng thái loại văn bản luật " + das.law_type_name;

                            log.log_module = "Luật - Loại văn bản";
                            log.id_key = das.law_type_id.ToString();
                            log.created_date = DateTime.Now;
                            log.created_by = uid;
                            log.created_token_id = tid;
                            log.created_ip = ip;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = trangthai.IntID, contents }), domainurl + "law_doc_types/Update_StatusTypes", ip, tid, "Lỗi khi cập nhật trạng thái loại văn bản luật", 0, "law_doc_types");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        public async Task<HttpResponseMessage> ImportExcel()
        {
            ...
            using (DBEntities db = new DBEntities())
            {
                ...
                try
                {
                    ...
                    var task = Request.Content.ReadAsMultipartAsync(provider).ContinueWith<HttpResponseMessage>(t =>
                    {
                        ...
                        using (ExcelPackage pck = new ExcelPackage(temp))
                        {
                            try
                            {
                                List<law_doc_types> dvs = new List<law_doc_types>();
                                ...
                                for (int i = 5; i <= ws.Dimension.End.Row; i++)
                                {
                                    ...
                                    law_doc_types dv = new law_doc_types();
                                    ...
                                    for (int j = 1; j <= ws.Dimension.End.Column; j++)
                                    {
                                        ...
                                        {
                                            case "law_type_code":
                                                dv.law_type_code = vl != null ? vl.ToString() : null;
                                                break;
                                            case "law_type_name":
                                                dv.law_type_name = vl != null ? vl.ToString() : null;
                                                break;
                                            case "is_order":
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fpath, contents }), domainurl + "/law_doc_types/ImportExcel", ip, tid, "Lỗi khi Import Excel", 0, "law_doc_types");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        public async Task<HttpResponseMessage> GetDataProc([System.Web.Mvc.Bind(Include = "str")][FromBody] JObject data)
        {
            ...
            try
            {
                ...
                if (helper.wlog)
                {
                    using (DBEntities db = new DBEntities())
                    {
                        ...
                        log.controller = domainurl + "law_doc_types/GetDataProc";
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = proc, contents }), domainurl + "law_doc_types/GetDataProc", ip, tid, nameErrProc, 0, "law_doc_types");
                if (!helper.debug)
                {
