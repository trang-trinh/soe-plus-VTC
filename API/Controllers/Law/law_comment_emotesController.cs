...
namespace API.Controllers
{
    ...
    public class law_comment_emotesController : ApiController
    {
        ...
        //[AllowAnonymous]
        [HttpPost]
        public async Task<HttpResponseMessage> Add_Emote([System.Web.Mvc.Bind(Include = "created_by,created_date,created_ip,created_token_id")] law_comment_emotes emote)
        {
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    db.law_comment_emotes.Add(emote);
            ...
            catch (DbEntityValidationException e)
            {
                ...
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = contents }), domainurl + "law_comment_emotes/Add_Emote", ip, tid, "Lỗi khi thêm biểu tượng cảm xúc cho comment luật", 0, "law_comment_emotes");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        public async Task<HttpResponseMessage> GetDataProc([System.Web.Mvc.Bind(Include = "str")][FromBody] JObject data)
        {
            ...
            try
            {
                ...
                if (helper.wlog)
                {
                    using (DBEntities db = new DBEntities())
                    {
                        ...
                        log.controller = domainurl + "law_comment_emotes/GetDataProc";
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = proc, contents }), domainurl + "law_comment_emotes/GetDataProc", ip, tid, nameErrProc, 0, "law_comment_emotes");
                if (!helper.debug)
                {
