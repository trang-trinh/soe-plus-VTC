...
namespace API.Controllers
{
    ...
    public class law_doc_languageController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_Language([System.Web.Mvc.Bind(Include = "language_name,law_language_id,created_by,created_date,created_ip,created_token_id")] law_doc_language language)
        {
            ...
            try
            {
                ...
                {
                    language.created_by = uid;
                    language.created_date = DateTime.Now;
                    language.created_ip = ip;
                    language.created_token_id = tid;
                    db.law_doc_language.Add(language);
                    ...
                    if (helper.wlog)
                    {
                        ...
                        log.log_title = "Thêm ngôn ngữ " + language.language_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = language });
                        log.log_module = "Luật - Ngôn ngữ";
                        log.id_key = language.law_language_id.ToString();
            ...
            catch (DbEntityValidationException e)
            {
                ...
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = contents }), domainurl + "law_doc_language/Add_Language", ip, tid, "Lỗi khi thêm ngôn ngữ", 0, "law_doc_language");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_Language([System.Web.Mvc.Bind(Include = "language_name,law_language_id,modified_by,modified_date,modified_ip,modified_token_id")] law_doc_language language)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    language.modified_by = uid;
                    language.modified_date = DateTime.Now;
                    language.modified_ip = ip;
                    language.modified_token_id = tid;
                    ...
                    if (helper.wlog)
                    {
                        ...
                        cms_logs log = new cms_logs();
                        log.log_title = "Sửa ngôn ngữ " + language.language_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = language });
                        log.log_module = "Luật - Ngôn ngữ";
                        log.id_key = language.law_language_id.ToString();
                        log.created_date = DateTime.Now;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "law_doc_language/Update_Language", ip, tid, "Lỗi khi cập nhật ngôn ngữ", 0, "law_doc_language");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_Language([System.Web.Mvc.Bind(Include = "")][FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    var das = await db.law_doc_language.Where(a => id.Contains(a.law_language_id)).ToListAsync();
                    if (das != null)
                    {
                        List<law_doc_language> del = new List<law_doc_language>();
                        foreach (var da in das)
                        {
                            ...
                            if (helper.wlog)
                            {
                                ...
                                cms_logs log = new cms_logs();
                                log.log_title = "Xóa ngôn ngữ " + da.language_name;
                                log.log_module = "Luật - Ngôn ngữ";
                                log.id_key = da.law_language_id.ToString();
                                log.created_date = DateTime.Now;
                                log.created_by = uid;
                                log.created_token_id = tid;
                                log.created_ip = ip;
                        ...
                        db.law_doc_language.RemoveRange(del);
                    }
                    await db.SaveChangesAsync();
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "law_doc_language/Delete_Language", ip, tid, "Lỗi khi xoá ngôn ngữ", 0, "law_doc_language");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusLanguage([System.Web.Mvc.Bind(Include = "IntID,BitTrangthai")] Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    var das = db.law_doc_language.Where(a => a.law_language_id == ID_Change_Status).FirstOrDefault<law_doc_language>();
                    if (das != null)
                    {
                        das.modified_by = uid;
                        ...
                        if (helper.wlog)
                        {
                            ...
                            cms_logs log = new cms_logs();
                            log.log_title = "Sửa trạng thái ngôn ngữ " + das.language_name;

                            log.log_module = "Luật - Ngôn ngữ";
                            log.id_key = das.law_language_id.ToString();
                            log.created_date = DateTime.Now;
                            log.created_by = uid;
                            log.created_token_id = tid;
                            log.created_ip = ip;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = trangthai.IntID, contents }), domainurl + "law_doc_language/Update_StatusLanguage", ip, tid, "Lỗi khi cập nhật trạng thái ngôn ngữ", 0, "law_doc_language");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        public async Task<HttpResponseMessage> ImportExcel()
        {
            ...
            using (DBEntities db = new DBEntities())
            {
                ...
                try
                {
                    ...
                    var task = Request.Content.ReadAsMultipartAsync(provider).ContinueWith<HttpResponseMessage>(t =>
                    {
                        ...
                        using (ExcelPackage pck = new ExcelPackage(temp))
                        {
                            try
                            {
                                List<law_doc_language> dvs = new List<law_doc_language>();
                                ...
                                for (int i = 5; i <= ws.Dimension.End.Row; i++)
                                {
                                    ...
                                    law_doc_language dv = new law_doc_language();
                                    ...
                                    for (int j = 1; j <= ws.Dimension.End.Column; j++)
                                    {
                                        ...
                                        {
                                            case "language_name":
                                                dv.language_name = vl != null ? vl.ToString() : null;
                                                break;
                                            case "is_order":
                                ...
                                if (dvs.Count > 0)
                                {
                                    db.law_doc_language.AddRange(dvs);
                                }
                                db.SaveChanges();
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fpath, contents }), domainurl + "/law_doc_language/ImportExcel", ip, tid, "Lỗi khi Import Excel", 0, "law_doc_language");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        public async Task<HttpResponseMessage> GetDataProc([System.Web.Mvc.Bind(Include = "str")][FromBody] JObject data)
        {
            ...
            try
            {
                ...
                if (helper.wlog)
                {
                    using (DBEntities db = new DBEntities())
                    {
                        ...
                        log.controller = domainurl + "law_doc_language/GetDataProc";
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = proc, contents }), domainurl + "law_doc_language/GetDataProc", ip, tid, nameErrProc, 0, "law_doc_language");
                if (!helper.debug)
                {
