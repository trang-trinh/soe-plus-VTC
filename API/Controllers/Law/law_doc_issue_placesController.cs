...
namespace API.Controllers
{
    ...
    public class law_doc_issue_placesController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_IssuePlace([System.Web.Mvc.Bind(Include = "issue_place_name,issue_place_id,created_by,created_date,created_ip,created_token_id")] law_doc_issue_places law_issue_place)
        {
            ...
            try
            {
                ...
                {
                    law_issue_place.created_by = uid;
                    law_issue_place.created_date = DateTime.Now;
                    law_issue_place.created_ip = ip;
                    law_issue_place.created_token_id = tid;
                    db.law_doc_issue_places.Add(law_issue_place);
                    ...
                    if (helper.wlog)
                    {
                        ...
                        log.log_title = "Thêm nơi ban hành " + law_issue_place.issue_place_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = law_issue_place });
                        log.log_module = "Luật - Nơi ban hành";
                        log.id_key = law_issue_place.issue_place_id.ToString();
            ...
            catch (DbEntityValidationException e)
            {
                ...
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = contents }), domainurl + "law_doc_issue_places/Add_IssuePlace", ip, tid, "Lỗi khi thêm nơi ban hành", 0, "law_doc_issue_places");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_IssuePlace([System.Web.Mvc.Bind(Include = "issue_place_name,issue_place_id,modified_by,modified_date,modified_ip,modified_token_id")] law_doc_issue_places law_issue_place)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    law_issue_place.modified_by = uid;
                    law_issue_place.modified_date = DateTime.Now;
                    law_issue_place.modified_ip = ip;
                    law_issue_place.modified_token_id = tid;
                    db.Entry(law_issue_place).State = EntityState.Modified;
                    await db.SaveChangesAsync();
                    ...
                    if (helper.wlog)
                    {
                        ...
                        cms_logs log = new cms_logs();
                        log.log_title = "Sửa nơi ban hành " + law_issue_place.issue_place_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = law_issue_place });
                        log.log_module = "Luật - Nơi ban hành";
                        log.id_key = law_issue_place.issue_place_id.ToString();
                        log.created_date = DateTime.Now;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "law_doc_issue_places/Update_IssuePlace", ip, tid, "Lỗi khi cập nhật nơi ban hành", 0, "law_doc_issue_places");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_IssuePlace([System.Web.Mvc.Bind(Include = "")][FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    var das = await db.law_doc_issue_places.Where(a => id.Contains(a.issue_place_id)).ToListAsync();
                    if (das != null)
                    {
                        List<law_doc_issue_places> del = new List<law_doc_issue_places>();
                        foreach (var da in das)
                        {
                            ...
                            if (helper.wlog)
                            {
                                ...
                                cms_logs log = new cms_logs();
                                log.log_title = "Xóa nơi ban hành " + da.issue_place_name;
                                log.log_module = "Luật - Nơi ban hành";
                        ...
                        db.law_doc_issue_places.RemoveRange(del);
                    }
                    await db.SaveChangesAsync();
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "law_doc_issue_places/Delete_IssuePlace", ip, tid, "Lỗi khi xoá nơi ban hành", 0, "law_doc_issue_places");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusIssuePlaces([System.Web.Mvc.Bind(Include = "IntID,BitTrangthai")] Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    var das = db.law_doc_issue_places.Where(a => a.issue_place_id == ID_Change_Status).FirstOrDefault<law_doc_issue_places>();
                    if (das != null)
                    {
                        das.modified_by = uid;
                        ...
                        if (helper.wlog)
                        {
                            ...
                            cms_logs log = new cms_logs();
                            log.log_title = "Sửa trạng thái nơi ban hành " + das.issue_place_name;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = trangthai.IntID, contents }), domainurl + "law_doc_issue_places/Update_StatusIssuePlaces", ip, tid, "Lỗi khi cập nhật trạng thái nơi ban hành", 0, "law_doc_issue_places");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        public async Task<HttpResponseMessage> ImportExcel()
        {
            ...
            using (DBEntities db = new DBEntities())
            {
                ...
                try
                {
                    ...
                    var task = Request.Content.ReadAsMultipartAsync(provider).ContinueWith<HttpResponseMessage>(t =>
                    {
                        ...
                        using (ExcelPackage pck = new ExcelPackage(temp))
                        {
                            try
                            {
                                List<law_doc_issue_places> dvs = new List<law_doc_issue_places>();
                                ...
                                for (int i = 5; i <= ws.Dimension.End.Row; i++)
                                {
                                    ...
                                    law_doc_issue_places dv = new law_doc_issue_places();
                                ...
                                if (dvs.Count > 0)
                                {
                                    db.law_doc_issue_places.AddRange(dvs);
                                }
                                db.SaveChanges();
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fpath, contents }), domainurl + "/law_doc_issue_places/ImportExcel", ip, tid, "Lỗi khi Import Excel", 0, "law_doc_issue_places");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        public async Task<HttpResponseMessage> GetDataProc([System.Web.Mvc.Bind(Include = "str")][FromBody] JObject data)
        {
            ...
            try
            {
                ...
                if (helper.wlog)
                {
                    using (DBEntities db = new DBEntities())
                    {
                        ...
                        log.controller = domainurl + "law_doc_issue_places/GetDataProc";
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = proc, contents }), domainurl + "law_doc_issue_places/GetDataProc", ip, tid, nameErrProc, 0, "law_doc_issue_places");
                if (!helper.debug)
                {
