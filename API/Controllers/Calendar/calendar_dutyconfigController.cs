...
namespace API.Controllers.Calendar
{
    ...
    public class calendar_dutyconfigController : ApiController
    {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> update_duty_procedureform()
        {
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    calendar_duty_procedureform model = JsonConvert.DeserializeObject<calendar_duty_procedureform>(md);
                    ...
                    if (isAdd)
                    {
                        ...
                        db.calendar_duty_procedureform.Add(model);
                    }
                    else
                    {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> update_duty_signform()
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlaw = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    bool isAdd = bool.Parse(provider.FormData.GetValues("isAdd").SingleOrDefault());
                    var md = provider.FormData.GetValues("model").SingleOrDefault();
                    calendar_duty_signform model = JsonConvert.DeserializeObject<calendar_duty_signform>(md);
                    ...
                    if (isAdd)
                    {
                        ...
                        db.calendar_duty_signform.Add(model);
                    }
                    else
                    {
                        model.modified_by = uid;
                    ...
                    #endregion
                    #region users
                    var userold = await db.calendar_duty_signuserform.Where(x => x.signform_id == model.signform_id).ToListAsync();
                    if (userold.Count > 0)
                    {
                        db.calendar_duty_signuserform.RemoveRange(userold);
                    }
                    if (users.Count > 0)
                    {
                        List<calendar_duty_signuserform> signusers = new List<calendar_duty_signuserform>();
                        int stt = 0;
                        foreach (var users_id in users)
                        {
                            stt++;
                            var signuser = new calendar_duty_signuserform();
                        ...
                        {
                            db.calendar_duty_signuserform.AddRange(signusers);
                        }
                    }
                    #endregion
                    #region add law_logs
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> update_status_duty_procedureform([System.Web.Mvc.Bind(Include = "id,status")][FromBody] JObject data)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var model = await db.calendar_duty_procedureform.FindAsync(id);
                        if (model != null)
                        {
                            model.status = status;
                            #region add cms_logs
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> update_status_duty_signform([System.Web.Mvc.Bind(Include = "id,status")][FromBody] JObject data)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var model = await db.calendar_duty_signform.FindAsync(id);
                        if (model != null)
                        {
                            model.status = status;
                            #region add cms_logs
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> delete_duty_procedureform([System.Web.Mvc.Bind(Include = "")][FromBody] List<string> ids)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var das = await db.calendar_duty_procedureform.Where(a => ids.Contains(a.procedureform_id)).ToListAsync();
                        List<string> paths = new List<string>();
                        if (das != null)
                        {
                            List<calendar_duty_procedureform> del = new List<calendar_duty_procedureform>();
                            ...
                            db.calendar_duty_procedureform.RemoveRange(del);
                        }
                        await db.SaveChangesAsync();
                        return Request.CreateResponse(HttpStatusCode.OK, new { err = "0" });
                    }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> delete_duty_signform([System.Web.Mvc.Bind(Include = "")][FromBody] List<string> ids)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var das = await db.calendar_duty_signform.Where(a => ids.Contains(a.signform_id)).ToListAsync();
                        List<string> paths = new List<string>();
                        if (das != null)
                        {
                            List<calendar_duty_signform> del = new List<calendar_duty_signform>();
                            foreach (var da in das)
                            {
                                del.Add(da);
                                #region add cms_logs
                            ...
                            db.calendar_duty_signform.RemoveRange(del);
                        }
                        await db.SaveChangesAsync();
                        return Request.CreateResponse(HttpStatusCode.OK, new { err = "0" });
                    }
