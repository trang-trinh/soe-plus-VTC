...
namespace API.Controllers
{
    ...
    public class ca_statusController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_status([System.Web.Mvc.Bind(Include = "stamp_name,stamp_id,created_by,created_date,created_ip,created_token_id")] doc_ca_status Status)
        {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_status([System.Web.Mvc.Bind(Include = "stamp_id,stamp_name,modified_by,modified_date,modified_ip,modified_token_id")] doc_ca_status Status)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    Status.modified_by = uid;
                    Status.modified_date = DateTime.Now;
                    Status.modified_ip = ip;
                    Status.modified_token_id = tid;
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_status([System.Web.Mvc.Bind(Include = "")][FromBody] List<string> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_Status_Status([System.Web.Mvc.Bind(Include = "IntID,BitTrangthai")] Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_Handle_Status([System.Web.Mvc.Bind(Include = "TextID,BitTrangthai")] Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = trangthai.IntID, contents }), domainurl + "Status/Update_HandleStatus_Status", ip, tid, "Lỗi khi cập nhật trạng thái Trạng thái", 0, "Status");
                if (!helper.debug)
                {
                    contents = "";
                }
