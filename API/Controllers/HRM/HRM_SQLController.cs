...
" LEFT JOIN hrm_profile hp ON cts.profile_id = hp.profile_id ";
 ...
 " from smart_proc hcal   ";
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_review_form_users([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            string domainurl = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/";
            string sql = "";
            string sqlCount = "  select count(*) as totalRecords from hrm_review_form_users  hcal ";
            try
            {
                var selectStr = filterSQL.id == null ? (" Select TOP(" + filterSQL.PageSize + @") ") : "Select ";
                sql = selectStr + " hcal.*   " +
 " from hrm_review_form_users hcal   ";
                string super = claims.Where(x => x.Type == "super").FirstOrDefault()?.Value;
                string WhereSQL = "";
                ...
                if (!string.IsNullOrWhiteSpace(filterSQL.Search))
                {
                    WhereSQL = (WhereSQL.Trim() != "" ? (WhereSQL + " And  ") : "")
                        ...
                        + " ( CONTAINS(hcal.profile_user_name,'\"*" + filterSQL.Search.ToUpper() + "*\"') or hcal.profile_user_name like N'%" + filterSQL.Search.ToUpper() + "%' )";
                }
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_SQL/Filter_hrm_review_form_users", ip, tid, "Lỗi khi gọi Filter_hrm_review_form_users", 0, "hrm_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
            ...
            {
                string contents = helper.ExceptionMessage(e);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_SQL/Filter_hrm_review_form_users", ip, tid, "Lỗi khi gọi proc Filter_hrm_review_form_users", 0, "hrm_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
