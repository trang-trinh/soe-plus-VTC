    ...
    public class hrm_ca_SQLController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Filter_hrm_ca_decision([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            ...
            try
            {
                ...
                string WhereSQL = "";
                string checkOrgz = super == "True" ? " (  tm.organization_id =" + dvid + " OR tm.organization_id IN (SELECT *    FROM dbo.udf_getChildOrg(" + dvid + ") uco)  ) " : (" ( ( tm.organization_id = 0 or tm.organization_id =" + dvid + " ) or tm.is_system=1 ) ");
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_ca_config_work_location([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_ca_allowance_wage([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            string domainurl = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/";
            string sql = "";
            string sqlCount = " Select count(allowance_wage_id)  as totalRecords from hrm_ca_allowance_wage tm";
            try
            {
                var selectStr = filterSQL.id == null ? (" Select TOP(" + filterSQL.PageSize + @") ") : "Select ";
                sql = selectStr + " tm.* " +

                    " from hrm_ca_allowance_wage tm ";
                string super = claims.Where(x => x.Type == "super").FirstOrDefault()?.Value;
                string WhereSQL = "";
                ...
                if (!string.IsNullOrWhiteSpace(filterSQL.Search))
                {
                    WhereSQL = (WhereSQL.Trim() != "" ? (WhereSQL + " And  ") : "")
                        ...
                        + " CONTAINS( tm.allowance_wage_name,'\"*" + filterSQL.Search.ToUpper() + "*\"') " +
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_allowance_wage", ip, tid, "Lỗi khi gọi Filter_hrm_ca_allowance_wage", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
            ...
            {
                string contents = helper.ExceptionMessage(e);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_allowance_wage", ip, tid, "Lỗi khi gọi proc Filter_hrm_ca_allowance_wage", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_config_work_location([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            string domainurl = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/";
            string sql = "";
            string sqlCount = " Select count(config_work_location_id)  as totalRecords from hrm_config_work_location tm";
            try
            {
                var selectStr = filterSQL.id == null ? (" Select TOP(" + filterSQL.PageSize + @") ") : "Select ";
                sql = selectStr + " tm.* " +

                    " from hrm_config_work_location tm ";
                string super = claims.Where(x => x.Type == "super").FirstOrDefault()?.Value;
                string WhereSQL = "";
                ...
                if (!string.IsNullOrWhiteSpace(filterSQL.Search))
                {
                    WhereSQL = (WhereSQL.Trim() != "" ? (WhereSQL + " And  ") : "")
                        ...
                        + " CONTAINS( tm.config_work_location_name,'\"*" + filterSQL.Search.ToUpper() + "*\"') " +
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_config_work_location", ip, tid, "Lỗi khi gọi Filter_hrm_config_work_location", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
            ...
            {
                string contents = helper.ExceptionMessage(e);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_config_work_location", ip, tid, "Lỗi khi gọi proc Filter_hrm_config_work_location", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_ca_holiday_type([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            string domainurl = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/";
            string sql = "";
            string sqlCount = " Select count(holiday_type_id)  as totalRecords from hrm_ca_holiday_type tm";
            try
            {
                var selectStr = filterSQL.id == null ? (" Select TOP(" + filterSQL.PageSize + @") ") : "Select ";
                sql = selectStr + " tm.* " +

                    " from hrm_ca_holiday_type tm ";
                string super = claims.Where(x => x.Type == "super").FirstOrDefault()?.Value;
                string WhereSQL = "";
                ...
                if (!string.IsNullOrWhiteSpace(filterSQL.Search))
                {
                    WhereSQL = (WhereSQL.Trim() != "" ? (WhereSQL + " And  ") : "")
                        ...
                        + " CONTAINS( tm.holiday_type_name,'\"*" + filterSQL.Search.ToUpper() + "*\"') " +
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_holiday_type", ip, tid, "Lỗi khi gọi Filter_hrm_ca_holiday_type", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
            ...
            {
                string contents = helper.ExceptionMessage(e);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_holiday_type", ip, tid, "Lỗi khi gọi proc Filter_hrm_ca_holiday_type", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
