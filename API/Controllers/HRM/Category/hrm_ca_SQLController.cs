...
namespace API.Controllers.HRM.Category
{
    ...
    public class hrm_ca_SQLController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Filter_hrm_ca_leaving_reason([System.Web.Mvc.Bind(Include = "fieldSQLS,Search,sqlO,PageNo,PageSize,next,id")][FromBody] FilterSQL filterSQL)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            if (identity == null)
            ...
            string domainurl = HttpContext.Current.Request.Url.Scheme + "://" + HttpContext.Current.Request.Url.Host + ":" + HttpContext.Current.Request.Url.Port + "/";
            string sql = "";
            string sqlCount = " Select count(leaving_reason_id)  as totalRecords from hrm_ca_leaving_reason tm";
            try
            {
                var selectStr = filterSQL.id == null ? (" Select TOP(" + filterSQL.PageSize + @") ") : "Select ";
                sql = selectStr + " tm.* " +

                    " from hrm_ca_leaving_reason tm ";
                string super = claims.Where(x => x.Type == "super").FirstOrDefault()?.Value;
                string WhereSQL = "";
                ...
                if (!string.IsNullOrWhiteSpace(filterSQL.Search))
                {
                    WhereSQL = (WhereSQL.Trim() != "" ? (WhereSQL + " And  ") : "")
                        ...
                        + " CONTAINS( tm.leaving_reason_name,'\"*" + filterSQL.Search.ToUpper() + "*\"') " +
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_leaving_reason", ip, tid, "Lỗi khi gọi Filter_hrm_ca_leaving_reason", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
            ...
            {
                string contents = helper.ExceptionMessage(e);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = sql, contents }), domainurl + "/hrm_ca_SQL/Filter_hrm_ca_leaving_reason", ip, tid, "Lỗi khi gọi proc Filter_hrm_ca_leaving_reason", 0, "hrm_ca_SQL");
                if (!helper.debug)
                {
                    contents = "";
                }
